...
pipeline {
  ...
  stages {
    stage('ASSEMBLY') {
      ...
      steps {
        cambpmRunMaven('.', 'clean install source:jar -Pdistro,distro-ce,distro-wildfly,distro-webjar com.mycila:license-maven-plugin:check', false, true)
        ...
        script {
          ...
          if (cambpmWithLabels('master')) {
            cambpmRunMaven('.', 'org.sonatype.plugins:nexus-staging-maven-plugin:deploy-staged -DaltStagingDirectory=${WORKSPACE}/staging -DskipStaging=true', false, true)
          }
        }
