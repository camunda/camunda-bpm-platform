 ...
 */
package com.camunda.fox.platform.subsystem.impl.deployment.scanner;
 ...
 * @author Falko Menge
 */
public class VfsProcessApplicationResourceScanner {
    ...
    try {
      List<VirtualFile> processes = processArchiveRoot.getChildrenRecursively(new VirtualFileFilter() {
        public boolean accepts(VirtualFile file) {
          return ScanningUtil.isDeployable(file.getName());
        }
      });
      for (final VirtualFile process : processes) {
        ...
        List<VirtualFile> diagrams = process.getParent().getChildren(new VirtualFileFilter() {
          public boolean accepts(VirtualFile file) {
            return ScanningUtil.isDiagramForProcess(file.getName(), process.getName());
          }
        });
