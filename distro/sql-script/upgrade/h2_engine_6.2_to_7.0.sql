...
/** add ACT_INST_ID_ column to variables table */
alter table ACT_RU_VARIABLE
    add ACT_INST_ID_ varchar(64);

/** populate ACT_INST_ID_ from executions (order is important) */
/** task variables */
UPDATE ACT_RU_VARIABLE V
SET ACT_INST_ID_ = (
  SELECT ACT_INST_ID_ 
  FROM ACT_RU_EXECUTION E
  WHERE V.EXECUTION_ID_ = E.ID_
)
WHERE ACT_INST_ID_ IS NULL AND TASK_ID_ IS NOT NULL;
...
WHERE ACT_INST_ID_ IS NULL;

/** subprocess variables */
UPDATE ACT_RU_VARIABLE V
SET ACT_INST_ID_ = (
  SELECT PARENT.ACT_INST_ID_ 
  FROM ACT_RU_EXECUTION E 
    INNER JOIN ACT_RU_EXECUTION PARENT
    ON E.PARENT_ID_ = PARENT.ID_
  WHERE V.EXECUTION_ID_ = E.ID_
  AND E.IS_SCOPE_ = true
