...
export JBOSS_HOME="$(dirname "$0")/server/wildfly-${version.wildfly11}"

BROWSERS="gnome-www-browser x-www-browser firefox chromium chromium-browser google-chrome"

echo "starting camunda BPM ${project.version} on Wildfly Application Server ${version.wildfly11}";
...
/bin/sh "$(dirname "$0")/server/wildfly-${version.wildfly11}/bin/standalone.sh"
