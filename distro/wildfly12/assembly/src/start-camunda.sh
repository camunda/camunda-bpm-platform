...
export JBOSS_HOME="$(dirname "$0")/server/wildfly-${version.wildfly12}"

BROWSERS="gnome-www-browser x-www-browser firefox chromium chromium-browser google-chrome"

echo "starting camunda BPM ${project.version} on Wildfly Application Server ${version.wildfly12}";
...
/bin/sh "$(dirname "$0")/server/wildfly-${version.wildfly12}/bin/standalone.sh"
