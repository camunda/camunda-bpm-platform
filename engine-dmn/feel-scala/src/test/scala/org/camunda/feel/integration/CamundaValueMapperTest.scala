...
class CamundaValueMapperTest extends FlatSpec with Matchers {
  ...
  it should "map Camunda Spin JSON object as context" in {
    ...
    camundaFeelEngine.evaluateSimpleExpression[Any](""" json """, context) should be (Map(
      "customer" -> "Kermit",
      "language" -> "en"
    ).asJava)
  }
  ...
  it should "map a nested Camunda Spin JSON object as context" in {
    ...
    val json: SpinJsonNode = Spin.JSON("""{"customer": "Kermit", "address": {"city": "Berlin", "zipCode": 10961}}""")
  ...
  it should "map Camunda Spin XML object with attributes" in {
    ...
    camundaFeelEngine.evaluateSimpleExpression[Any](""" xml """, context) should be (Map("customer" -> Map(
      "@name" -> "Kermit",
      "@language" -> "en"
    ).asJava).asJava)
  }
  ...
  it should "map Camunda Spin XML object with list of child objects" in {
    ...
    camundaFeelEngine.evaluateSimpleExpression[Any](""" xml """, context) should be (Map("data" -> Map(
      "customer" -> List(
  ...
  it should "map Camunda Spin XML object without content" in {
    ...
    camundaFeelEngine.evaluateSimpleExpression[Any](""" xml """, context) should be (Map("customer" -> null).asJava)
  }
