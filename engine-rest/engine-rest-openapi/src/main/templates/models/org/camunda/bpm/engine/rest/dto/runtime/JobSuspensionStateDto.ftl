...
If you come here from PUT /job/{id}/suspended please note that this enpoint uses SuspensionStateDto because it only uses the suspended attribute.
Should you need to extend it, it is probably a good idea to create an additional file.-->

<@lib.dto extends="SuspensionStateDto"
          desc = "Defines by which selection criterion to activate or suspend jobs.
                  This seleciton criterion are mutually exclusive and can only be on of:
                  * `jobDefinitionId`
                  * `processDefinitionId`
                  * `processInstanceId`
    ...
    <@lib.property
        ...
        desc = "Only activate or suspend jobs of a process definition which belongs to a tenant
                with the given id. Works only when selection with `processDefinitionKey`."
    />
    ...
    <@lib.property
        ...
        desc = "Only activate or suspend jobs of a process definition which belongs to no tenant.
                Value may only be `true`, as `false` is the default behavior. Works only when selection with `processDefinitionKey`."
        last = true
    />
