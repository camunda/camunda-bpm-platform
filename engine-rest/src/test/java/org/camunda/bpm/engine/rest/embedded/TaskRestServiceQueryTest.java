...
public class TaskRestServiceQueryTest extends AbstractTaskRestServiceQueryTest {
  ...
  public void testSimpleTaskQuery() {
    ...
    Assert.assertEquals(ExampleDataProvider.EXAMPLE_TASK_NAME, returnedTaskName);
    Assert.assertEquals(ExampleDataProvider.EXAMPLE_TASK_ID, returnedId);
    Assert.assertEquals(ExampleDataProvider.EXAMPLE_TASK_ASSIGNEE_NAME, returendAssignee);
    Assert.assertEquals(ExampleDataProvider.EXAMPLE_TASK_CREATE_TIME, returnedCreateTime);
    Assert.assertEquals(ExampleDataProvider.EXAMPLE_TASK_DUE_DATE, returnedDueDate);
    Assert.assertEquals(ExampleDataProvider.EXAMPLE_TASK_DELEGATION_STATE.toString(), returnedDelegationState);
    Assert.assertEquals(ExampleDataProvider.EXAMPLE_TASK_DESCRIPTION, returnedDescription);
    Assert.assertEquals(ExampleDataProvider.EXAMPLE_TASK_EXECUTION_ID, returnedExecutionId);
    Assert.assertEquals(ExampleDataProvider.EXAMPLE_TASK_OWNER, returnedOwner);
    Assert.assertEquals(ExampleDataProvider.EXAMPLE_TASK_PARENT_TASK_ID, returnedParentTaskId);
    Assert.assertEquals(ExampleDataProvider.EXAMPLE_TASK_PRIORITY, returnedPriority);
    Assert.assertEquals(ExampleDataProvider.EXAMPLE_PROCESS_DEFINITION_ID, returnedProcessDefinitionId);
    Assert.assertEquals(ExampleDataProvider.EXAMPLE_PROCESS_INSTANCE_ID, returnedProcessInstanceId);
    Assert.assertEquals(ExampleDataProvider.EXAMPLE_TASK_DEFINITION_KEY, returnedTaskDefinitionKey);
