 ...
 * Process application context is important for the engine to access custom classes as well as
 * process-application-level entities like script engines or Spin data formats.
 *
 * <p>
 * By default, the process engine only guarantees to switch into the context of the process
 * application when it executes custom code (e.g. a JavaDelegate). This utility allows to declare a
 * process application into which the process engine then switches as soon as it begins executing a
 * command.
 *
 ...
 *   runtimeService.setVariable("processInstanceId", "variableName", Variables.objectValue(anObject)
 ...
 * as configured in that process application to serialize the object value. Without declaring the
 * context, the global json data format is used.
 *
 * <p>
...
public class ProcessApplicationContext {
   ...
   * {@link #clear()} is called. The context is bound to the current thread. This method should
   * always be used in a try-finally block to ensure that {@link #clear()} is called under any
   * circumstances.
   *
   ...
   *          the process application to switch into
   */
  ...
  /**
   * <p>
   * Takes a callable and executes all engine API invocations within that callable in the context of
   * the given process application
   *
   * <p>
   * Equivalent to
   * 
   ...
   *          the name of the process application to switch into
   */
  public static <T> T withProcessApplicationContext(Callable<T> callable,
      String processApplicationName) throws Exception {
    try {
   ...
   */
  public static <T> T withProcessApplicationContext(Callable<T> callable,
      ProcessApplicationReference reference) throws Exception {
    try {
   ...
   */
  public static <T> T withProcessApplicationContext(Callable<T> callable,
      ProcessApplicationInterface processApplication) throws Exception {
    try {
