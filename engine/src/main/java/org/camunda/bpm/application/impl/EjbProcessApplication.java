 ...
 * nature of the {@link ProcessApplicationReference} held by the process engine.
 * </p>
 *
 * <h2>Usage</h2>
 * <p>
 * In order to add a custom EJB process application to your application, extend this class and
 ...
 * Application and get EJB invocation semantics for the invocation. For example, if your process
 * application provides a {@link JavaDelegate} implementation, the process engine will call the
 * {@link EjbProcessApplication EjbProcessApplication's}
 * {@link #execute(java.util.concurrent.Callable)} Method and from that method invoke the
 * {@link JavaDelegate}. This makes sure that
 * <ul>
 * <li>the call is intercepted by the EJB container and "enters" the process application
 * legally.</li>
 * <li>the {@link JavaDelegate} may take advantage of the {@link EjbProcessApplication}'s invocation
 * context and resolve resources from the component's Environment (such as a
 * <code>java:comp/BeanManager</code>).
 * </ul>
 * </p>
 *
 *
 ...
 * {@link ManagementService#registerProcessApplication(String, ProcessApplicationReference)}, the
 * process application passes a reference to itself to the process engine. This reference allows the
 * process engine to reference the process application. The {@link EjbProcessApplication} takes
 * advantage of the Ejb Containers naming context and passes a reference containing the
 * EJBProcessApplication's Component Name to the process engine. Whenever the process engine needs
 * access to process application, the actual component instance is looked up and invoked.
 * </p>
 *
 * @author Daniel Meyer
 *
