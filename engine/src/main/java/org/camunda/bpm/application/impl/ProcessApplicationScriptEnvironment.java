...
public class ProcessApplicationScriptEnvironment {
  ...
  /**
   * <p>
   * Returns an instance of {@link ScriptEngine} for the given <code>scriptEngineName</code>.
   * </p>
   *
   * <p>
   * Iff the given parameter <code>cache</code> is set <code>true</code>, then the instance
   * {@link ScriptEngine} will be cached.
   * </p>
   *
   * @param scriptEngineName
   *          the name of the {@link ScriptEngine} to return
   * @param cache
   *          a boolean value which indicates whether the {@link ScriptEngine} should be cached or
   *          not.
   *
  ...
  public ScriptEngine getScriptEngineForName(String scriptEngineName, boolean cache) {
    if (processApplicationScriptEngineResolver == null) {
      synchronized (this) {
        if (processApplicationScriptEngineResolver == null) {
          processApplicationScriptEngineResolver = new ScriptEngineResolver(
              new ScriptEngineManager(getProcessApplicationClassloader()));
        }
      }
    }
