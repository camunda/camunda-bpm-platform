 ...
 * <p>
 * An authorization is created between a user/group and a resource. It describes the user/group's
 * <em>permissions</em> to access that resource. An authorization may express different permissions,
 * such as the permission to READ, UPDATE, DELETE the resource. (See {@link Authorization} for
 * details).
 * </p>
 * 
 * <h2>Granting / revoking permissions</h2>
 * <p>
 * In order to grant the permission to access a certain resource, an authorization object is
 * created:
 * 
 * <pre>
 ...
 * </pre>
 * 
 * As a result, the given user or group will have permission to READ the referenced process
 * definition.
 * </p>
 *
 * @author Daniel Meyer
...
public interface AuthorizationService {
  ...
  /**
   * <p>
   * Returns a new (transient) {@link Authorization} object. The Object is not yet persistent and
   * must be saved using the {@link #saveAuthorization(Authorization)} method.
   * </p>
   * 
   * @param authorizationType
   *          the type of the authorization. Legal values: {@link Authorization#AUTH_TYPE_GLOBAL},
   ...
   *           {@link Resources#AUTHORIZATION}.
   */
  public Authorization createNewAuthorization(int authorizationType);

  /**
   * Allows saving an {@link Authorization} object. Use this method for persisting new transient
   * {@link Authorization} objects obtained through {@link #createNewAuthorization(int)} or for
   ...
   * @throws AuthorizationException
   *           if the user has no {@link Permissions#CREATE} permissions (in case of persisting a
   *           transient object) or no {@link Permissions#UPDATE} permissions (in case of updating a
   *           persistent object) on {@link Resources#AUTHORIZATION}
   */
   ...
   * @throws ProcessEngineException
   *           if no such authorization exists or if an internal error occurs.
   * @throws AuthorizationException
   ...
   * 
   * <p>
   * This method checks for the resource type, see {@link Authorization#ANY}
   * </p>
   * 
   * @param userId
   *          the id of the user for which the check is performed.
   * @param groupIds
   *          a list of group ids the user is member of
   * @param permission
