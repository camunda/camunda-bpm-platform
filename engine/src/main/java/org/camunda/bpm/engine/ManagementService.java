...
public interface ManagementService {
   ...
   * <ol>
   * <li>The process engine will execute atomic operations within the context of that
   * ProcessApplication</li>
  ...
  /**
   * Deactivate a deployment for a given ProcessApplication. This removes the association between
   * the process engine and the process application and optionally removes the associated process
  ...
  /**
   * @return the name of the process application that is currently registered for the given
   *         deployment or 'null' if no process application is currently registered.
   *
   * @throws AuthorizationException
   *           If the user is not a member of the group {@link Groups#CAMUNDA_ADMIN}.
   */
  ...
  /**
   * Returns a new {@link JobDefinitionQuery} implementation, that can be used to dynamically query
   * the job definitions.
   */
   ...
   *
   * <p>
   * <strong>Note:</strong> All {@link Job}s of the provided job definition will be
   * <strong>not</strong> activated.
   * </p>
   *
   * <p>
   ...
   *
   * @param activateJobs
   *          If true, all the {@link Job}s of the provided job definition will be activated too.
   *
   * @throws ProcessEngineException
   *           If the job definition id is equal null.
   * @throws AuthorizationException
   ...
   *           </ul>
   *
   *           If <code>activateJobs</code> is <code>true</code>, the user must further possess one
   ...
   *          The date on which the job definition will be activated. If null, the job definition is
   *          activated immediately. Note: The {@link JobExecutor} needs to be active to use this!
   *
   * @throws ProcessEngineException
   *           If the job definition id is equal null.
   * @throws AuthorizationException
   ...
   *
   * @param suspendJobs
   *          If true, all the {@link Job}s of the provided job definition will be suspended too.
   *
   * @throws ProcessEngineException
   *           If the job definition id is equal null.
   * @throws AuthorizationException
   ...
   *           </ul>
   *
   *           If <code>suspendJobs</code> is <code>true</code>, the user must further possess one
   *           of the following permissions:
   *           <ul>
   *           <li>{@link Permissions#UPDATE_INSTANCE} on {@link Resources#PROCESS_DEFINITION}</li>
   *           <li>{@link Permissions#UPDATE} on any {@link Resources#PROCESS_INSTANCE}</li>
   ...
   *          The date on which the job definition will be suspended. If null, the job definition is
   *          suspended immediately. Note: The {@link JobExecutor} needs to be active to use this!
   *
   * @throws ProcessEngineException
   *           If the job definition id is equal null.
   * @throws AuthorizationException
  ...
  /**
   * Activate or suspend jobs using a fluent builder. Specify the jobs by calling one of the
   * <i>by</i> methods, like <i>byJobId</i>. To update the suspension state call
   * {@link UpdateJobSuspensionStateBuilder#activate()} or
  ...
  /**
   * Activate or suspend job definitions using a fluent builder. Specify the job definitions by
   * calling one of the <i>by</i> methods, like <i>byJobDefinitionId</i>. To update the suspension
   * state call {@link UpdateJobDefinitionSuspensionStateBuilder#activate()} or
   ...
   *
   * Whenever the JobExecutor fails to execute a job, this value is decremented. When it hits zero,
   * the job is supposed to be dead and not retried again. In that case, this method can be used to
   ...
   *           permission on {@link Resources#PROCESS_DEFINITION} and no
   *           {@link ProcessInstancePermissions#RETRY_JOB} permission on
   *           {@link Resources#PROCESS_INSTANCE} and no
   *           {@link ProcessDefinitionPermissions#RETRY_JOB} permission on
   *           {@link Resources#PROCESS_DEFINITION}.
   */
   ...
   *
   * @param jobIds
   *          ids of the jobs to modify, cannot be null.
   * @param retries
   *          number of retries.
   *
   ...
   *           {@link Resources#PROCESS_INSTANCE} and no {@link Permissions#UPDATE_INSTANCE}
   *           permission on {@link Resources#PROCESS_DEFINITION} and no
   *           {@link Permissions#RETRY_JOB} permission on {@link Resources#PROCESS_INSTANCE} and no
   *           {@link Permissions#RETRY_JOB} permission on {@link Resources#PROCESS_DEFINITION}.
   */
   ...
   *           {@link BatchPermissions#CREATE_BATCH_SET_JOB_RETRIES} permission on
   ...
   *
   * @param jobQuery
   *          query that identifies which jobs should be modified, cannot be null.
   * @param retries
   *          number of retries.
   *
   * @throws BadUserRequestException
   ...
   * increase the number of retries.
   *
   * Either jobIds or jobQuery has to be provided. If both are provided resulting list of affected
   * jobs will contain jobs matching query as well as jobs defined in the list.
   *
   * @param jobIds
   *          ids of the jobs to modify.
   * @param jobQuery
   *          query that identifies which jobs should be modified.
   * @param retries
   *          number of retries.
   *
   * @throws BadUserRequestException
   *           if neither jobIds, nor jobQuery is provided or result in empty list
   * @throws AuthorizationException
   *           If the user has no {@link Permissions#CREATE} or
   *           {@link BatchPermissions#CREATE_BATCH_SET_JOB_RETRIES} permission on
   *           {@link Resources#BATCH}.
   ...
   *          ids of the process instances that for which jobs retries will be set
   * @param query
   *          query that identifies process instances with jobs that have to be modified
   * @param retries
   *          number of retries.
   *
   * @throws AuthorizationException
   ...
   *           {@link Resources#BATCH}.
   */
  Batch setJobRetriesAsync(List<String> processInstanceIds, ProcessInstanceQuery query,
      int retries);

  /**
   * <p>
   * Set the number of retries of all <strong>failed</strong> {@link Job jobs} of the provided job
   * definition id.
   * </p>
   *
   * <p>
   * Whenever the JobExecutor fails to execute a job, this value is decremented. When it hits zero,
   * the job is supposed to be <strong>failed</strong> and not retried again. In that case, this
   * method can be used to increase the number of retries.
   * </p>
   *
   * <p>
   * {@link Incident Incidents} of the involved failed {@link Job jobs} will not be resolved using
   * this method! When the execution of a job was successful the corresponding incident will be
   * resolved.
   * </p>
   *
   * @param jobDefinitionId
   *          id of the job definition, cannot be null.
   * @param retries
   *          number of retries.
   *
   * @throws AuthorizationException
  ...
  /**
   * Sets a new due date for the provided id. When newDuedate is null, the job is executed with the
   * next job executor run.
   *
   * @param jobId
   ...
   * 
   * @param jobId
   *          id of job to recalculate, must neither be null nor empty.
   * @param creationDateBased
   *          indicates whether the recalculation should be based on the creation date of the job or
   *          the current date
   * 
   * @throws AuthorizationException
   *           If the user has no {@link Permissions#UPDATE} permission on
   ...
   *
   * @param jobId
   *          the id of the job to modify, must not be null
   * @param priority
  ...
  /**
   * <p>
   * Sets an explicit priority for jobs of the given job definition. Jobs created after invoking
   * this method receive the given priority. This setting overrides any setting specified in the
   * BPMN 2.0 XML.
   * </p>
   *
   * <p>
   * The overriding priority can be cleared by using the method
   * {@link #clearOverridingJobPriorityForJobDefinition(String)}.
   * </p>
   *
   * @param jobDefinitionId
   *          the id of the job definition to set the priority for
   * @param priority
   *          the priority to set;
   *
   * @throws AuthorizationException
  ...
  /**
   * <p>
   * Sets an explicit default priority for jobs of the given job definition. Jobs created after
   * invoking this method receive the given priority. This setting overrides any setting specified
   * in the BPMN 2.0 XML.
   * </p>
   *
   * <p>
   * If <code>cascade</code> is true, priorities of already existing jobs are updated accordingly.
   * </p>
   *
   * <p>
   * The overriding priority can be cleared by using the method
   ...
   *           </ul>
   *
   *           If cascade is <code>true</code>, the user must further possess one of the following
   *           permissions:
   *           <ul>
   *           <li>{@link Permissions#UPDATE} on {@link Resources#PROCESS_INSTANCE}</li>
   *           <li>{@link Permissions#UPDATE_INSTANCE} on {@link Resources#PROCESS_DEFINITION}</li>
  ...
  /**
   * <p>
   * Clears the job definition's overriding job priority if set. After invoking this method, new
   * jobs of the given definition receive the priority as specified in the BPMN 2.0 XML or the
  ...
  /**
   * Query for the number of activity instances aggregated by activities of a single process
   * definition.
   *
   * @throws AuthorizationException
   *           If the user has no {@link Permissions#READ} permission on
  ...
  /**
   * Get the deployments that are registered the engine's job executor. This set is only relevant,
  ...
  /**
   * Register a deployment for the engine's job executor. This is required, if the engine
   * configuration property <code>jobExecutorDeploymentAware</code> is set. If set to false, the job
   * executor will execute any job.
   *
   * @throws AuthorizationException
   *           If the user is not a member of the group {@link Groups#CAMUNDA_ADMIN}.
   */
  ...
  /**
   * Unregister a deployment for the engine's job executor. If the engine configuration property
   * <code>jobExecutorDeploymentAware</code> is set, jobs for the given deployment will no longer
   * get acquired.
   *
   * @throws AuthorizationException
   *           If the user is not a member of the group {@link Groups#CAMUNDA_ADMIN}.
  ...
  /**
   * Deletes all metrics events which are older than the specified timestamp. If the timestamp is
   * null, all metrics will be deleted
   *
   * @param timestamp
   *          or null
   * @since 7.3
  ...
  /**
   * Deletes all metrics events which are older than the specified timestamp and reported by the
   * given reporter. If a parameter is null, all metric events are matched in that regard.
   *
   * @param timestamp
   *          or null
   * @param reporter
   ...
   *
   * <p>
   * <strong>Note:</strong> All {@link JobDefinition}s and {@link Job}s related to the provided
   * batch will be suspended.
   * </p>
   *
   * @throws BadUserRequestException
   ...
   *
   * If cascade is set to true the historic batch instances and the historic jobs logs are also
   * removed.
   *
   * @throws AuthorizationException
