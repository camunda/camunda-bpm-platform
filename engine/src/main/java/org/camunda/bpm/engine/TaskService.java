...
public interface TaskService {
   ...
   *
   * @throws AuthorizationException
   *           If the task is already present and the user has no {@link Permissions#UPDATE}
   *           permission on {@link Resources#TASK} or no {@link Permissions#UPDATE_TASK} permission
   *           on {@link Resources#PROCESS_DEFINITION}. Or if the task is not present and the user
   *           has no {@link Permissions#CREATE} permission on {@link Resources#TASK}.
   */
   ...
   *
   * @param taskId
   *          The id of the task that will be deleted, cannot be null. If no task exists with the
   *          given taskId, the operation is ignored.
   *
   * @throws ProcessEngineException
   *           when an error occurs while deleting the task or in case the task is part of a running
   ...
   *
   * @param taskIds
   *          The id's of the tasks that will be deleted, cannot be null. All id's in the list that
   *          don't have an existing task will be ignored.
   *
   * @throws ProcessEngineException
   *           when an error occurs while deleting the tasks or in case one of the tasks is part of
   *           a running process or case instance.
   * @throws AuthorizationException
   *           If the user has no {@link Permissions#DELETE} permission on {@link Resources#TASK}.
   */
  ...
  /**
   * Claim responsibility for a task: the given user is made {@link Task#getAssignee() assignee} for
   * the task. The difference with {@link #setAssignee(String, String)} is that here a check is done
   * if the task already has a user assigned to it. No check is done whether the user is known by
   * the identity component.
   *
   * @param taskId
   *          task to claim, cannot be null.
   * @param userId
   *          user that claims the task. When userId is null the task is unclaimed, assigned to no
   *          one.
   *
   * @throws ProcessEngineException
   *           when the task doesn't exist or when the task is already claimed by another user.
   * @throws AuthorizationException
   *           If the user has no {@link Permissions#UPDATE} permission on {@link Resources#TASK} or
   *           no {@link Permissions#UPDATE_TASK} permission on {@link Resources#PROCESS_DEFINITION}
   ...
   *
   * This method is typically called by a task list user interface after a task form has been
   * submitted by the {@link Task#getAssignee() assignee}.
   *
   * @param taskId
   ...
   *
   * This means that the {@link Task#getAssignee() assignee} is set and the
   * {@link Task#getDelegationState() delegation state} is set to {@link DelegationState#PENDING}.
   * If no owner is set on the task, the owner is set to the current {@link Task#getAssignee()
   * assignee} of the task. The new assignee must use {@link TaskService#resolveTask(String)} to
   * report back to the owner. Only the owner can {@link TaskService#complete(String) complete} the
   * task.
   *
   * @param taskId
   ...
   * {@link TaskService#delegateTask(String, String) delegated} to her and that it can be sent back
   * to the {@link Task#getOwner() owner}. Can only be called when this task is
   * {@link DelegationState#PENDING} delegation. After this method returns, the
   * {@link Task#getDelegationState() delegation state} is set to {@link DelegationState#RESOLVED}
   * and the task can be {@link TaskService#complete(String) completed}.
   *
   * @param taskId
   ...
   * Marks that the {@link Task#getAssignee() assignee} is done with the task
   * {@link TaskService#delegateTask(String, String) delegated} to her and that it can be sent back
   * to the {@link Task#getOwner() owner} with the provided variables. Can only be called when this
   * task is {@link DelegationState#PENDING} delegation. After this method returns, the
   * {@link Task#getDelegationState() delegation state} is set to {@link DelegationState#RESOLVED}
   * and the task can be {@link TaskService#complete(String) completed}.
   *
   * @param taskId
   ...
   *
   * This method is typically called by a task list user interface after a task form has been
   * submitted by the {@link Task#getAssignee() assignee} and the required task parameters have been
   * provided.
   *
   * @param taskId
   *          the id of the task to complete, cannot be null.
  ...
  /**
   * Retrieves the {@link IdentityLink}s associated with the given task. Such an
   * {@link IdentityLink} informs how a certain identity (eg. group or user) is associated with a
   * certain task (eg. as candidate, assignee, etc.)
   *
   * @throws ProcessEngineException
   ...
   *           <ul>
   *           <li>{@link ProcessDefinitionPermissions#UPDATE_TASK_VARIABLE} permission on
   *           {@link Resources#PROCESS_DEFINITION}</li>
   *           <li>{@link Permissions#UPDATE_TASK} permission on
   *           {@link Resources#PROCESS_DEFINITION}</li>
   *           </ul>
   */
   ...
   *           <li>if the user has no {@link Permissions#READ} permission on {@link Resources#TASK}
   *           or</li>
   *           <li>if {@link ProcessEngineConfiguration#enforceSpecificVariablePermission this}
   *           configuration is enabled and the user has no {@link TaskPermissions#READ_VARIABLE}
   *           permission on {@link Resources#TASK}</li>
   *           </p>
   *           <p>
   *           In case the task is part of a running process instance:</li>
   *           <li>if the user has no {@link Permissions#READ} permission on {@link Resources#TASK}
   *           and no {@link Permissions#READ_TASK} permission on
   *           {@link Resources#PROCESS_DEFINITION}</li>
   *           <li>in case {@link ProcessEngineConfiguration#enforceSpecificVariablePermission this}
   *           configuration is enabled and the user has no {@link TaskPermissions#READ_VARIABLE}
   *           permission on {@link Resources#TASK} and no
   *           {@link ProcessDefinitionPermissions#READ_TASK_VARIABLE} permission on
   *           {@link Resources#PROCESS_DEFINITION}</li>
   *           </p>
   */
   ...
   * @since 7.2
   */
  VariableMap getVariablesTyped(String taskId, Collection<String> variableNames,
      boolean deserializeValues);
   ...
   * @since 7.2
   */
  VariableMap getVariablesLocalTyped(String taskId, Collection<String> variableNames,
      boolean deserializeValues);
   ...
   *
   * <p>
   * As of Camunda BPM 7.4 task events are only written in context of a logged in user. This
   * behavior can be toggled in the process engine configuration using the property
   * <code>legacyUserOperationLog</code> (default false). To restore the engine's previous behavior,
   * set the flag to <code>true</code>.
   * </p>
   *
   * @deprecated This method has been deprecated as of camunda BPM 7.1. It has been replaced with
   *             the operation log. See {@link UserOperationLogEntry} and
   ...
   * content please use method in runtime service to operate on process instance.
   *
   * Either taskId or processInstanceId has to be provided
   *
   * @param taskId
   *          - task that should have an attachment
   * @param processInstanceId
   *          - id of a process to use if task id is null
   * @param attachmentType
   *          - name of the attachment, can be null
   * @param attachmentName
   *          - name of the attachment, can be null
   * @param attachmentDescription
   *          - full text description, can be null
   * @param content
  ...
  /**
   * Add a new attachment to a task and/or a process instance and use an url as the content please
   * use method in runtime service to operate on process instance
   *
   * Either taskId or processInstanceId has to be provided
   *
   * @param taskId
