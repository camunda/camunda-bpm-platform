...
/**
 * <p>
 * An {@link Authorization} assigns a set of {@link Permission Permissions} to an identity to
 * interact with a given {@link Resource}.
 * </p>
 * <p>
 * EXAMPLES:
 * <ul>
 ...
 * <li>Nobody is allowed to edit process variables in the cockpit application, except the distinct
 * user 'admin'.</li>
 * </ul>
 *
 * <h2>Identities</h2>
 * <p>
 * camunda BPM distinguished two types of identities: <em>users</em> and <em>groups</em>.
 * Authorizations can either range over all users (userId = {@link #ANY}), an individual
 * {@link User} or a {@link Group} of users.
 * </p>
 *
 * <h2>Permissions</h2>
 * <p>
 * A {@link Permission} defines the way an identity is allowed to interact with a certain resource.
 * Examples of permissions are {@link Permissions#CREATE CREATE}, {@link Permissions#READ READ},
 * {@link Permissions#UPDATE UPDATE}, {@link Permissions#DELETE DELETE}, ... See {@link Permissions}
 * for a set of built-in permissions.
 * </p>
 *
 * <p>
 ...
 * {@link Resources#GROUP GROUPS}, {@link Resources#USER USERS}, process-definitions,
 * process-instances, tasks ... See {@link Resources} for a set of built-in resource. The camunda
 * BPM framework supports custom resources.
 * </p>
 *
 ...
 * <ul>
 * <li><strong>Global Authorizations</strong> ({@link #AUTH_TYPE_GLOBAL}) range over all users and
 * groups (userId = {@link #ANY}) and are usually used for fixing the "base" permission for a
 * resource.</li>
 * <li><strong>Grant Authorizations</strong> ({@link #AUTH_TYPE_GRANT}) range over users and groups
 * and grant a set of permissions. Grant authorizations are commonly used for adding permissions to
 * a user or group that the global authorization revokes.</li>
 * <li><strong>Revoke Authorizations</strong> ({@link #AUTH_TYPE_REVOKE}) range over users and
 * groups and revoke a set of permissions. Revoke authorizations are commonly used for revoking
 * permissions to a user or group the the global authorization grants.</li>
 * </ul>
 * </p>
 *
 * <h2>Authorization Precedence</h2>
 * <p>
 * Authorizations may range over all users, an individual user or a group of users or . They may
 * apply to an individual resource instance or all instances of the same type (resourceId =
 * {@link #ANY}). The precedence is as follows:
 * <ol>
...
public interface Authorization {
  ...
  /**
   * A Global Authorization ranges over all users and groups (userId = {@link #ANY}) and are usually
   * used for fixing the "base" permission for a resource.
   */
  public static final int AUTH_TYPE_GLOBAL = 0;

  /**
   * A Grant Authorization ranges over a users or a group and grants a set of permissions. Grant
   * authorizations are commonly used for adding permissions to a user or group that the global
   * authorization revokes.
   */
  public static final int AUTH_TYPE_GRANT = 1;

  /**
   * A Revoke Authorization ranges over a user or a group and revokes a set of permissions. Revoke
   * authorizations are commonly used for revoking permissions to a user or group the the global
   ...
   *           if this {@link Authorization} is of type {@link #AUTH_TYPE_REVOKE}
   */
   ...
   *          the permission to check for
   * @throws IllegalStateException
   *           if this {@link Authorization} is of type {@link #AUTH_TYPE_GRANT}
   */
  ...
  /**
   * Allows checking whether this authorization grants / revokes a set of permissions. Usually the
   * set of built-in permissions is used: {@link Permissions#values()}
   *
   * The return value of this method depends on the type of the authorization:
   * <ul>
   * <li>For {@link #AUTH_TYPE_GLOBAL}: all permissions in the parameter list granted by this
   * authorization are returned.</li>
   * <li>For {@link #AUTH_TYPE_GRANT}: all permissions in the parameter list granted by this
   * authorization are returned.</li>
   * <li>For {@link #AUTH_TYPE_REVOKE}: all permissions in the parameter list revoked by this
   * authorization are returned.</li>
   * </ul>
   *
   * @param an
   *          array of permissions to check for.
   * @return Returns the set of {@link Permission Permissions} provided by this
   *         {@link Authorization}.
   */
