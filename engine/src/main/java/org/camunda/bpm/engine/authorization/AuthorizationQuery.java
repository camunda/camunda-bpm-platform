...
public interface AuthorizationQuery extends Query<AuthorizationQuery, Authorization> {
  ...
  /**
   * only selects authorizations which grant the permissions represented by the parameter. If this
   * method is called multiple times, all passed-in permissions will be checked with AND semantics.
   * Example:
   * 
   * <pre>
   * authorizationQuery.userId("user1").resourceType("processDefinition").resourceId("2313")
   *     .hasPermission(Permissions.READ).hasPermission(Permissions.UPDATE)
   *     .hasPermission(Permissions.DELETE).list();
   * </pre>
   * 
   * Selects all Authorization objects which provide READ,UPDATE,DELETE Permissions for the given
   * user.
   * 
   */
