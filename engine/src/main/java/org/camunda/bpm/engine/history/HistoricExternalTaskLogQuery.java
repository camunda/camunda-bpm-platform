...
public interface HistoricExternalTaskLogQuery
  ...
  /**
   * Only select historic external task log entries which are associated with one of the given
   * activity ids.
   **/
  ...
  /**
   * Only select log entries where the external task had a priority higher than or equal to the
   * given priority.
   */
  HistoricExternalTaskLogQuery priorityHigherThanOrEquals(long priority);

  /**
   * Only select log entries where the external task had a priority lower than or equal to the given
   * priority.
   */
   ...
   * ordering of external task log entries without tenant id is database-specific.
   */
