...
public interface HistoricProcessInstanceQuery
  ...
  /**
   * Only select historic process instances with the given process instance. {@link ProcessInstance)
  ...
  /**
   * Only select historic process instances that are defined by process definition which name is
   ...
   *          with (string%), ends with (%string) or contains (%string%).
   */
  HistoricProcessInstanceQuery processInstanceBusinessKeyLike(
  ...
  /**
   * Only select historic process instances with incident status either 'open' or 'resolved'. To get
   * all process instances with incidents, use {@link HistoricProcessInstanceQuery#withIncidents()}.
   *
   * @param status
   *          indicates the incident status, which is either 'open' or 'resolved'
   ...
   * Only select process instances which have a variable value greater than the passed value. The
   * type only applies to already ended process instances, otherwise use a
   * {@link ProcessInstanceQuery} instead! of variable is determined based on the value, using types
   * configured in {@link ProcessEngineConfiguration#getVariableSerializers()}. Byte-arrays and
   ...
   * the ended. Only applies to already ended process instances, otherwise use a
   ...
   * ended. Only applies to already ended process instances, otherwise use a
   * {@link ProcessInstanceQuery} instead! This can be used on string variables only.
   * 
   * @param name
   *          cannot be null.
   * @param value
   *          cannot be null. The string can include the wildcard character '%' to express
  ...
  /**
   * Order by the process definition key (needs to be followed by {@link #asc()} or
   * {@link #desc()}).
   */
  HistoricProcessInstanceQuery orderByProcessDefinitionKey();

  /**
   * Order by the process definition name (needs to be followed by {@link #asc()} or
   * {@link #desc()}).
   */
  HistoricProcessInstanceQuery orderByProcessDefinitionName();

  /**
   * Order by the process definition version (needs to be followed by {@link #asc()} or
   * {@link #desc()}).
   */
  ...
  /**
   * Only select historic process instances having a sub process instance with the given process
   * instance id.
   *
  ...
  /**
   * Only select historic process instances having a sub case instance with the given case instance
   * id.
   *
   * Note that there will always be maximum only <b>one</b> such process instance that can be the
   * result of this query.
   ...
   * ordering of historic process instances without tenant id is database-specific.
   */
  HistoricProcessInstanceQuery orderByTenantId();

  /**
   * Only select historic process instances that were started as of the provided date. (Date will be
   * adjusted to reflect midnight)
   * 
  ...
  /**
   * Only select historic process instances that were finished as of the provided date. (Date will
   * be adjusted to reflect one second before midnight)
   * 
   * @deprecated use {@link #startedAfter(Date)} and {@link #startedBefore(Date)} instead
   */
  @Deprecated
   ...
   * </p>
   *
   * @return an object of the type {@link HistoricProcessInstanceQuery} on which an arbitrary amount
   *         of filter criteria could be applied. The several filter criteria will be linked
   *         together by an OR expression.
   *
   ...
   *
   * @return an object of the type {@link HistoricProcessInstanceQuery} on which an arbitrary amount
   *         of filter criteria could be applied. The filter criteria will be linked together by an
   *         AND expression.
   *
   * @throws ProcessEngineException
