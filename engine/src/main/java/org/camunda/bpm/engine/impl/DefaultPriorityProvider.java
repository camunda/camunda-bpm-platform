...
/**
 * Represents a default priority provider, which contains some functionality to evaluate the
 * priority. Can be used as base class for other priority providers. *
 * 
 * @author Christopher Zell <christopher.zell@camunda.com>
 * @param <T>
 *          the type of the param to determine the priority
 */
public abstract class DefaultPriorityProvider<T> implements PriorityProvider<T> {
  ...
  /**
   * Evaluates a given value provider with the given execution entity to determine the correct
   * value. The error message heading is used for the error message if the validation fails because
   * the value is no valid priority.
   * 
   * @param valueProvider
   *          the provider which contains the value
   * @param execution
   *          the execution entity
   * @param errorMessageHeading
   *          the heading which is used for the error message
    ...
    if (!(value instanceof Number)) {
      ...
      if (isValidLongValue(numberValue)) {
        ...
        throw new ProcessEngineException(
            errorMessageHeading + ": Priority value must be either Short, Integer, or Long");
      }
    }
  }
  ...
  /**
   * Returns the priority defined in the specific entity. Like a job definition priority or an
   * activity priority. The result can also be null in that case the process priority will be used.
   *
   * @param execution
   ...
   *          the job definition id if related to a job
  ...
  /**
   * Returns the priority defined in the process definition. Can also be null in that case the
   * fallback is the default priority.
   * 
   * @param execution
   *          the current execution
   * @param param
  ...
  /**
   * Returns the priority which is defined in the given process definition. The priority value is
   * identified with the given propertyKey. Returns null if the process definition is null or no
   * priority was defined.
   * 
   * @param processDefinition
   *          the process definition that should contains the priority
   * @param propertyKey
   *          the key which identifies the property
   * @param execution
   *          the current execution
   * @param errorMsgHead
   *          the error message header which is used if the evaluation fails
  ...
  protected Long getProcessDefinedPriority(ProcessDefinitionImpl processDefinition,
    ...
    if (processDefinition != null) {
      ParameterValueProvider priorityProvider = (ParameterValueProvider) processDefinition
   ...
   * 
   * @param execution
   *          the current execution entity
   * @param value
   *          the current value
   * @param e
   *          the exception which was catched
   */
  protected abstract void logNotDeterminingPriority(ExecutionEntity execution, Object value,
      ProcessEngineException e);

  protected boolean isSymptomOfContextSwitchFailure(Throwable t, ExecutionEntity contextExecution) {
    // a context switch failure can occur, if the current engine has no PA registration for the
    // deployment
