 ...
 * {@link ProcessDefinitionEntity}s. Overwrite this class if you want to gain some control over this
 * mechanism, e.g. setting different version numbers, or you want to use your own
 * {@link BpmnParser}.
 *
 * @author Tom Baeyens
 * @author Joram Barrez
...
public class BpmnDeployer extends AbstractDefinitionDeployer<ProcessDefinitionEntity> {
  ...
  protected static final PropertyMapKey<String, List<JobDeclaration<?, ?>>> JOB_DECLARATIONS_PROPERTY = new PropertyMapKey<String, List<JobDeclaration<?, ?>>>(
  ...
  protected List<ProcessDefinitionEntity> transformDefinitions(DeploymentEntity deployment,
    ...
    BpmnParse bpmnParse = bpmnParser.createParse().sourceInputStream(inputStream)
        .deployment(deployment).name(resource.getName());
  ...
  protected ProcessDefinitionEntity findDefinitionByDeploymentAndKey(String deploymentId,
    ...
    return getProcessDefinitionManager().findProcessDefinitionByDeploymentAndKey(deploymentId,
        definitionKey);
  }
  ...
  protected ProcessDefinitionEntity findLatestDefinitionByKeyAndTenantId(String definitionKey,
      String tenantId) {
    return getProcessDefinitionManager().findLatestProcessDefinitionByKeyAndTenantId(definitionKey,
        tenantId);
  }
  ...
  protected void definitionAddedToDeploymentCache(DeploymentEntity deployment,
      ProcessDefinitionEntity definition, Properties properties) {
    List<JobDeclaration<?, ?>> declarations = properties.get(JOB_DECLARATIONS_PROPERTY)
        .get(definition.getKey());

    updateJobDeclarations(declarations, definition, deployment.isNew());

    ProcessDefinitionEntity latestDefinition = findLatestDefinitionByKeyAndTenantId(
  ...
  protected void persistedDefinitionLoaded(DeploymentEntity deployment,
      ProcessDefinitionEntity definition, ProcessDefinitionEntity persistedDefinition) {
  ...
  protected void handlePersistedDefinition(ProcessDefinitionEntity definition,
      ProcessDefinitionEntity persistedDefinition, DeploymentEntity deployment,
      Properties properties) {
    // check if persisted definition is not null, since the process definition can be deleted by the
    // user
    // in such cases we don't want to handle them
    // we can't do this in the parent method, since other siblings want to handle them like {@link
  ...
  protected void updateJobDeclarations(List<JobDeclaration<?, ?>> jobDeclarations,
      ProcessDefinitionEntity processDefinition, boolean isNewDeployment) {
    ...
    if (isNewDeployment) {
      ...
      List<JobDefinitionEntity> existingDefinitions = jobDefinitionManager
  ...
  /**
   * adjust all event subscriptions responsible to start process instances (timer start event,
   * message start event). The default behavior is to remove the old subscriptions and add new ones
   * for the new deployed process definitions.
   */
  ...
  protected void removeObsoleteTimers(ProcessDefinitionEntity processDefinition) {
    List<JobEntity> jobsToDelete = getJobManager().findJobsByConfiguration(
  ...
  protected void removeObsoleteEventSubscriptions(ProcessDefinitionEntity processDefinition,
    ...
    if (latestProcessDefinition != null) {
      ...
      List<EventSubscriptionEntity> messageEventSubscriptions = eventSubscriptionManager
          .findEventSubscriptionsByConfiguration(EventType.MESSAGE.name(),
      ...
      List<EventSubscriptionEntity> signalEventSubscriptions = eventSubscriptionManager
          .findEventSubscriptionsByConfiguration(EventType.SIGNAL.name(),
              latestProcessDefinition.getId());
      ...
      List<EventSubscriptionEntity> conditionalEventSubscriptions = eventSubscriptionManager
          .findEventSubscriptionsByConfiguration(EventType.CONDITONAL.name(),
              latestProcessDefinition.getId());
  ...
  public void addEventSubscriptions(ProcessDefinitionEntity processDefinition) {
    Map<String, EventSubscriptionDeclaration> eventDefinitions = processDefinition.getProperties()
  ...
  protected void addMessageStartEventSubscription(
    ...
    if (isSameMessageEventSubscriptionAlreadyPresent(messageEventDefinition, tenantId)) {
      throw LOG.messageEventSubscriptionWithSameNameExists(processDefinition.getResourceName(),
          messageEventDefinition.getUnresolvedEventName());
    }

    EventSubscriptionEntity newSubscription = messageEventDefinition
        .createSubscriptionForStartEvent(processDefinition);
    newSubscription.insert();

  }

  protected boolean isSameMessageEventSubscriptionAlreadyPresent(
      EventSubscriptionDeclaration eventSubscription, String tenantId) {
    // look for subscriptions for the same name in db:
    List<EventSubscriptionEntity> subscriptionsForSameMessageName = getEventSubscriptionManager()
        .findEventSubscriptionsByNameAndTenantId(EventType.MESSAGE.name(),
    ...
    // remove subscriptions for different type of event (i.e. remove intermediate message event
    // subscriptions)
    subscriptionsForSameMessageName = filterSubscriptionsOfDifferentType(eventSubscription,
  ...
  /**
   * It is possible to deploy a process containing a start and intermediate message event that wait
   * for the same message or to have two processes, one with a message start event and the other one
   * with a message intermediate event, that subscribe for the same message. Therefore we have to
   * find out if there are subscriptions for the other type of event and remove those.
   *
   * @param eventSubscription
   * @param subscriptionsForSameMessageName
   */
  protected List<EventSubscriptionEntity> filterSubscriptionsOfDifferentType(
      EventSubscriptionDeclaration eventSubscription,
      List<EventSubscriptionEntity> subscriptionsForSameMessageName) {
    ArrayList<EventSubscriptionEntity> filteredSubscriptions = new ArrayList<EventSubscriptionEntity>(
        subscriptionsForSameMessageName);

    for (EventSubscriptionEntity subscriptionEntity : new ArrayList<EventSubscriptionEntity>(
        subscriptionsForSameMessageName)) {
  ...
  protected boolean isSubscriptionOfDifferentTypeAsDeclaration(
  ...
  protected void addSignalStartEventSubscription(EventSubscriptionDeclaration signalEventDefinition,
      ProcessDefinitionEntity processDefinition) {
    EventSubscriptionEntity newSubscription = signalEventDefinition
        .createSubscriptionForStartEvent(processDefinition);

    newSubscription.insert();
  }

  protected void addConditionalStartEventSubscription(
      EventSubscriptionDeclaration conditionalEventDefinition,
      ProcessDefinitionEntity processDefinition) {
    EventSubscriptionEntity newSubscription = conditionalEventDefinition
        .createSubscriptionForStartEvent(processDefinition);
  ...
  protected void addAuthorizationsFromIterator(Set<Expression> exprSet,
      ProcessDefinitionEntity processDefinition, ExprType exprType) {
    if (exprSet != null) {
