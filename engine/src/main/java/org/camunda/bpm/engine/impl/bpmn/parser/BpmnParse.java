...
public class BpmnParse extends Parse {
  ...
  public static final List<String> VARIABLE_EVENTS = Arrays.asList(VariableListener.CREATE,
   ...
   */
  @Deprecated
  public static final String PROPERTYNAME_ERROR_EVENT_DEFINITIONS = BpmnProperties.ERROR_EVENT_DEFINITIONS
      .getName();
  ...
  protected static final String ATTRIBUTEVALUE_T_FORMAL_EXPRESSION = BpmnParser.BPMN20_NS
      + ":tFormalExpression";

  public static final String PROPERTYNAME_IS_MULTI_INSTANCE = "isMultiInstance";

  public static final Namespace CAMUNDA_BPMN_EXTENSIONS_NS = new Namespace(
      BpmnParser.CAMUNDA_BPMN_EXTENSIONS_NS, BpmnParser.ACTIVITI_BPMN_EXTENSIONS_NS);
  ...
  /**
   * A list of all element IDs. This allows us to parse only what we actually support but still
   * validate the references among elements we do not support.
   */
   ...
   *
   * All the map's elements are defined outside the process definition(s), which means that this map
   * doesn't need to be re-initialized for each new process definition.
   */
      ...
      if (importType.equals("http://schemas.xmlsoap.org/wsdl/")) {
        ...
        try {
          wsdlImporterClass = Class.forName(
              "org.camunda.bpm.engine.impl.webservice.CxfWSDLImporter", true,
  ...
  /**
   * Parses the messages of the given definitions file. Messages are not contained within a process
   * element, but they can be referenced from inner process elements.
   */
  public void parseMessages() {
    for (Element messageElement : rootElement.elements("message")) {
      ...
      MessageDefinition messageDefinition = new MessageDefinition(this.targetNamespace + ":" + id,
          messageExpression);
  ...
  /**
   * Parses the signals of the given definitions file. Signals are not contained within a process
   * element, but they can be referenced from inner process elements.
   */
  ...
  public void parseErrors() {
    for (Element errorElement : rootElement.elements("error")) {
      ...
      if (errorMessage != null) {
        error.setErrorMessageExpression(
            createParameterValueProvider(errorMessage, expressionManager));
      }
  ...
  /**
   * Parses the collaboration definition defined within the 'definitions' root element and get all
   * participants to lookup their process references during DI parsing.
   */
  ...
  public ProcessDefinitionEntity parseProcess(Element processElement) {
    ...
    /*
     * Mapping object model - bpmn xml: processDefinition.id -> generated by processDefinition.key
    ...
    processDefinition.setProperty(PROPERTYNAME_JOB_PRIORITY,
        parsePriority(processElement, PROPERTYNAME_JOB_PRIORITY));
    processDefinition.setProperty(PROPERTYNAME_TASK_PRIORITY,
        parsePriority(processElement, PROPERTYNAME_TASK_PRIORITY));
    processDefinition
        .setVersionTag(processElement.attributeNS(CAMUNDA_BPMN_EXTENSIONS_NS, "versionTag"));

    try {
      String historyTimeToLive = processElement.attributeNS(CAMUNDA_BPMN_EXTENSIONS_NS,
          "historyTimeToLive", Context.getProcessEngineConfiguration().getHistoryTimeToLive());
  ...
  protected void parseIntermediateCatchEvents(Element scopeElement, ScopeImpl parentScope,
    ...
    for (Element intermediateCatchEventElement : intermediateCatchEventElements.values()) {
      ...
      if (parentScope.findActivity(intermediateCatchEventElement.attribute("id")) == null) {
        ...
        ActivityImpl activity = parseIntermediateCatchEvent(intermediateCatchEventElement,
            parentScope, null);
  ...
  protected void parseStartAuthorization(Element scopeElement, ProcessDefinition definition) {
    ...
    if (extentionsElement != null) {
      List<Element> potentialStarterElements = extentionsElement
          .elementsNS(CAMUNDA_BPMN_EXTENSIONS_NS, POTENTIAL_STARTER);
    ...
    String candidateUsersString = scopeElement.attributeNS(CAMUNDA_BPMN_EXTENSIONS_NS,
    ...
    String candidateGroupsString = scopeElement.attributeNS(CAMUNDA_BPMN_EXTENSIONS_NS,
  ...
  protected void parseAssociations(Element scopeElement, ScopeImpl parentScope,
    ...
    for (Element associationElement : scopeElement.elements("association")) {
      ...
      // (like for instance text annotations so do not throw an exception if sourceActivity or
      ...
      if (sourceActivity == null && !elementIds.contains(sourceRef)) {
        ...
        if (sourceActivity != null && ActivityTypes.BOUNDARY_COMPENSATION
            .equals(sourceActivity.getProperty(BpmnProperties.TYPE.getName()))) {
  ...
  protected ActivityImpl parseCompensationHandlerForCompensationBoundaryEvent(ScopeImpl parentScope,
      ActivityImpl sourceActivity, String targetRef, Map<String, Element> compensationHandlers) {
    ...
    compensationHandlerActivity.getProperties().set(BpmnProperties.COMPENSATION_BOUNDARY_EVENT,
        sourceActivity);
  ...
  protected void parseAssociationOfCompensationBoundaryEvent(Element associationElement,
    ...
    if (!targetActivity.isCompensationHandler()) {
      ...
      if (compensationHandler != null && compensationHandler.isSubProcessScope()) {
        ...
        compensatedActivity.setProperty(PROPERTYNAME_COMPENSATION_HANDLER_ID,
  ...
  /**
   * Parses the start events of a certain level in the process (process, subprocess or another
   * scope).
   *
   * @param parentElement
    ...
    if (startEventElements.size() > 0) {
      for (Element startEventElement : startEventElements) {
        ...
        if (scope instanceof ProcessDefinitionEntity) {
          parseProcessDefinitionStartEvent(startEventActivity, startEventElement, parentElement,
              scope);
          startEventActivities.add(startEventActivity);
        } else {
          parseScopeStartEvent(startEventActivity, startEventElement, parentElement,
              (ActivityImpl) scope);
        }
  ...
  protected void selectInitial(List<ActivityImpl> startEventActivities,
    ...
    for (ActivityImpl activityImpl : startEventActivities) {
      if (exclusiveStartEventTypes
          .contains(activityImpl.getProperty(BpmnProperties.TYPE.getName()))) {
        if (initial == null) {
  ...
  protected void parseProcessDefinitionStartEvent(ActivityImpl startEventActivity,
    ...
    String initiatorVariableName = startEventElement.attributeNS(CAMUNDA_BPMN_EXTENSIONS_NS,
        "initiator");
    ...
    if (timerEventDefinition != null) {
      ...
      startEventActivity.getProperties().set(BpmnProperties.TYPE,
          ActivityTypes.START_EVENT_MESSAGE);

      EventSubscriptionDeclaration messageStartEventSubscriptionDeclaration = parseMessageEventDefinition(
      ...
      ensureNoExpressionInMessageStartEvent(messageEventDefinition,
      ...
      ConditionalEventDefinition conditionalEventDefinition = parseConditionalEventDefinition(
          conditionEventDefinition, startEventActivity);
      conditionalEventDefinition.setStartEvent(true);
      conditionalEventDefinition.setActivityId(startEventActivity.getId());
      startEventActivity.getProperties().set(BpmnProperties.CONDITIONAL_EVENT_DEFINITION,
          conditionalEventDefinition);

      addEventSubscriptionDeclaration(conditionalEventDefinition, processDefinition,
          startEventElement);
    }
  }
      ...
      for (Element startEventElement : startEventElements) {
        ...
        if (startEventElement.attribute("id").equals(processDefinition.getInitial().getId())) {
          ...
          String startFormHandlerClassName = startEventElement
              .attributeNS(CAMUNDA_BPMN_EXTENSIONS_NS, "formHandlerClass");
          ...
          startFormHandler.parseConfiguration(startEventElement, deployment, processDefinition,
              this);

          processDefinition
              .setStartFormHandler(new DelegateStartFormHandler(startFormHandler, deployment));
        }
  ...
  protected void parseScopeStartEvent(ActivityImpl startEventActivity, Element startEventElement,
    ...
    Element escalationEventDefinitionElement = startEventElement
        .element(ESCALATION_EVENT_DEFINITION);
    Element conditionalEventDefinitionElement = startEventElement
        .element(CONDITIONAL_EVENT_DEFINITION);
    ...
    if (scopeActivity.isTriggeredByEvent()) {
      ...
      if (errorEventDefinition != null) {
        ...
        EventSubscriptionDeclaration messageStartEventSubscriptionDeclaration = parseMessageEventDefinition(
            messageEventDefinition);
        parseEventDefinitionForSubprocess(messageStartEventSubscriptionDeclaration,
            startEventActivity, messageEventDefinition);

      } else if (signalEventDefinition != null) {
        startEventActivity.getProperties().set(BpmnProperties.TYPE,
            ActivityTypes.START_EVENT_SIGNAL);

        EventSubscriptionDeclaration eventSubscriptionDeclaration = parseSignalEventDefinition(
            signalEventDefinition, false);
        ...
        EscalationEventDefinition escalationEventDefinition = createEscalationEventDefinitionForEscalationHandler(
            escalationEventDefinitionElement, scopeActivity, isInterrupting);
        addEscalationEventDefinition(startEventActivity.getEventScope(), escalationEventDefinition,
            escalationEventDefinitionElement);
      } else if (conditionalEventDefinitionElement != null) {

        final ConditionalEventDefinition conditionalEventDef = parseConditionalStartEventForEventSubprocess(
            conditionalEventDefinitionElement, startEventActivity, isInterrupting);
      ...
      if (signalEventDefinition != null) {
        addError(
            "signalEventDefintion only allowed on start event if subprocess is an event subprocess",
            messageEventDefinition);
      }
  ...
  protected void setErrorCodeVariableOnErrorEventDefinition(Element errorEventDefinition,
    ...
    String errorCodeVar = errorEventDefinition.attributeNS(CAMUNDA_BPMN_EXTENSIONS_NS,
        "errorCodeVariable");
  ...
  protected void setErrorMessageVariableOnErrorEventDefinition(Element errorEventDefinition,
      ErrorEventDefinition definition) {
    String errorMessageVariable = errorEventDefinition.attributeNS(CAMUNDA_BPMN_EXTENSIONS_NS,
  ...
  protected EventSubscriptionDeclaration parseMessageEventDefinition(
    ...
    if (messageDefinition == null) {
      addError("Invalid 'messageRef': no message with id '" + messageRef + "' found.",
          messageEventDefinition);
    }
  ...
  protected void addEventSubscriptionDeclaration(EventSubscriptionDeclaration subscription,
    ...
    if (subscription.getEventType().equals(EventType.MESSAGE.name())
        && (!subscription.hasEventName())) {
      addError("Cannot have a message event subscription with an empty or missing name", element);
    }

    Map<String, EventSubscriptionDeclaration> eventDefinitions = scope.getProperties()
        .get(BpmnProperties.EVENT_SUBSCRIPTION_DECLARATIONS);

    // if this is a message event, validate that it is the only one with the provided name for this
    // scope
    if (hasMultipleMessageEventDefinitionsWithSameName(subscription, eventDefinitions.values())) {
      addError(
          "Cannot have more than one message event subscription with name '"
              + subscription.getUnresolvedEventName() + "' for scope '" + scope.getId() + "'",
          element);
    }

    // if this is a signal event, validate that it is the only one with the provided name for this
    // scope
    if (hasMultipleSignalEventDefinitionsWithSameName(subscription, eventDefinitions.values())) {
      addError(
          "Cannot have more than one signal event subscription with name '"
              + subscription.getUnresolvedEventName() + "' for scope '" + scope.getId() + "'",
          element);
    }
    // if this is a conditional event, validate that it is the only one with the provided condition
    if (subscription.isStartEvent() && hasMultipleConditionalEventDefinitionsWithSameCondition(
        subscription, eventDefinitions.values())) {
      addError("Cannot have more than one conditional event subscription with the same condition '"
          + ((ConditionalEventDefinition) subscription).getConditionAsString() + "'", element);
    }

    scope.getProperties().putMapEntry(BpmnProperties.EVENT_SUBSCRIPTION_DECLARATIONS,
        subscription.getActivityId(), subscription);
  }

  protected boolean hasMultipleMessageEventDefinitionsWithSameName(
      EventSubscriptionDeclaration subscription,
      Collection<EventSubscriptionDeclaration> eventDefinitions) {
    return hasMultipleEventDefinitionsWithSameName(subscription, eventDefinitions,
        EventType.MESSAGE.name());
  }

  protected boolean hasMultipleSignalEventDefinitionsWithSameName(
      EventSubscriptionDeclaration subscription,
      Collection<EventSubscriptionDeclaration> eventDefinitions) {
    return hasMultipleEventDefinitionsWithSameName(subscription, eventDefinitions,
  ...
  protected boolean hasMultipleConditionalEventDefinitionsWithSameCondition(
      EventSubscriptionDeclaration subscription,
      Collection<EventSubscriptionDeclaration> eventDefinitions) {
    if (subscription.getEventType().equals(EventType.CONDITONAL.name())) {
      for (EventSubscriptionDeclaration eventDefinition : eventDefinitions) {
        if (eventDefinition.getEventType().equals(EventType.CONDITONAL.name())
            && eventDefinition.isStartEvent() == subscription.isStartEvent()
            && ((ConditionalEventDefinition) eventDefinition).getConditionAsString()
                .equals(((ConditionalEventDefinition) subscription).getConditionAsString())) {
          return true;
        }
      }
    }
  ...
  protected boolean hasMultipleEventDefinitionsWithSameName(
      EventSubscriptionDeclaration subscription,
      Collection<EventSubscriptionDeclaration> eventDefinitions, String eventType) {
    if (subscription.getEventType().equals(eventType)) {
      for (EventSubscriptionDeclaration eventDefinition : eventDefinitions) {
        if (eventDefinition.getEventType().equals(eventType)
            && eventDefinition.getUnresolvedEventName()
                .equals(subscription.getUnresolvedEventName())
            && eventDefinition.isStartEvent() == subscription.isStartEvent()) {
          return true;
        }
      }
  ...
  protected void addEventSubscriptionJobDeclaration(EventSubscriptionJobDeclaration jobDeclaration,
      ActivityImpl activity, Element element) {
    List<EventSubscriptionJobDeclaration> jobDeclarationsForActivity = (List<EventSubscriptionJobDeclaration>) activity
        .getProperty(PROPERTYNAME_EVENT_SUBSCRIPTION_JOB_DECLARATION);

    if (jobDeclarationsForActivity == null) {
      jobDeclarationsForActivity = new ArrayList<EventSubscriptionJobDeclaration>();
      activity.setProperty(PROPERTYNAME_EVENT_SUBSCRIPTION_JOB_DECLARATION,
          jobDeclarationsForActivity);
    }

    if (activityAlreadyContainsJobDeclarationEventType(jobDeclarationsForActivity,
        jobDeclaration)) {
      addError(
          "Activity contains already job declaration with type " + jobDeclaration.getEventType(),
          element);
    }
   ...
   */
  protected boolean activityAlreadyContainsJobDeclarationEventType(
      List<EventSubscriptionJobDeclaration> jobDeclarationsForActivity,
   ...
   */
  public void parseActivities(List<Element> activityElements, Element parentElement,
  ...
  protected ActivityImpl parseActivity(Element activityElement, Element parentElement,
      ScopeImpl scopeElement) {
    ...
    if (activityElement.getTagName().equals(ActivityTypes.GATEWAY_EXCLUSIVE)) {
      ...
      activity = parseTransaction(activityElement, scopeElement);
        ...
        || activityElement.getTagName().equals(ActivityTypes.GATEWAY_COMPLEX)) {
    ...
    if (activity.isAsyncAfter()) {
      for (PvmTransition transition : activity.getOutgoingTransitions()) {
        if (transition.getId() == null) {
          addError("Sequence flow with sourceRef='" + activity.getId()
              + "' must have an id, activity with id '" + activity.getId() + "' uses 'asyncAfter'.",
              null);
        }
      }
    }
  ...
  public void validateExclusiveGateway(ActivityImpl activity) {
    if (activity.getOutgoingTransitions().size() == 0) {
      ...
      addError("Exclusive Gateway '" + activity.getId() + "' has no outgoing sequence flows.",
          null);
      ...
      if (condition != null) {
        addError(
            "Exclusive Gateway '" + activity.getId() + "' has only one outgoing sequence flow ('"
                + flow.getId() + "'). This is not allowed to have a condition.",
            null);
      }
    } else {
      ...
      for (PvmTransition flow : activity.getOutgoingTransitions()) {
        ...
        if (hasConditon && isDefaultFlow) {
          addError("Exclusive Gateway '" + activity.getId() + "' has outgoing sequence flow '"
              + flow.getId() + "' which is the default flow but has a condition too.", null);
        }
      }
      if (hasDefaultFlow || flowsWithoutCondition.size() > 1) {
        ...
        for (PvmTransition flow : flowsWithoutCondition) {
          addError("Exclusive Gateway '" + activity.getId() + "' has outgoing sequence flow '"
              + flow.getId() + "' without condition which is not the default flow.", null);
        }
        ...
        addWarning("Exclusive Gateway '" + activity.getId() + "' has outgoing sequence flow '"
            + flow.getId()
  ...
  public ActivityImpl parseIntermediateCatchEvent(Element intermediateEventElement,
      ScopeImpl scopeElement, ActivityImpl eventBasedGateway) {
    ...
    Element conditionalEventDefinitionElement = intermediateEventElement
        .element(CONDITIONAL_EVENT_DEFINITION);

    // shared by all events except for link event
    IntermediateCatchEventActivityBehavior defaultCatchBehaviour = new IntermediateCatchEventActivityBehavior(
    ...
    if (timerEventDefinition != null) {
      ...
      ConditionalEventDefinition conditionalEvent = parseIntermediateConditionalEventDefinition(
          conditionalEventDefinitionElement, nestedActivity);
      nestedActivity.setActivityBehavior(
          new IntermediateConditionalEventBehavior(conditionalEvent, isEventBaseGatewayPresent));
    } else {
    ...
    for (BpmnParseListener parseListener : parseListeners) {
      parseListener.parseIntermediateCatchEvent(intermediateEventElement, scopeElement,
          nestedActivity);
    }
  ...
  protected void parseIntermediateLinkEventCatchBehavior(Element intermediateEventElement,
    ...
    if (eventLinkTargets.containsKey(linkName)) {
      addError("Multiple Intermediate Catch Events with the same link event name ('" + linkName
      ...
      if (!linkName.equals(elementName)) {
        ...
        addWarning(
            "Link Event named '" + elementName + "' contains link event definition with name '"
                + linkName + "' - it is recommended to use the same name for both.",
            intermediateEventElement);
      }
  ...
  protected void parseIntermediateMessageEventDefinition(Element messageEventDefinition,
    ...
    nestedActivity.getProperties().set(BpmnProperties.TYPE,
        ActivityTypes.INTERMEDIATE_EVENT_MESSAGE);

    EventSubscriptionDeclaration messageDefinition = parseMessageEventDefinition(
        messageEventDefinition);
    messageDefinition.setActivityId(nestedActivity.getId());
    addEventSubscriptionDeclaration(messageDefinition, nestedActivity.getEventScope(),
        messageEventDefinition);

    for (BpmnParseListener parseListener : parseListeners) {
      parseListener.parseIntermediateMessageCatchEventDefinition(messageEventDefinition,
          nestedActivity);
    }
  }
  ...
  public ActivityImpl parseIntermediateThrowEvent(Element intermediateEventElement,
    ...
    Element signalEventDefinitionElement = intermediateEventElement
        .element(SIGNAL_EVENT_DEFINITION);
    Element compensateEventDefinitionElement = intermediateEventElement
        .element(COMPENSATE_EVENT_DEFINITION);
    Element linkEventDefinitionElement = intermediateEventElement.element(LINK_EVENT_DEFINITION);
    Element messageEventDefinitionElement = intermediateEventElement
        .element(MESSAGE_EVENT_DEFINITION);
    Element escalationEventDefinition = intermediateEventElement
        .element(ESCALATION_EVENT_DEFINITION);
    ...
    if (signalEventDefinitionElement != null) {
      nestedActivityImpl.getProperties().set(BpmnProperties.TYPE,
          ActivityTypes.INTERMEDIATE_EVENT_SIGNAL_THROW);

      EventSubscriptionDeclaration signalDefinition = parseSignalEventDefinition(
          signalEventDefinitionElement, true);
      activityBehavior = new ThrowSignalEventActivityBehavior(signalDefinition);
    } else if (compensateEventDefinitionElement != null) {
      nestedActivityImpl.getProperties().set(BpmnProperties.TYPE,
          ActivityTypes.INTERMEDIATE_EVENT_COMPENSATION_THROW);
      ...
      if (isServiceTaskLike(messageEventDefinitionElement)) {
        ...
        activityBehavior = parseServiceTaskLike(ActivityTypes.INTERMEDIATE_EVENT_MESSAGE_THROW,
            messageEventDefinitionElement, scopeElement).getActivityBehavior();
      } else {
        // default to non behavior if no service task
        // properties have been specified
        nestedActivityImpl.getProperties().set(BpmnProperties.TYPE,
            ActivityTypes.INTERMEDIATE_EVENT_NONE_THROW);
        activityBehavior = new IntermediateThrowNoneEventActivityBehavior();
      }
    } else if (escalationEventDefinition != null) {
      nestedActivityImpl.getProperties().set(BpmnProperties.TYPE,
          ActivityTypes.INTERMEDIATE_EVENT_ESCALATION_THROW);
      ...
      if (escalation != null && escalation.getEscalationCode() == null) {
        addError("throwing escalation event must have an 'escalationCode'",
    ...
    for (BpmnParseListener parseListener : parseListeners) {
      parseListener.parseIntermediateThrowEvent(intermediateEventElement, scopeElement,
          nestedActivityImpl);
    }
  ...
  protected CompensateEventDefinition parseThrowCompensateEventDefinition(
      final Element compensateEventDefinitionElement, ScopeImpl scopeElement) {
    final String activityRef = compensateEventDefinitionElement.attribute("activityRef");
    boolean waitForCompletion = TRUE
        .equals(compensateEventDefinitionElement.attribute("waitForCompletion", TRUE));

    if (activityRef != null) {
      if (scopeElement.findActivityAtLevelOfSubprocess(activityRef) == null) {
        Boolean isTriggeredByEvent = scopeElement.getProperties()
            .get(BpmnProperties.TRIGGERED_BY_EVENT);
        ...
        if (scopeElement.findActivityAtLevelOfSubprocess(activityRef) == null) {
          ...
          scopeElement.addToBacklog(activityRef, new ScopeImpl.BacklogErrorCallback() {
            ...
            public void callback() {
              addError("Invalid attribute value for 'activityRef': no activity with id '"
                  + activityRef + "' in scope '" + scopeId + "'", compensateEventDefinitionElement);
            }
          });
        }
      }
  ...
  protected void parseBoundaryCompensateEventDefinition(Element compensateEventDefinition,
    ...
    for (ActivityImpl sibling : activity.getFlowScope().getActivities()) {
      if (sibling.getProperty(BpmnProperties.TYPE.getName()).equals("compensationBoundaryCatch")
          && sibling.getEventScope().equals(hostActivity) && sibling != activity) {
        addError(
  ...
  protected ActivityBehavior parseBoundaryCancelEventDefinition(Element cancelEventDefinition,
    ...
    if (transaction.getActivityBehavior() != null
        && transaction.getActivityBehavior() instanceof MultiInstanceActivityBehavior) {
    ...
    for (ActivityImpl sibling : activity.getFlowScope().getActivities()) {
      if ("cancelBoundaryCatch".equals(sibling.getProperty(BpmnProperties.TYPE.getName()))
  ...
  public ScopeImpl parseMultiInstanceLoopCharacteristics(Element activityElement, ScopeImpl scope) {
    ...
    if (miLoopCharacteristics == null) {
      ...
      boolean isSequential = parseBooleanAttribute(miLoopCharacteristics.attribute("isSequential"),
          false);
      ...
      if (loopCardinality != null) {
        ...
        behavior
            .setLoopCardinalityExpression(expressionManager.createExpression(loopCardinalityText));
      }
      ...
      if (completionCondition != null) {
        ...
        behavior.setCompletionConditionExpression(
            expressionManager.createExpression(completionConditionText));
      }

      // activiti:collection
      String collection = miLoopCharacteristics.attributeNS(CAMUNDA_BPMN_EXTENSIONS_NS,
          "collection");
      if (collection != null) {
      ...
      if (loopDataInputRef != null) {
        ...
        if (loopDataInputRefText != null) {
          if (loopDataInputRefText.contains("{")) {
            behavior
                .setCollectionExpression(expressionManager.createExpression(loopDataInputRefText));
          } else {
      ...
      String elementVariable = miLoopCharacteristics.attributeNS(CAMUNDA_BPMN_EXTENSIONS_NS,
          "elementVariable");
      ...
      // Validation
      if (behavior.getLoopCardinalityExpression() == null
          && behavior.getCollectionExpression() == null
      ...
      for (BpmnParseListener parseListener : parseListeners) {
        parseListener.parseMultiInstanceLoopCharacteristics(activityElement, miLoopCharacteristics,
            miBodyScope);
      }
  ...
  /**
   * Parses the generic information of an activity element (id, name, documentation, etc.), and
  ...
  public ActivityImpl createActivityOnScope(Element activityElement, ScopeImpl scopeElement) {
    ...
    activity.setProperty(PROPERTYNAME_JOB_PRIORITY,
        parsePriority(activityElement, PROPERTYNAME_JOB_PRIORITY));
  ...
  /**
   * Sets the delegates for the activity, which will be called if the attribute asyncAfter or
   * asyncBefore was changed.
   *
   * @param activity
    ...
    activity.setDelegateAsyncAfterUpdate(new ActivityImpl.AsyncAfterUpdate() {
      ...
      public void updateAsyncAfter(boolean asyncAfter, boolean exclusive) {
        if (asyncAfter) {
          ...
          removeMessageJobDeclarationWithJobConfiguration(activity,
              MessageJobDeclaration.ASYNC_AFTER);
        }
      }
    });
    ...
    activity.setDelegateAsyncBeforeUpdate(new ActivityImpl.AsyncBeforeUpdate() {
      ...
      public void updateAsyncBefore(boolean asyncBefore, boolean exclusive) {
        if (asyncBefore) {
          ...
          removeMessageJobDeclarationWithJobConfiguration(activity,
              MessageJobDeclaration.ASYNC_BEFORE);
        }
      }
    });
  }
  ...
  /**
   * Adds the new message job declaration to existing declarations. There will be executed an
   * existing check before the adding is executed.
   *
   * @param messageJobDeclaration
   *          the new message job declaration
   * @param activity
  ...
  protected void addMessageJobDeclaration(MessageJobDeclaration messageJobDeclaration,
      ActivityImpl activity, boolean exclusive) {
   ...
   *
   * @param msgJobdecl
   *          the message job declaration which is searched
   * @param procDefKey
   *          the corresponding process definition key
   * @param activityId
   *          the corresponding activity id
  ...
  /**
   * Removes a job declaration which belongs to the given activity and has the given job
   * configuration.
   *
   * @param activity
   *          the activity of the job declaration
  ...
  protected void removeMessageJobDeclarationWithJobConfiguration(ActivityImpl activity,
    ...
    List<MessageJobDeclaration> messageJobDeclarations = (List<MessageJobDeclaration>) activity
        .getProperty(PROPERTYNAME_MESSAGE_JOB_DECLARATION);
  ...
  public ActivityImpl parseEventBasedGateway(Element eventBasedGwElement, Element parentElement,
      ScopeImpl scope) {
    ...
    if (activity.isAsyncAfter()) {
      addError("'asyncAfter' not supported for " + eventBasedGwElement.getTagName() + " elements.",
          eventBasedGwElement);
    }
  ...
  protected ScriptTaskActivityBehavior parseScriptTaskElement(Element scriptTaskElement) {
    ...
    String scriptResource = scriptTaskElement.attributeNS(CAMUNDA_BPMN_EXTENSIONS_NS,
        PROPERTYNAME_RESOURCE);

    try {
      ExecutableScript script = ScriptUtil.getScript(language, scriptSource, scriptResource,
          expressionManager);
  ...
  public ActivityImpl parseServiceTaskLike(String elementName, Element serviceTaskElement,
      ScopeImpl scope) {
    ...
    String className = serviceTaskElement.attributeNS(CAMUNDA_BPMN_EXTENSIONS_NS,
        PROPERTYNAME_CLASS);
    String expression = serviceTaskElement.attributeNS(CAMUNDA_BPMN_EXTENSIONS_NS,
    ...
    if (type != null) {
      ...
      if (resultVariableName != null) {
        addError(
            "'resultVariableName' not supported for " + elementName + " elements using 'class'",
            serviceTaskElement);
      }
      ...
      if (resultVariableName != null) {
        addError("'resultVariableName' not supported for " + elementName
            + " elements using 'delegateExpression'", serviceTaskElement);
      }
  ...
  public ActivityImpl parseBusinessRuleTask(Element businessRuleTaskElement, ScopeImpl scope) {
    String decisionRef = businessRuleTaskElement.attributeNS(CAMUNDA_BPMN_EXTENSIONS_NS,
        "decisionRef");
    if (decisionRef != null) {
  ...
  protected ActivityImpl parseDmnBusinessRuleTask(Element businessRuleTaskElement,
    ...
    ParameterValueProvider definitionKeyProvider = createParameterValueProvider(decisionRef,
        expressionManager);
    ...
    parseVersion(businessRuleTaskElement, activity, callableElement, "decisionRefBinding",
        "decisionRefVersion");
    parseVersionTag(businessRuleTaskElement, activity, callableElement, "decisionRefBinding",
    ...
    DmnBusinessRuleTaskActivityBehavior behavior = new DmnBusinessRuleTaskActivityBehavior(
        callableElement, resultVariable, decisionResultMapper);
  ...
  protected DecisionResultMapper parseDecisionResultMapper(Element businessRuleTaskElement) {
    ...
    String decisionResultMapper = businessRuleTaskElement.attributeNS(CAMUNDA_BPMN_EXTENSIONS_NS,
        "mapDecisionResult");
    DecisionResultMapper mapper = DecisionEvaluationUtil
        .getDecisionResultMapperForName(decisionResultMapper);
  ...
  /**
   * Parse async continuation of an activity and create async jobs for the activity. <br/>
   * <br/>
   * When the activity is marked as multi instance, then async jobs create instead for the multi
   * instance body. When the wrapped activity has async characteristics in
   * 'multiInstanceLoopCharacteristics' element, then async jobs create additionally for the wrapped
   * activity.
   */
  ...
  protected ParameterValueProvider parsePriority(Element element, String priorityAttribute) {
    String priorityAttributeValue = element.attributeNS(CAMUNDA_BPMN_EXTENSIONS_NS,
        priorityAttribute);
    ...
    if (priorityAttributeValue == null) {
      ...
      if (!StringUtil.isExpression(priorityAttributeValue)) {
        ...
        try {
          ...
          addError("Value '" + priorityAttributeValue + "' for attribute '" + priorityAttribute
  ...
  protected void parseEmailServiceTask(ActivityImpl activity, Element serviceTaskElement,
      List<FieldDeclaration> fieldDeclarations) {
    validateFieldDeclarationsForEmail(serviceTaskElement, fieldDeclarations);
    activity.setActivityBehavior(
        (MailActivityBehavior) instantiateDelegate(MailActivityBehavior.class, fieldDeclarations));
  }

  protected void parseShellServiceTask(ActivityImpl activity, Element serviceTaskElement,
      List<FieldDeclaration> fieldDeclarations) {
    validateFieldDeclarationsForShell(serviceTaskElement, fieldDeclarations);
    activity.setActivityBehavior(
        (ActivityBehavior) instantiateDelegate(ShellActivityBehavior.class, fieldDeclarations));
  }

  protected void parseExternalServiceTask(ActivityImpl activity, Element serviceTaskElement) {
    activity.setScope(true);

    ParameterValueProvider topicNameProvider = parseTopic(serviceTaskElement,
        PROPERTYNAME_EXTERNAL_TASK_TOPIC);
    ParameterValueProvider priorityProvider = parsePriority(serviceTaskElement,
        PROPERTYNAME_TASK_PRIORITY);
    activity
        .setActivityBehavior(new ExternalTaskActivityBehavior(topicNameProvider, priorityProvider));
  }
  ...
  protected void validateFieldDeclarationsForShell(Element serviceTaskElement,
    ...
    for (FieldDeclaration fieldDeclaration : fieldDeclarations) {
      ...
      if ((fieldName.equals("wait") || fieldName.equals("redirectError")
          || fieldName.equals("cleanEnv")) && !fieldValue.toLowerCase().equals(TRUE)
          && !fieldValue.toLowerCase().equals("false")) {
        addError("undefined value for shell " + fieldName + " parameter :" + fieldValue.toString(),
            serviceTaskElement);
      }
  ...
  public List<FieldDeclaration> parseFieldDeclarations(Element element) {
    ...
    List<Element> fieldDeclarationElements = elementWithFieldInjections
        .elementsNS(CAMUNDA_BPMN_EXTENSIONS_NS, "field");
  ...
  protected FieldDeclaration parseFieldDeclaration(Element serviceTaskElement,
    ...
    FieldDeclaration fieldDeclaration = parseStringFieldDeclaration(fieldDeclarationElement,
        serviceTaskElement, fieldName);
    if (fieldDeclaration == null) {
      fieldDeclaration = parseExpressionFieldDeclaration(fieldDeclarationElement,
          serviceTaskElement, fieldName);
    }
  ...
  protected FieldDeclaration parseStringFieldDeclaration(Element fieldDeclarationElement,
    ...
    try {
      String fieldValue = getStringValueFromAttributeOrElement("stringValue", "string",
          fieldDeclarationElement);
      if (fieldValue != null) {
        return new FieldDeclaration(fieldName, Expression.class.getName(),
            new FixedValue(fieldValue));
      }
  ...
  protected FieldDeclaration parseExpressionFieldDeclaration(Element fieldDeclarationElement,
      Element serviceTaskElement, String fieldName) {
    try {
      String expression = getStringValueFromAttributeOrElement(PROPERTYNAME_EXPRESSION,
          PROPERTYNAME_EXPRESSION, fieldDeclarationElement);
  ...
  protected String getStringValueFromAttributeOrElement(String attributeName, String elementName,
      Element element) {
    String value = null;
    ...
    if (attributeValue != null && childElement != null) {
      addError("Can't use attribute '" + attributeName + "' and element '" + elementName
      ...
      if (stringElementText == null || stringElementText.length() == 0) {
        addError("No valid value found in attribute '" + attributeName + "' nor element '"
            + elementName + "'", element);
      } else {
        // Use text of element
  ...
  public ActivityImpl parseUserTask(Element userTaskElement, ScopeImpl scope) {
    ...
    TaskDefinition taskDefinition = parseTaskDefinition(userTaskElement, activity.getId(),
  ...
  public TaskDefinition parseTaskDefinition(Element taskElement, String taskDefinitionKey,
      ProcessDefinitionEntity processDefinition) {
    TaskFormHandler taskFormHandler;
    String taskFormHandlerClassName = taskElement.attributeNS(CAMUNDA_BPMN_EXTENSIONS_NS,
        "formHandlerClass");
    ...
    TaskDefinition taskDefinition = new TaskDefinition(
        new DelegateTaskFormHandler(taskFormHandler, deployment));
  ...
  protected void parseHumanPerformerResourceAssignment(Element performerElement,
    ...
    if (raeElement != null) {
      ...
      if (feElement != null) {
        taskDefinition
            .setAssigneeExpression(expressionManager.createExpression(feElement.getText()));
      }
    }
  }
  ...
  protected void parseUserTaskCustomExtensions(Element taskElement, TaskDefinition taskDefinition) {
    ...
    // Candidate users
    String candidateUsersString = taskElement.attributeNS(CAMUNDA_BPMN_EXTENSIONS_NS,
        CANDIDATE_USERS_EXTENSION);
    if (candidateUsersString != null) {
      List<String> candidateUsers = parseCommaSeparatedList(candidateUsersString);
      for (String candidateUser : candidateUsers) {
        taskDefinition
            .addCandidateUserIdExpression(expressionManager.createExpression(candidateUser.trim()));
      }
    }

    // Candidate groups
    String candidateGroupsString = taskElement.attributeNS(CAMUNDA_BPMN_EXTENSIONS_NS,
    ...
    // Due date
    String dueDateExpression = taskElement.attributeNS(CAMUNDA_BPMN_EXTENSIONS_NS,
        DUE_DATE_EXTENSION);
    ...
    // follow up date
    String followUpDateExpression = taskElement.attributeNS(CAMUNDA_BPMN_EXTENSIONS_NS,
        FOLLOW_UP_DATE_EXTENSION);
    if (followUpDateExpression != null) {
      taskDefinition
          .setFollowUpDateExpression(expressionManager.createExpression(followUpDateExpression));
    }

    // Priority
    final String priorityExpression = taskElement.attributeNS(CAMUNDA_BPMN_EXTENSIONS_NS,
        PRIORITY_EXTENSION);
  ...
  /**
   * Parses the given String as a list of comma separated entries, where an entry can possibly be an
   * expression that has comma's.
   *
  ...
  protected void parseTaskListeners(Element userTaskElement, TaskDefinition taskDefinition) {
    ...
    if (extentionsElement != null) {
      List<Element> taskListenerElements = extentionsElement.elementsNS(CAMUNDA_BPMN_EXTENSIONS_NS,
          "taskListener");
  ...
  protected TaskListener parseTaskListener(Element taskListenerElement) {
    ...
    if (className != null) {
      taskListener = new ClassDelegateTaskListener(className,
    ...
    for (Element endEventElement : parentElement.elements("endEvent")) {
      ...
      if (errorEventDefinition != null) { // error end event
        ...
        if (errorRef == null || "".equals(errorRef)) {
          ...
          if (error != null) {
            activity.setActivityBehavior(new ErrorEndEventActivityBehavior(error.getErrorCode(),
                error.getErrorMessageExpression()));
          } else {
        ...
        if (isServiceTaskLike(messageEventDefinitionElement)) {
          ...
          ActivityImpl act = parseServiceTaskLike(ActivityTypes.END_EVENT_MESSAGE,
              messageEventDefinitionElement, scope);
        ...
        activity
            .setActivityBehavior(new CompensationEventActivityBehavior(compensateEventDefinition));
        activity.setProperty(PROPERTYNAME_THROWS_COMPENSATION, true);
        activity.setScope(true);
        ...
        Escalation escalation = findEscalationForEscalationEventDefinition(
            escalationEventDefinition);
        if (escalation != null && escalation.getEscalationCode() == null) {
   ...
   *
   * Note that the boundary events are not parsed during the parsing of the bpmn activities, since
   * the semantics are different (boundaryEvent needs to be added as nested activity to the
   * reference activity on PVM level).
   *
   * @param parentElement
   *          The 'parent' element that contains the activities (process, subprocess).
   * @param flowScope
  ...
  public void parseBoundaryEvents(Element parentElement, ScopeImpl flowScope) {
    for (Element boundaryEventElement : parentElement.elements("boundaryEvent")) {
      ...
      if (attachedToRef == null || attachedToRef.equals("")) {
        addError("AttachedToRef is required when using a timerEventDefinition",
      ...
      Element conditionalEventDefinition = boundaryEventElement
          .element(CONDITIONAL_EVENT_DEFINITION);
      ...
      if (isCancelActivity) {
        ...
        boundaryEventActivity
            .setActivityStartBehavior(ActivityStartBehavior.CONCURRENT_IN_FLOW_SCOPE);
      }

      // Catch event behavior is the same for most types
      ActivityBehavior behavior = new BoundaryEventActivityBehavior();
      if (timerEventDefinition != null) {
        parseBoundaryTimerEventDefinition(timerEventDefinition, isCancelActivity,
            boundaryEventActivity);
        ...
        parseBoundarySignalEventDefinition(signalEventDefinition, isCancelActivity,
            boundaryEventActivity);
        ...
        parseBoundaryMessageEventDefinition(messageEventDefinition, isCancelActivity,
            boundaryEventActivity);

      } else if (escalationEventDefinition != null) {

        if (attachedActivity.isSubProcessScope()
            || attachedActivity.getActivityBehavior() instanceof CallActivityBehavior) {
        ...
        behavior = parseBoundaryConditionalEventDefinition(conditionalEventDefinition,
            isCancelActivity, boundaryEventActivity);
      } else {
  ...
  /**
   * Parses a boundary timer event. The end-result will be that the given nested activity will get
   * the appropriate {@link ActivityBehavior}.
   *
   ...
   *          The activity which maps to the structure of the timer event on the boundary of another
   *          activity. Note that this is NOT the activity onto which the boundary event is
   *          attached, but a nested activity inside this activity, specifically created for this
   *          event.
   */
  public void parseBoundaryTimerEventDefinition(Element timerEventDefinition, boolean interrupting,
    ...
    TimerDeclarationImpl timerDeclaration = parseTimer(timerEventDefinition, boundaryActivity,
    ...
    for (BpmnParseListener parseListener : parseListeners) {
      parseListener.parseBoundaryTimerEventDefinition(timerEventDefinition, interrupting,
          boundaryActivity);
    }
  }

  public void parseBoundarySignalEventDefinition(Element element, boolean interrupting,
  ...
  public void parseBoundaryMessageEventDefinition(Element element, boolean interrupting,
    ...
    addEventSubscriptionDeclaration(messageEventDefinition, messageActivity.getEventScope(),
        element);
  ...
  protected void parseTimerStartEventDefinition(Element timerEventDefinition,
    ...
    TimerDeclarationImpl timerDeclaration = parseTimer(timerEventDefinition, timerActivity,
  ...
  protected void parseTimerStartEventDefinitionForEventSubprocess(Element timerEventDefinition,
      ActivityImpl timerActivity, boolean interrupting) {
    timerActivity.getProperties().set(BpmnProperties.TYPE, ActivityTypes.START_EVENT_TIMER);
  ...
  protected void parseEventDefinitionForSubprocess(
      EventSubscriptionDeclaration subscriptionDeclaration, ActivityImpl activity,
      Element element) {
  ...
  protected void parseIntermediateSignalEventDefinition(Element element,
      ActivityImpl signalActivity) {
    signalActivity.getProperties().set(BpmnProperties.TYPE,
        ActivityTypes.INTERMEDIATE_EVENT_SIGNAL);
  ...
  protected void parseSignalCatchEventDefinition(Element element, ActivityImpl signalActivity,
    ...
    EventSubscriptionJobDeclaration catchingAsyncDeclaration = new EventSubscriptionJobDeclaration(
        signalDefinition);
    catchingAsyncDeclaration.setJobPriorityProvider(
        (ParameterValueProvider) signalActivity.getProperty(PROPERTYNAME_JOB_PRIORITY));
   ...
   *
   * @param signalEventDefinitionElement
   *          the Signal Event Definition element
   * @param isThrowing
   *          true if a Throwing signal event is being parsed
   * @return
   */
  protected EventSubscriptionDeclaration parseSignalEventDefinition(
      Element signalEventDefinitionElement, boolean isThrowing) {
    ...
    if (signalRef == null) {
      ...
      if (isThrowing) {
        ...
        signalEventDefinition = new EventSubscriptionDeclaration(signalDefinition.getExpression(),
            EventType.SIGNAL, payload);
      } else {
        signalEventDefinition = new EventSubscriptionDeclaration(signalDefinition.getExpression(),
            EventType.SIGNAL);
      }

      boolean throwingAsync = TRUE.equals(
          signalEventDefinitionElement.attributeNS(CAMUNDA_BPMN_EXTENSIONS_NS, "async", "false"));
  ...
  protected TimerDeclarationImpl parseTimer(Element timerEventDefinition,
      ActivityImpl timerActivity, String jobHandlerType) {
    // TimeDate
    ...
    if (expression == null) {
      addError("Timer needs configuration (either timeDate, timeCycle or timeDuration is needed).",
          timerEventDefinition);
    }

    // Parse the timer declaration
    // TODO move the timer declaration into the bpmn activity or next to the TimerSession
    TimerDeclarationImpl timerDeclaration = new TimerDeclarationImpl(expression, type,
        jobHandlerType);
    timerDeclaration.setRawJobHandlerConfiguration(timerActivity.getId());
    timerDeclaration
        .setExclusive(TRUE.equals(timerEventDefinition.attributeNS(CAMUNDA_BPMN_EXTENSIONS_NS,
    ...
    addJobDeclarationToProcessDefinition(timerDeclaration,
        (ProcessDefinition) timerActivity.getProcessDefinition());

    timerDeclaration.setJobPriorityProvider(
        (ParameterValueProvider) timerActivity.getProperty(PROPERTYNAME_JOB_PRIORITY));
  ...
  public void parseBoundaryErrorEventDefinition(Element errorEventDefinition,
    ...
    for (BpmnParseListener parseListener : parseListeners) {
      parseListener.parseBoundaryErrorEventDefinition(errorEventDefinition, true,
          (ActivityImpl) boundaryEventActivity.getEventScope(), boundaryEventActivity);
    }
  }

  protected void addErrorEventDefinition(ErrorEventDefinition errorEventDefinition,
      ScopeImpl catchingScope) {
    catchingScope.getProperties().addListItem(BpmnProperties.ERROR_EVENT_DEFINITIONS,
        errorEventDefinition);

    List<ErrorEventDefinition> errorEventDefinitions = catchingScope.getProperties()
  ...
  protected void parseBoundaryEscalationEventDefinition(Element escalationEventDefinitionElement,
      boolean cancelActivity, ActivityImpl boundaryEventActivity) {
    boundaryEventActivity.getProperties().set(BpmnProperties.TYPE,
        ActivityTypes.BOUNDARY_ESCALATION);

    EscalationEventDefinition escalationEventDefinition = createEscalationEventDefinitionForEscalationHandler(
        escalationEventDefinitionElement, boundaryEventActivity, cancelActivity);
    addEscalationEventDefinition(boundaryEventActivity.getEventScope(), escalationEventDefinition,
        escalationEventDefinitionElement);

    for (BpmnParseListener parseListener : parseListeners) {
      parseListener.parseBoundaryEscalationEventDefinition(escalationEventDefinitionElement,
          cancelActivity, boundaryEventActivity);
    }
  }

  /**
   * Find the referenced escalation of the given escalation event definition. Add errors if the
   * referenced escalation not found.
   *
   * @return referenced escalation or <code>null</code>, if referenced escalation not found
   */
  protected Escalation findEscalationForEscalationEventDefinition(
  ...
  protected EscalationEventDefinition createEscalationEventDefinitionForEscalationHandler(
      Element escalationEventDefinitionElement, ActivityImpl escalationHandler,
      boolean cancelActivity) {
    EscalationEventDefinition escalationEventDefinition = new EscalationEventDefinition(
        escalationHandler, cancelActivity);
    ...
    String escalationCodeVariable = escalationEventDefinitionElement
        .attributeNS(CAMUNDA_BPMN_EXTENSIONS_NS, "escalationCodeVariable");
  ...
  protected void addEscalationEventDefinition(ScopeImpl catchingScope,
    ...
    // ensure there is only one escalation handler (e.g. escalation boundary event, escalation event
    // subprocess) what can catch the escalation event
    for (EscalationEventDefinition existingEscalationEventDefinition : catchingScope.getProperties()
      ...
      if (existingEscalationEventDefinition.getEscalationHandler().isSubProcessScope()
        ...
        if (existingEscalationEventDefinition.getEscalationCode() == null
            && escalationEventDefinition.getEscalationCode() == null) {
          addError(
              ...
              element);
        } else if (existingEscalationEventDefinition.getEscalationCode() == null
            || escalationEventDefinition.getEscalationCode() == null) {
          addError(
              ...
              element);
        } else if (existingEscalationEventDefinition.getEscalationCode()
            .equals(escalationEventDefinition.getEscalationCode())) {
          addError("multiple escalation event subprocesses with the same escalationCode '"
              + escalationEventDefinition.getEscalationCode() + "' are not supported on same scope",
              element);
        }
        ...
        if (existingEscalationEventDefinition.getEscalationCode() == null
          ...
          addError(
            ...
            .equals(escalationEventDefinition.getEscalationCode())) {
          addError("multiple escalation boundary events with the same escalationCode '"
              + escalationEventDefinition.getEscalationCode() + "' are not supported on same scope",
              element);
        }
      }
    ...
    catchingScope.getProperties().addListItem(BpmnProperties.ESCALATION_EVENT_DEFINITIONS,
        escalationEventDefinition);
  }

  protected void addTimerDeclaration(ScopeImpl scope, TimerDeclarationImpl timerDeclaration) {
    scope.getProperties().putMapEntry(BpmnProperties.TIMER_DECLARATIONS,
        timerDeclaration.getActivityId(), timerDeclaration);
  }
   ...
   *
   * @param element
   *          the XML element which contains the conditional event information
   * @param interrupting
   *          indicates if the event is interrupting or not
   * @param conditionalActivity
   *          the conditional event activity
   * @return the boundary conditional event behavior which contains the condition
   */
  public BoundaryConditionalEventActivityBehavior parseBoundaryConditionalEventDefinition(
      Element element, boolean interrupting, ActivityImpl conditionalActivity) {
    conditionalActivity.getProperties().set(BpmnProperties.TYPE,
        ActivityTypes.BOUNDARY_CONDITIONAL);

    ConditionalEventDefinition conditionalEventDefinition = parseConditionalEventDefinition(element,
        conditionalActivity);
    conditionalEventDefinition.setInterrupting(interrupting);
    addEventSubscriptionDeclaration(conditionalEventDefinition, conditionalActivity.getEventScope(),
        element);

    for (BpmnParseListener parseListener : parseListeners) {
      parseListener.parseBoundaryConditionalEventDefinition(element, interrupting,
          conditionalActivity);
    }
  ...
  public ConditionalEventDefinition parseIntermediateConditionalEventDefinition(Element element,
    ...
    conditionalActivity.getProperties().set(BpmnProperties.TYPE,
        ActivityTypes.INTERMEDIATE_EVENT_CONDITIONAL);
  ...
  public ConditionalEventDefinition parseConditionalStartEventForEventSubprocess(Element element,
    ...
    for (BpmnParseListener parseListener : parseListeners) {
      parseListener.parseConditionalStartEventForEventSubprocess(element, conditionalActivity,
          interrupting);
    }
  ...
  protected ConditionalEventDefinition parseConditionalEventDefinition(Element element,
    ...
    if (conditionExprElement != null) {
      ...
      conditionalActivity.getProcessDefinition().getProperties()
          .set(BpmnProperties.HAS_CONDITIONAL_EVENTS, true);
      ...
      final String variableEvents = element.attributeNS(CAMUNDA_BPMN_EXTENSIONS_NS,
          "variableEvents");
      ...
      for (String variableEvent : variableEventsList) {
        if (!VARIABLE_EVENTS.contains(variableEvent)) {
          addWarning("Variable event: " + variableEvent
              + " is not valid. Possible variable change events are: "
              + Arrays.toString(VARIABLE_EVENTS.toArray()), element);
        }
      }
  ...
  /**
   * Parses a subprocess (formally known as an embedded subprocess): a subprocess defined within
  ...
  public ActivityImpl parseCallActivity(Element callActivityElement, ScopeImpl scope,
    ...
    String className = callActivityElement.attributeNS(CAMUNDA_BPMN_EXTENSIONS_NS,
        PROPERTYNAME_VARIABLE_MAPPING_CLASS);
    String delegateExpression = callActivityElement.attributeNS(CAMUNDA_BPMN_EXTENSIONS_NS,
    ...
    if (calledElement != null) {
      ...
      ParameterValueProvider definitionKeyProvider = createParameterValueProvider(calledElement,
          expressionManager);
      callableElement.setDefinitionKeyValueProvider(definitionKeyProvider);

    } else {
      behavior = new CaseCallActivityBehavior();
      ParameterValueProvider definitionKeyProvider = createParameterValueProvider(caseRef,
          expressionManager);
      callableElement.setDefinitionKeyValueProvider(definitionKeyProvider);
    ...
    // parse version
    parseVersion(callActivityElement, activity, callableElement, bindingAttributeName,
        versionAttributeName);

    // parse versionTag
    parseVersionTag(callActivityElement, activity, callableElement, bindingAttributeName,
        versionTagAttributeName);
  ...
  protected void parseBinding(Element callActivityElement, ActivityImpl activity,
      BaseCallableElement callableElement, String bindingAttributeName) {
    String binding = callActivityElement.attributeNS(CAMUNDA_BPMN_EXTENSIONS_NS,
  ...
  protected void parseTenantId(Element callingActivityElement, ActivityImpl activity,
  ...
  protected void parseVersion(Element callingActivityElement, ActivityImpl activity,
      BaseCallableElement callableElement, String bindingAttributeName,
    ...
    if (binding != null && binding.equals(CallableElementBinding.VERSION) && version == null) {
      addError(
          "Missing attribute '" + versionAttributeName + "' when '" + bindingAttributeName
              + "' has value '" + CallableElementBinding.VERSION.getValue() + "'",
  ...
  protected void parseVersionTag(Element callingActivityElement, ActivityImpl activity,
      BaseCallableElement callableElement, String bindingAttributeName,
      String versionTagAttributeName) {
    String versionTag = null;

    CallableElementBinding binding = callableElement.getBinding();
    versionTag = callingActivityElement.attributeNS(CAMUNDA_BPMN_EXTENSIONS_NS,
        versionTagAttributeName);

    if (binding != null && binding.equals(CallableElementBinding.VERSION_TAG)
        && versionTag == null) {
      addError(
          "Missing attribute '" + versionTagAttributeName + "' when '" + bindingAttributeName
              + "' has value '" + CallableElementBinding.VERSION_TAG.getValue() + "'",
          callingActivityElement);
    }

    ParameterValueProvider versionTagProvider = createParameterValueProvider(versionTag,
        expressionManager);
    ...
    if (extensionsElement != null) {
      ...
      for (Element inElement : extensionsElement.elementsNS(CAMUNDA_BPMN_EXTENSIONS_NS, "in")) {
        ...
        if (businessKey != null && !businessKey.isEmpty()) {
          ParameterValueProvider businessKeyValueProvider = createParameterValueProvider(
              businessKey, expressionManager);
  ...
  protected void parseOutputParameter(Element callActivityElement, ActivityImpl activity,
      CallableElement callableElement) {
  ...
  protected boolean attributeValueEquals(Element element, String attribute,
  ...
  protected CallableElementParameter parseCallableElementProvider(Element parameterElement) {
    ...
    if (ALL.equals(variables)) {
      ...
      boolean strictValidation = !Context.getProcessEngineConfiguration()
      ...
      if (strictValidation && source == null) {
        addError("Missing parameter 'source' or 'sourceExpression' when passing variables",
            parameterElement);
      }
  ...
  /**
   * Parses the properties of an element (if any) that can contain properties (processes,
   * activities, etc.)
   *
  ...
  public void parseProperty(Element propertyElement, ActivityImpl activity) {
    ...
    if (name == null) {
      if (id == null) {
        addError("Invalid property usage on line " + propertyElement.getLine()
  ...
  public void parseSequenceFlow(Element processElement, ScopeImpl scope,
    ...
    for (Element sequenceFlowElement : processElement.elements("sequenceFlow")) {
      ...
      if ((sourceActivity == null && compensationHandlers.containsKey(sourceRef))
        ...
        addError("Invalid source '" + sourceRef + "' of sequence flow '" + id + "'",
            sequenceFlowElement);
        ...
        // ignore
      } else if (destinationActivity
          .getActivityBehavior() instanceof IntermediateCatchEventActivityBehavior
          && (destinationActivity.getEventScope() != null) && (destinationActivity.getEventScope()
              .getActivityBehavior() instanceof EventBasedGatewayActivityBehavior)) {
        addError(
            "Invalid incoming sequenceflow for intermediateCatchEvent with id '"
                + destinationActivity.getId() + "' connected to an event-based gateway.",
            sequenceFlowElement);
  ...
  protected Condition parseConditionExpression(Element conditionExprElement) {
    ...
    String resource = conditionExprElement.attributeNS(CAMUNDA_BPMN_EXTENSIONS_NS,
        PROPERTYNAME_RESOURCE);
    if (type != null) {
    ...
    if (language == null) {
      ...
      try {
        ExecutableScript script = ScriptUtil.getScript(language, expression, resource,
            expressionManager);
  ...
  public void parseExecutionListenersOnScope(Element scopeElement, ScopeImpl scope) {
    Element extentionsElement = scopeElement.element("extensionElements");
    if (extentionsElement != null) {
      List<Element> listenerElements = extentionsElement.elementsNS(CAMUNDA_BPMN_EXTENSIONS_NS,
          "executionListener");
  ...
  public void parseExecutionListenersOnTransition(Element activitiElement,
    ...
    if (extensionElements != null) {
      List<Element> listenerElements = extensionElements.elementsNS(CAMUNDA_BPMN_EXTENSIONS_NS,
          "executionListener");
      for (Element listenerElement : listenerElements) {
        ExecutionListener listener = parseExecutionListener(listenerElement);
        if (listener != null) {
  ...
  public ExecutionListener parseExecutionListener(Element executionListenerElement) {
    ...
    String delegateExpression = executionListenerElement
        .attribute(PROPERTYNAME_DELEGATE_EXPRESSION);
    Element scriptElement = executionListenerElement.elementNS(CAMUNDA_BPMN_EXTENSIONS_NS,
        "script");
  ...
  public void parseBPMNShape(Element bpmnShapeElement) {
    ...
    if (bpmnElement != null && !"".equals(bpmnElement)) {
      ...
      if (participantProcesses.get(bpmnElement) != null) {
        ProcessDefinitionEntity procDef = getProcessDefinition(
            participantProcesses.get(bpmnElement));
        ...
        if (activity != null) {
          ...
          if (lane != null) {
            ...
            parseDIBounds(bpmnShapeElement, lane);
                                                          ...
                                                          // still reference
                                                          // 'something'
            addError("Invalid reference in 'bpmnElement' attribute, activity " + bpmnElement
                + " not found", bpmnShapeElement);
          }
        }
      }
    } else {
  ...
  protected void parseDIBounds(Element bpmnShapeElement, HasDIBounds target) {
    ...
    if (bounds != null) {
      target.setX(
          parseDoubleAttribute(bpmnShapeElement, "x", bounds.attribute("x"), true).intValue());
      target.setY(
          parseDoubleAttribute(bpmnShapeElement, "y", bounds.attribute("y"), true).intValue());
      target
          .setWidth(parseDoubleAttribute(bpmnShapeElement, "width", bounds.attribute("width"), true)
              .intValue());
      target.setHeight(
          parseDoubleAttribute(bpmnShapeElement, "height", bounds.attribute("height"), true)
              .intValue());
    } else {
      ...
      if (sequenceFlows != null && sequenceFlows.containsKey(sequenceFlowId)) {
        ...
        if (waypointElements.size() >= 2) {
          ...
          for (Element waypointElement : waypointElements) {
            waypoints.add(
                parseDoubleAttribute(waypointElement, "x", waypointElement.attribute("x"), true)
                    .intValue());
            waypoints.add(
                parseDoubleAttribute(waypointElement, "y", waypointElement.attribute("y"), true)
                    .intValue());
          }
          sequenceFlow.setWaypoints(waypoints);
        } else {
          addError("Minimum 2 waypoint elements must be definted for a 'BPMNEdge'",
              bpmnEdgeElement);
        }
                                                         ...
                                                         // reference
                                                         // 'something'
        addError("Invalid reference in 'bpmnElement' attribute, sequenceFlow " + sequenceFlowId
            + "not found", bpmnEdgeElement);
      }
    } else {
  ...
  public Boolean parseBooleanAttribute(String booleanText) {
    if (TRUE.equals(booleanText) || "enabled".equals(booleanText) || "on".equals(booleanText)
        || "active".equals(booleanText) || "yes".equals(booleanText)) {
      return Boolean.TRUE;
    }
    if ("false".equals(booleanText) || "disabled".equals(booleanText) || "off".equals(booleanText)
        || "inactive".equals(booleanText) || "no".equals(booleanText)) {
      return Boolean.FALSE;
    }
    return null;
  }

  public Double parseDoubleAttribute(Element element, String attributeName, String doubleText,
      boolean required) {
  ...
  protected boolean isStartable(Element element) {
    return TRUE.equalsIgnoreCase(
        element.attributeNS(CAMUNDA_BPMN_EXTENSIONS_NS, "isStartableInTasklist", TRUE));
  }

  protected boolean isExclusive(Element element) {
    return TRUE.equals(element.attributeNS(CAMUNDA_BPMN_EXTENSIONS_NS, "exclusive",
  ...
  protected boolean checkActivityInputOutputSupported(Element activityElement,
      ActivityImpl activity, IoMapping inputOutput) {
    String tagName = activityElement.getTagName();

    if (!(tagName.toLowerCase().contains("task") || tagName.contains("Event")
        || tagName.equals("transaction") || tagName.equals("subProcess")
        || tagName.equals("callActivity"))) {
      addError("camunda:inputOutput mapping unsupported for element type '" + tagName + "'.",
          activityElement);
      return false;
    }

    if (tagName.equals("subProcess")
        && TRUE.equals(activityElement.attribute("triggeredByEvent"))) {
      addError("camunda:inputOutput mapping unsupported for element type '" + tagName
  ...
  protected boolean checkActivityOutputParameterSupported(Element activityElement,
    ...
    if (tagName.equals("endEvent")) {
      addError("camunda:outputParameter not allowed for element type '" + tagName + "'.",
          activityElement);
      return true;
    } else if (getMultiInstanceScope(activity) != null) {
      addError("camunda:outputParameter not allowed for multi-instance constructs",
          activityElement);
      return false;
    } else {
      return true;
  ...
  protected void ensureNoIoMappingDefined(Element element) {
    ...
    if (inputOutput != null) {
      addError("camunda:inputOutput mapping unsupported for element type '" + element.getTagName()
          + "'.", element);
    }
  }
  ...
  protected void ensureNoExpressionInMessageStartEvent(Element element,
    ...
    if (messageStartEventSubscriptionDeclaration.hasEventName()) {
      eventNameContainsExpression = !messageStartEventSubscriptionDeclaration
    ...
    if (eventNameContainsExpression) {
      ...
      addError(
          "Invalid message name '" + messageStartName + "' for element '" + element.getTagName()
              + "': expressions in the message start event name are not allowed!",
          element);
    }
  }
