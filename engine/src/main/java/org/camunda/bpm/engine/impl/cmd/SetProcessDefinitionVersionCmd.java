 ...
 * {@link HistoricProcessInstance} using the new process definition version and the same business
 * key as the {@link ProcessInstance} that is to be migrated.
 *
...
public class SetProcessDefinitionVersionCmd implements Command<Void>, Serializable {
  ...
  public SetProcessDefinitionVersionCmd(String processInstanceId,
    ...
    ensureNotNull("The process definition version is mandatory", "processDefinitionVersion",
        processDefinitionVersion);
    ensurePositive("The process definition version must be positive", "processDefinitionVersion",
        processDefinitionVersion.longValue());
  ...
  public Void execute(CommandContext commandContext) {
    ...
    if (processInstance == null) {
      ...
      throw new ProcessEngineException("A process instance id is required, but the provided id "
    ...
    if (currentProcessDefinitionImpl instanceof ProcessDefinitionEntity) {
      ...
      currentProcessDefinition = deploymentCache
          .findDeployedProcessDefinitionById(currentProcessDefinitionImpl.getId());
    }

    ProcessDefinitionEntity newProcessDefinition = deploymentCache
        .findDeployedProcessDefinitionByKeyVersionAndTenantId(currentProcessDefinition.getKey(),
            processDefinitionVersion, currentProcessDefinition.getTenantId());
    ...
    List<JobEntity> jobs = commandContext.getJobManager()
        .findJobsByProcessInstanceId(processInstanceId);
    List<JobDefinitionEntity> currentJobDefinitions = commandContext.getJobDefinitionManager()
        .findByProcessDefinitionId(currentProcessDefinition.getId());
    List<JobDefinitionEntity> newVersionJobDefinitions = commandContext.getJobDefinitionManager()
        .findByProcessDefinitionId(newProcessDefinition.getId());

    Map<String, String> jobDefinitionMapping = getJobDefinitionMapping(currentJobDefinitions,
    ...
    List<IncidentEntity> incidents = commandContext.getIncidentManager()
        .findIncidentsByProcessInstance(processInstanceId);
    ...
    PropertyChange change = new PropertyChange("processDefinitionVersion",
        currentProcessDefinition.getVersion(), processDefinitionVersion);
    commandContext.getOperationLogManager().logProcessInstanceOperation(
        UserOperationLogEntry.OPERATION_TYPE_MODIFY_PROCESS_INSTANCE, processInstanceId, null, null,
        Collections.singletonList(change));

    return null;
  }

  protected Map<String, String> getJobDefinitionMapping(
  ...
  protected boolean jobDefinitionsMatch(JobDefinitionEntity currentJobDefinition,
      JobDefinitionEntity newJobDefinition) {
    boolean activitiesMatch = currentJobDefinition.getActivityId()
        .equals(newJobDefinition.getActivityId());

    boolean typesMatch = (currentJobDefinition.getJobType() == null
        && newJobDefinition.getJobType() == null)
        || (currentJobDefinition.getJobType() != null
            && currentJobDefinition.getJobType().equals(newJobDefinition.getJobType()));

    boolean configurationsMatch = (currentJobDefinition.getJobConfiguration() == null
        && newJobDefinition.getJobConfiguration() == null)
        || (currentJobDefinition.getJobConfiguration() != null && currentJobDefinition
            .getJobConfiguration().equals(newJobDefinition.getJobConfiguration()));

    return activitiesMatch && typesMatch && configurationsMatch;
  }

  protected void switchVersionOfJob(JobEntity jobEntity,
      ProcessDefinitionEntity newProcessDefinition, Map<String, String> jobDefinitionMapping) {
  ...
  protected void switchVersionOfIncident(CommandContext commandContext,
  ...
  protected void validateAndSwitchVersionOfExecution(CommandContext commandContext,
      ...
      if (newActivity == null) {
        throw new ProcessEngineException(
            "The new process definition " + "(key = '" + newProcessDefinition.getKey() + "') "
                + "does not contain the current activity " + "(id = '" + activityId + "') "
    ...
    List<TaskEntity> tasks = commandContext.getTaskManager()
