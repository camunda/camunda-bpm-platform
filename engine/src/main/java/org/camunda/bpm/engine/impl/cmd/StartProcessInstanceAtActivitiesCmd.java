...
public class StartProcessInstanceAtActivitiesCmd implements Command<ProcessInstanceWithVariables> {
  ...
  public ProcessInstanceWithVariables execute(CommandContext commandContext) {
    ...
    // apply modifications
    List<AbstractProcessInstanceModificationCommand> instructions = modificationBuilder
  ...
  /**
   * get the activity that is started by the first instruction, if exists; return null if the first
   * instruction is a start-transition instruction
   */
  protected ActivityImpl determineFirstActivity(ProcessDefinitionImpl processDefinition,
    ...
    AbstractProcessInstanceModificationCommand firstInstruction = modificationBuilder
        .getModificationOperations().get(0);
    ...
    if (firstInstruction instanceof AbstractInstantiationCmd) {
      ...
      ensureNotNull(NotValidException.class,
          "Element '" + instantiationInstruction.getTargetElementId()
