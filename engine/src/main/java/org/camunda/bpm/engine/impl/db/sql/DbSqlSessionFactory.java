...
public class DbSqlSessionFactory implements SessionFactory {
  ...
  static {
    ...
    String defaultOrderBy = " order by ${@org.camunda.bpm.engine.impl.db.sql.MybatisJoinHelper@orderBy(orderingProperties)} ";
    ...
    databaseSpecificLimitBetweenStatements.put(DB2, ", row_number() over (ORDER BY ${@org.camunda.bpm.engine.impl.db.sql.MybatisJoinHelper@orderBy(orderingProperties)}) rnk FROM ( select distinct RES.* ");
    databaseSpecificLimitBetweenFilterStatements.put(DB2, ", row_number() over (ORDER BY ${@org.camunda.bpm.engine.impl.db.sql.MybatisJoinHelper@orderBy(orderingProperties)}) rnk FROM ( select distinct RES.ID_, RES.REV_, RES.RESOURCE_TYPE_, RES.NAME_, RES.OWNER_ ");
    databaseSpecificOrderByStatements.put(DB2, "");
    databaseSpecificLimitBeforeNativeQueryStatements.put(DB2, "SELECT SUB.* FROM ( select RES.* , row_number() over (ORDER BY ${@org.camunda.bpm.engine.impl.db.sql.MybatisJoinHelper@orderBy(orderingProperties)}) rnk FROM (");
    ...
    databaseSpecificLimitBetweenStatements.put(MSSQL, ", row_number() over (ORDER BY ${@org.camunda.bpm.engine.impl.db.sql.MybatisJoinHelper@orderBy(orderingProperties)}) rnk FROM ( select distinct RES.* ");
    databaseSpecificLimitBetweenFilterStatements.put(MSSQL, "");
    databaseSpecificOrderByStatements.put(MSSQL, "");
    databaseSpecificLimitBeforeNativeQueryStatements.put(MSSQL, "SELECT SUB.* FROM ( select RES.* , row_number() over (ORDER BY ${@org.camunda.bpm.engine.impl.db.sql.MybatisJoinHelper@orderBy(orderingProperties)}) rnk FROM (");
