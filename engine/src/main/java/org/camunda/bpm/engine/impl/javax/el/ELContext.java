...
public abstract class ELContext {
  ...
  /**
   * Returns the context object associated with the given key. The ELContext maintains a collection
   * of context objects relevant to the evaluation of an expression. These context objects are used
   * by ELResolvers. This method is used to retrieve the context with the given key from the
   * collection. By convention, the object returned will be of the type specified by the key.
   ...
   * the ELResolver that will be consulted to resolve variables and properties during an expression
   * evaluation. This method retrieves the reference to the resolver. Once an ELContext is
   * constructed, the reference to the ELResolver associated with the context cannot be changed.
   * 
   ...
   * The {@link CompositeELResolver} checks this property to determine whether it should consider or
   ...
   * context objects relevant to the evaluation of an expression. These context objects are used by
   * ELResolvers. This method is used to add a context object to that collection. By convention, the
   * contextObject will be of the type specified by the key. However, this is not required and the
   * key is used strictly as a unique identifier.
   * 
   * @param key
  ...
  /**
   * Set the Locale for this instance. This method may be called by the party creating the instance,
   * such as JavaServer Faces or JSP, to enable the EL implementation to provide localized messages
   * to the user. If no Locale is set, the implementation must use the locale returned by
   * Locale.getDefault( ).
   * 
   * @param locale
