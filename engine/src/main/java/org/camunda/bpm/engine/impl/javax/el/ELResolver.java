...
public abstract class ELResolver {
   ...
   * base object. One use for this method is to assist tools in auto-completion. This assists tools
   * in auto-completion and also provides a way to express that the resolver accepts a primitive
   * value, such as an integer index into an array. For example, the {@link ArrayELResolver} will
   * accept any int as a property, so the return value would be Integer.class.
   *
   * @param context
   *          The context of this evaluation.
   * @param base
   *          The base object to return the most general property type for, or null to enumerate the
   ...
   * base object. One use for this method is to assist tools in auto-completion. If the base
   * parameter is null, the resolver must enumerate the list of top-level variables it can resolve.
   * The Iterator returned must contain zero or more instances of java.beans.FeatureDescriptor, in
   * no guaranteed order. In the case of primitive types such as int, the value null must be
   * returned. This is to prevent the useless iteration through all possible primitive values. A
   * return value of null indicates that this resolver does not handle the given base object or that
   * the results are too complex to represent with this method and the
   * {@link #getCommonPropertyType(ELContext, Object)} method should be used instead. Each
   * FeatureDescriptor will contain information about a single variable or property. In addition to
   * the standard properties, the FeatureDescriptor must have two named attributes (as set by the
   * setValue method):
   * <ul>
   ...
   * <li>{@link #RESOLVABLE_AT_DESIGN_TIME} - The value of this named attribute must be an instance
   * of java.lang.Boolean and indicates whether it is safe to attempt to resolve this property at
   * designtime. For instance, it may be unsafe to attempt a resolution at design time if the
   * ELResolver needs access to a resource that is only available at runtime and no acceptable
   * simulated value can be provided.</li>
   * </ul>
   * The caller should be aware that the Iterator returned might iterate through a very large or
   * even infinitely large set of properties. Care should be taken by the caller to not get stuck in
   * an infinite loop. This is a "best-effort" list. Not all ELResolvers will return completely
   * accurate results, but all must be callable at both design-time and runtime (i.e. whether or not
   * Beans.isDesignTime() returns true), without causing errors. The propertyResolved property of
   * the ELContext is not relevant to this method. The results of all ELResolvers are concatenated
   ...
   * {@link #setValue(ELContext, Object, Object, Object)} method. If this resolver handles the given
   ...
   * called, the caller should ignore the return value. This is not always the same as
   ...
   *          The context of this evaluation.
   * @param base
   *          The base object whose property value is to be analyzed, or null to analyze a top-level
   *          variable.
   * @param property
   ...
   *           if the given (base, property) pair is handled by this ELResolver but the specified
   ...
   * handles the given (base, property) pair, the propertyResolved property of the ELContext object
   * must be set to true by the resolver, before returning. If this property is not true after this
   * method is called, the caller should ignore the return value.
   *
   * @param context
   *          The context of this evaluation.
   * @param base
   *          The base object whose property value is to be returned, or null to resolve a top-level
   *          variable.
   * @param property
   ...
   * {@link #setValue(ELContext, Object, Object, Object)} will always fail. If this resolver handles
   * the given (base, property) pair, the propertyResolved property of the ELContext object must be
   ...
   * ELContext object must be set to true by the resolver, before returning. If this property is not
   * true after this method is called, the caller can safely assume no value has been set.
   *
   * @param context
   *          The context of this evaluation.
   * @param base
   ...
   * property of the <code>ELContext</code> object must be set to <code>true</code> by the resolver,
   * before returning. If this property is not <code>true</code> after this method is called, the
   ...
   * @param paramTypes
   *          An array of Class objects identifying the method's formal parameter types, in declared
   *          order. Use an empty array if the method has no parameters. Can be <code>null</code>,
   ...
   *           exception must be included as the cause property of this exception, if available. If
   *           the exception thrown is an <code>InvocationTargetException</code>, extract its
