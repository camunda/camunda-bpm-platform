...
public abstract class Expression implements Serializable {
  ...
  /**
   * Determines whether the specified object is equal to this Expression. The result is true if and
   * only if the argument is not null, is an Expression object that is the of the same type
   ...
   * debugging purposes but also for the purposes of comparison (e.g. to ensure the expression in a
   * configuration file has not changed). This method does not provide sufficient information to
   * re-create an expression. Two different expressions can have exactly the same expression string
   * but different function mappings. Serialization should be used to save and restore the state of
   * an Expression.
   * 
   ...
   * on each of the two objects must produce the same integer result. Implementations must take
