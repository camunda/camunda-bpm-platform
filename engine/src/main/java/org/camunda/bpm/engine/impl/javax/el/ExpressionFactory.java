...
public abstract class ExpressionFactory {
   ...
   * META-INF/services/javax.el.ExpressionFactory exists, then its first line, if present, is used
   * as the UTF-8 encoded name of the implementation class.</li>
   ...
   * whose key is "javax.el.ExpressionFactory", then the value of that entry is used as the name of
   * the implementation class.</li>
   * <li>Use the javax.el.ExpressionFactory system property. If a system property with this name is
   ...
   * same lookup procedure as the one used in newInstance(). If the argument properties is not null,
   * and if the implementation contains a constructor with a single parameter of type
   ...
   * @return An instance of ExpressionFactory.
   * @throws ELException
   *           if the class could not be found or if it is not a subclass of ExpressionFactory or if
  ...
  /**
   * Parses an expression into a {@link MethodExpression} for later evaluation. Use this method for
   * expressions that refer to methods. If the expression is a String literal, a MethodExpression is
   * created, which when invoked, returns the String literal, coerced to expectedReturnType. An
   * ELException is thrown if expectedReturnType is void or if the coercion of the String literal to
   * the expectedReturnType yields an error (see Section "1.16 Type Conversion"). This method should
   * perform syntactic validation of the expression. If in doing so it detects errors, it should
   * raise an ELException.
   * 
   * @param context
   ...
   *          expression. They can be null, in which case functions or variables are not supported
   *          for this expression. The object returned must invoke the same functions and access the
   *          same variable mappings regardless of whether the mappings in the provided
   *          FunctionMapper and VariableMapper instances change between calling
   *          ExpressionFactory.createMethodExpression() and any method on MethodExpression. Note
   *          that within the EL, the ${} and #{} syntaxes are treated identically. This includes
   *          the use of VariableMapper and FunctionMapper at expression creation time. Each is
   *          invoked if not null, independent of whether the #{} or ${} syntax is used for the
   *          expression.
   * @param expression
   *          The expression to parse
   * @param expectedReturnType
   *          The expected return type for the method to be found. After evaluating the expression,
   *          the MethodExpression must check that the return type of the actual method matches this
   *          type. Passing in a value of null indicates the caller does not care what the return
   *          type is, and the check is disabled.
   ...
   *          same variable mappings regardless of whether the mappings in the provided
   *          FunctionMapper and VariableMapper instances change between calling
   *          ExpressionFactory.createValueExpression() and any method on ValueExpression. Note that
   *          within the EL, the ${} and #{} syntaxes are treated identically. This includes the use
   *          of VariableMapper and FunctionMapper at expression creation time. Each is invoked if
   *          not null, independent of whether the #{} or ${} syntax is used for the expression.
   * @param expression
   *          The expression to parse
   * @param expectedType
   ...
   * object as a ValueExpression. The wrapper ValueExpression is read only, and returns the wrapped
   * object via its getValue() method, optionally coerced.
   * 
   * @param instance
   *          The object instance to be wrapped.
   * @param expectedType
   *          The type the result of the expression will be coerced to after evaluation. There will
   *          be no coercion if it is Object.class,
