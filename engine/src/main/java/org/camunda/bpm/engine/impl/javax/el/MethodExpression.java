...
public abstract class MethodExpression extends Expression {
   ...
   * expected return type of the method signature. An ELException is thrown if expectedReturnType is
   * void or if the coercion of the String literal to the expectedReturnType yields an error (see
   * Section "1.16 Type Conversion" of the EL specification). If not a String literal, evaluates the
   * expression relative to the provided context, invokes the method that was found using the
   * supplied parameters, and returns the result of the method invocation. Any parameters passed to
   * this method is ignored if isLiteralText() is true.
   * 
   * @param context
   *          The context of this evaluation.
   * @param params
   ...
   *           if no suitable method can be found.
   * @throws ELException
   *           if a String literal is specified and expectedReturnType of the MethodExpression is
   *           void or if the coercion of the String literal to the expectedReturnType yields an
   *           error (see Section "1.16 Type Conversion").
   * @throws ELException
   *           if an exception was thrown while performing property or variable resolution. The
   *           thrown exception must be included as the cause property of this exception, if
   *           available. If the exception thrown is an InvocationTargetException, extract its cause
   *           and pass it to the ELException constructor.
   */
