...
/**
 * Simple (thread-safe) LRU cache. After the cache size reached a certain limit, the least recently
 * used entry is removed, when adding a new entry.
 *
...
public final class Cache implements TreeCache {
  ...
  /**
   * Constructor. If the least recently used entry is removed from the primary cache, it is added to
   * the secondary map.
   * 
   * @param size
   *          maximum primary cache size
