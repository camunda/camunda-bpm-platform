...
public class Scanner {
   ...
   */
  public enum Symbol {
    EOF, PLUS("'+'"), MINUS("'-'"), MUL("'*'"), DIV("'/'|'div'"), MOD("'%'|'mod'"), LPAREN(
        "'('"), RPAREN("')'"), IDENTIFIER, NOT("'!'|'not'"), AND("'&&'|'and'"), OR(
            "'||'|'or'"), EMPTY("'empty'"), INSTANCEOF("'instanceof'"), INTEGER, FLOAT, TRUE(
                "'true'"), FALSE("'false'"), STRING, NULL("'null'"), LE("'<='|'le'"), LT(
                    "'<'|'lt'"), GE("'>='|'ge'"), GT("'>'|'gt'"), EQ("'=='|'eq'"), NE(
                        "'!='|'ne'"), QUESTION("'?'"), COLON("':'"), TEXT, DOT("'.'"), LBRACK(
                            "'['"), RBRACK("']'"), COMMA("','"), START_EVAL_DEFERRED(
  ...
  protected boolean isEval() {
    return token != null && token.getSymbol() != Symbol.TEXT
      ...
      if (c == '\\') {
        if (i == l) {
          ...
          if (c == '\\' || c == quote) {
            ...
            throw new ScanException(position, "invalid escape sequence \\" + c,
                "\\" + quote + " or \\\\");
          }
        }
      } else if (c == quote) {
  ...
  /**
   * Scan next token. After calling this method, {@link #getToken()} and {@link #getPosition()} can
   * be used to retreive the token's image and input position.
   * 
