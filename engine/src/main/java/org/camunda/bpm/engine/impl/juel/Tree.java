...
/**
 * Parsed expression, usually created by a {@link org.camunda.bpm.engine.impl.juel.TreeBuilder}. The
 * {@link #bind(FunctionMapper, VariableMapper)} method is used to create
 * {@link org.camunda.bpm.engine.impl.juel.Bindings}, which are needed at evaluation time to lookup
 * functions and variables. The tree itself does not contain such information, because it would make
 * the tree depend on the function/variable mapper supplied at parse time.
 * 
...
public class Tree {
   ...
   */
  public Tree(ExpressionNode root, Collection<FunctionNode> functions,
      Collection<IdentifierNode> identifiers, boolean deferred) {
    super();
    this.root = root;
