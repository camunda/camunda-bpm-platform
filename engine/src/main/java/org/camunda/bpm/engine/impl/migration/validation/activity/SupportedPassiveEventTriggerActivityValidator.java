...
/**
 * Validator for events that passively wait for an event, i.e. without being activated by sequence
 * flow (e.g. boundary events and event subprocess start events but not intermediate catch events).
 *
...
public class SupportedPassiveEventTriggerActivityValidator implements MigrationActivityValidator {
  ...
  public static final List<String> supportedTypes = Arrays.asList(ActivityTypes.BOUNDARY_MESSAGE,
      ActivityTypes.BOUNDARY_SIGNAL, ActivityTypes.BOUNDARY_TIMER,
      ActivityTypes.BOUNDARY_COMPENSATION, ActivityTypes.BOUNDARY_CONDITIONAL,
      ActivityTypes.START_EVENT_MESSAGE, ActivityTypes.START_EVENT_SIGNAL,
      ActivityTypes.START_EVENT_TIMER, ActivityTypes.START_EVENT_COMPENSATION,
      ActivityTypes.START_EVENT_CONDITIONAL);

  public boolean valid(ActivityImpl activity) {
    return activity != null
        && (!isPassivelyWaitingEvent(activity) || isSupportedEventType(activity));
  }
