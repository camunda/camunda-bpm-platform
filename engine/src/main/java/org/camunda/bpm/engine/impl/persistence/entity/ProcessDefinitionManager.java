...
public class ProcessDefinitionManager extends AbstractManager
  ...
  public ProcessDefinitionEntity findLatestProcessDefinitionByKey(String processDefinitionKey) {
    ...
    List<ProcessDefinitionEntity> processDefinitions = getDbEntityManager().selectList(
        "selectLatestProcessDefinitionByKey", configureParameterizedQuery(processDefinitionKey));
  ...
  public ProcessDefinitionEntity findLatestProcessDefinitionByKeyAndTenantId(
    ...
    if (tenantId == null) {
      return (ProcessDefinitionEntity) getDbEntityManager()
          .selectOne("selectLatestProcessDefinitionByKeyWithoutTenantId", parameters);
    } else {
      return (ProcessDefinitionEntity) getDbEntityManager()
          .selectOne("selectLatestProcessDefinitionByKeyAndTenantId", parameters);
    }
  }
  ...
  public List<ProcessDefinition> findProcessDefinitionsByQueryCriteria(
    ...
    return getDbEntityManager().selectList("selectProcessDefinitionsByQueryCriteria",
  ...
  public long findProcessDefinitionCountByQueryCriteria(
    ...
    configureProcessDefinitionQuery(processDefinitionQuery);
    return (Long) getDbEntityManager().selectOne("selectProcessDefinitionCountByQueryCriteria",
  ...
  public ProcessDefinitionEntity findProcessDefinitionByKeyVersionAndTenantId(
      String processDefinitionKey, Integer processDefinitionVersion, String tenantId) {
    return findProcessDefinitionByKeyVersionOrVersionTag(processDefinitionKey,
        processDefinitionVersion, null, tenantId);
  }

  public ProcessDefinitionEntity findProcessDefinitionByKeyVersionTagAndTenantId(
      String processDefinitionKey, String processDefinitionVersionTag, String tenantId) {
    return findProcessDefinitionByKeyVersionOrVersionTag(processDefinitionKey, null,
        processDefinitionVersionTag, tenantId);
  }

  protected ProcessDefinitionEntity findProcessDefinitionByKeyVersionOrVersionTag(
      String processDefinitionKey, Integer processDefinitionVersion,
      String processDefinitionVersionTag, String tenantId) {
    Map<String, Object> parameters = new HashMap<String, Object>();
    ...
    @SuppressWarnings("unchecked")
    List<ProcessDefinitionEntity> results = getDbEntityManager()
        .selectList("selectProcessDefinitionByKeyVersionAndTenantId", parameters);
    if (results.size() == 1) {
      return results.get(0);
    } else if (results.size() > 1) {
      if (processDefinitionVersion != null) {
        throw LOG.toManyProcessDefinitionsException(results.size(), processDefinitionKey, "version",
            processDefinitionVersion.toString(), tenantId);
      } else if (processDefinitionVersionTag != null) {
        throw LOG.toManyProcessDefinitionsException(results.size(), processDefinitionKey,
            "versionTag", processDefinitionVersionTag, tenantId);
      }
    }
    return null;
  }
  ...
  public String findPreviousProcessDefinitionId(String processDefinitionKey, Integer version,
      String tenantId) {
  ...
  public List<ProcessDefinition> findDefinitionsByKeyAndTenantId(String processDefinitionKey,
      String tenantId, boolean isTenantIdSet) {
    Map<String, Object> parameters = new HashMap<String, Object>();
    parameters.put("processDefinitionKey", processDefinitionKey);
  ...
  public void updateProcessDefinitionSuspensionStateById(String processDefinitionId,
    ...
    getDbEntityManager().update(ProcessDefinitionEntity.class,
        "updateProcessDefinitionSuspensionStateByParameters",
  ...
  public void updateProcessDefinitionSuspensionStateByKeyAndTenantId(String processDefinitionKey,
      String tenantId, SuspensionState suspensionState) {
    Map<String, Object> parameters = new HashMap<String, Object>();
    parameters.put("processDefinitionKey", processDefinitionKey);
  ...
  /**
   * Cascades the deletion of the process definition to the process instances. Skips the custom
   * listeners if the flag was set to true.
   *
   * @param processDefinitionId
   *          the process definition id
   * @param skipCustomListeners
   *          true if the custom listeners should be skipped at process instance deletion
  ...
  protected void cascadeDeleteProcessInstancesForProcessDefinition(String processDefinitionId,
    ...
    getProcessInstanceManager().deleteProcessInstancesByProcessDefinition(processDefinitionId,
        "deleted process definition", true, skipCustomListeners, skipIoMappings);
  }
  ...
  protected void cascadeDeleteHistoryForProcessDefinition(String processDefinitionId) {
    ...
    getHistoricIdentityLinkManager()
        .deleteHistoricIdentityLinksLogByProcessDefinitionId(processDefinitionId);
  ...
  protected void deleteTimerStartEventsForProcessDefinition(ProcessDefinition processDefinition) {
    List<JobEntity> timerStartJobs = getJobManager().findJobsByConfiguration(
  ...
  /**
   * Deletes the subscriptions for the process definition, which is identified by the given process
   * definition id.
   *
   * @param processDefinitionId
  ...
  /**
   * Deletes the given process definition from the database and cache. If cascadeToHistory and
   * cascadeToInstances is set to true it deletes the history and the process instances.
   *
   * *Note*: If more than one process definition, from one deployment, is deleted in a single
   * transaction and the cascadeToHistory and cascadeToInstances flag was set to true it can cause a
   * dirty deployment cache. The process instances of ALL process definitions must be deleted,
   * before every process definition can be deleted! In such cases the cascadeToInstances flag have
   * to set to false!
   *
   * On deletion of all process instances, the task listeners will be deleted as well. Deletion of
   * tasks and listeners needs the redeployment of deployments. It can cause to problems if is done
   * sequential with the deletion of process definition in a single transaction.
   *
   * *For example*: Deployment contains two process definition. First process definition and
   * instances will be removed, also cleared from the cache. Second process definition will be
   * removed and his instances. Deletion of instances will cause redeployment this deploys again
   * first into the cache. Only the second will be removed from cache and first remains in the cache
   * after the deletion process.
   *
   * @param processDefinition
   *          the process definition which should be deleted
   * @param processDefinitionId
   *          the id of the process definition
   * @param cascadeToHistory
   *          if true the history will deleted as well
   * @param cascadeToInstances
   *          if true the process instances are deleted as well
   * @param skipCustomListeners
   *          if true skips the custom listeners on deletion of instances
   * @param skipIoMappings
   *          specifies whether input/output mappings for tasks should be invoked
   */
  public void deleteProcessDefinition(ProcessDefinition processDefinition,
      String processDefinitionId, boolean cascadeToHistory, boolean cascadeToInstances,
      boolean skipCustomListeners, boolean skipIoMappings) {
    ...
    if (cascadeToHistory) {
      ...
      ProcessInstanceQueryImpl procInstQuery = new ProcessInstanceQueryImpl()
          .processDefinitionId(processDefinitionId);
      long processInstanceCount = getProcessInstanceManager()
          .findProcessInstanceCountByQueryCriteria(procInstQuery);
    ...
    getDbEntityManager().delete(ProcessDefinitionEntity.class, "deleteProcessDefinitionsById",
        processDefinitionId);
  ...
  @Override
  public ProcessDefinitionEntity findDefinitionByKeyVersionTagAndTenantId(String definitionKey,
      String definitionVersionTag, String tenantId) {
    return findProcessDefinitionByKeyVersionTagAndTenantId(definitionKey, definitionVersionTag,
        tenantId);
  }
