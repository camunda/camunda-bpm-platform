 ...
 * </ul>
 *
 * 2. In certain situations, executions which were both scope and concurrent were created. This used
 * to be the case if a scope execution already had a single scope child execution and then
 * concurrency was introduced (by a on interrupting boundary event or a non-interrupting event
 * subprocess). In that case the existing scope execution was made concurrent. Starting from 7.3
 * this behavior is considered legacy. The new behavior is that the existing scope execution will
 * not be made concurrent, instead, a new, concurrent execution will be created and be interleaved
 * between the parent and the existing scope execution.
 * <p>
 *
 * @author Daniel Meyer
 * @since 7.3
...
public class LegacyBehavior {
  ...
  /**
   * Prunes a concurrent scope. This can only happen if (a) the process instance has been migrated
   * from a previous version to a new version of the process engine
   *
  ...
  /**
   * Cancels an execution which is both concurrent and scope. This can only happen if (a) the
   * process instance has been migrated from a previous version to a new version of the process
   * engine
   *
   * See: javadoc of this class for note about concurrent scopes.
   *
   * @param execution
   *          the concurrent scope execution to destroy
   * @param cancelledScopeActivity
   *          the activity that cancels the execution; it must hold that cancellingActivity's event
   *          scope is the scope the execution is responsible for
   */
   ...
   * been migrated from a previous version to a 7.3+ version of the process engine
   *
   * See: javadoc of this class for note about concurrent scopes.
   *
   * @param execution
   *          the execution to destroy
   */
  ...
  public static boolean eventSubprocessConcurrentChildExecutionEnded(
          ...
          if (flowScope == endedExecution.getActivity().getProcessDefinition()) {
            ...
            if (activityBehavior instanceof CompositeActivityBehavior) {
              ((CompositeActivityBehavior) activityBehavior)
                  .concurrentChildExecutionEnded(scopeExecution, endedExecution);
            }
          }
        }
      }
  ...
  /**
   * Destroy an execution for an activity that was previously not a scope and now is (e.g. event
   * subprocess)
   */
  ...
  protected static boolean isLegacyBehaviorRequired(ActivityExecution scopeExecution) {
    ...
    Map<ScopeImpl, PvmExecutionImpl> activityExecutionMapping = scopeExecution
    ...
    return activityExecutionMapping.get(activity) == activityExecutionMapping
        .get(activity.getFlowScope());
  }

  /**
   * In case the process instance was migrated from a previous version, activities which are now
   * parsed as scopes do not have scope executions. Use the flow scopes of these activities in order
   * to find their execution. - For an event subprocess this is the scope execution of the scope in
   * which the event subprocess is embeded in - For a multi instance sequential subprocess this is
   * the multi instace scope body.
   *
   * @param scope
  ...
  /**
   * Creates an activity execution mapping, when the scope hierarchy and the execution hierarchy are
   * out of sync.
   *
   ...
   * @return
   */
  public static Map<ScopeImpl, PvmExecutionImpl> createActivityExecutionMapping(
      List<PvmExecutionImpl> scopeExecutions, List<ScopeImpl> scopes) {
    PvmExecutionImpl deepestExecution = scopeExecutions.get(0);
    if (isLegacyAsyncAtMultiInstance(deepestExecution)) {
      // in case the deepest execution is in fact async at multi-instance, the multi instance body
      // is part
      // of the list of scopes, however it is not instantiated yet or has already ended. Thus it
      // must be removed.
      scopes.remove(0);
    }
    ...
    // further up the hierarchy miss scopes. So we need to iterate from the top down and skip all
    // scopes which
  ...
  protected static boolean wasNoScope72(ActivityImpl activity) {
    ...
    ActivityBehavior parentActivityBehavior = (ActivityBehavior) (activity.getFlowScope() != null
        ? activity.getFlowScope().getActivityBehavior()
        : null);
  ...
  protected static boolean wasNoScope73(ActivityImpl activity,
      PvmExecutionImpl scopeExecutionCandidate) {
    ActivityBehavior activityBehavior = activity.getActivityBehavior();
  ...
  protected static boolean isMultiInstanceInCompensation(ActivityImpl activity,
      PvmExecutionImpl scopeExecutionCandidate) {
    return activity.getActivityBehavior() instanceof MultiInstanceActivityBehavior
  ...
  /**
   * This returns true only if the provided execution has reached its wait state in a legacy engine
   * version, because only in that case, it can be async and waiting at the inner activity wrapped
   * by the miBody. In versions >= 7.3, the execution would reference the multi-instance body
   * instead.
   */
  protected static boolean isLegacyAsyncAtMultiInstance(PvmExecutionImpl execution) {
    ...
    if (activity != null) {
      ...
      boolean isAtMultiInstance = activity.getParentFlowScopeActivity() != null
          && activity.getParentFlowScopeActivity()
  ...
  /**
   * Tolerates the broken execution trees fixed with CAM-3727 where there may be more ancestor scope
   * executions than ancestor flow scopes;
   *
   * In that case, the argument execution is removed, the parent execution of the argument is
   * returned such that one level of mismatch is corrected.
   *
   * Note that this does not necessarily skip the correct scope execution, since the broken
   * parent-child relationships may be anywhere in the tree (e.g. consider a non-interrupting
   * boundary event followed by a subprocess (i.e. scope), when the subprocess ends, we would skip
   * the subprocess's execution).
   *
   */
  public static PvmExecutionImpl determinePropagatingExecutionOnEnd(
  ...
  /**
   * <p>
   * Required for migrating active sequential MI receive tasks. These activities were formerly not
   * scope, but are now. This has the following implications:
   *
   * <p>
   * Before migration:
   * <ul>
   ...
   * After migration:
   * <ul>
   * <li>a new subscription is created for every instance
   * <li>the new subscription is attached to a dedicated scope execution as a child of the miBody
   * scope execution
   * </ul>
   *
   * <p>
  ...
  public static void removeLegacySubscriptionOnParent(ExecutionEntity execution,
    ...
    ActivityBehavior parentBehavior = (ActivityBehavior) (activity.getFlowScope() != null
        ? activity.getFlowScope().getActivityBehavior()
        : null);

    if (behavior instanceof ReceiveTaskActivityBehavior
        && parentBehavior instanceof MultiInstanceActivityBehavior) {
      List<EventSubscriptionEntity> parentSubscriptions = execution.getParent()
          .getEventSubscriptions();

      for (EventSubscriptionEntity subscription : parentSubscriptions) {
        // distinguish a boundary event on the mi body with the same message name from the receive
        // task subscription
  ...
  public static void removeLegacyNonScopesFromMapping(Map<ScopeImpl, PvmExecutionImpl> mapping) {
    ...
    for (Map.Entry<PvmExecutionImpl, List<ScopeImpl>> scopesForExecution : scopesForExecutions
        .entrySet()) {
  ...
  /**
   * This is relevant for {@link GetActivityInstanceCmd} where in case of legacy multi-instance
   * execution trees, the default algorithm omits multi-instance activity instances.
   */
  ...
  /**
   * When deploying an async job definition for an activity wrapped in an miBody, set the activity
   * id to the miBody except the wrapped activity is marked as async.
   *
    ...
    for (JobDefinitionEntity jobDefinition : jobDefinitions) {
      ...
      if (activityId != null) {
        ...
        if (!isAsync(activity) && isActivityWrappedInMultiInstanceBody(activity)
            && isAsyncJobDefinition(jobDefinition)) {
   ...
   *
   * Background: in <= 7.2 async jobs were created for the inner activity, although the semantics
   * are that they are executed before the miBody is entered
   */
  ...
  /**
   * With prior versions, the boundary event was already executed when compensation was performed;
   * Thus, after compensation completes, the execution is signalled waiting at the boundary event.
   */
   ...
   * </p>
   *
   * @return true, if the argument is not a leaf and has an invalid (i.e. legacy) non-null activity
   *         id
   */
  public static boolean hasInvalidIntermediaryActivityId(PvmExecutionImpl execution) {
    return !execution.getNonEventScopeExecutions().isEmpty()
        && !CompensationBehavior.isCompensationThrowing(execution);
  }

  /**
   * Returns true if the given execution is in a compensation-throwing activity but there is no
   * dedicated scope execution in the given mapping.
   */
    ...
    if (CompensationBehavior.isCompensationThrowing(execution)) {
      ...
      if (compensationThrowingActivity.isScope()) {
        return activityExecutionMapping
            .get(compensationThrowingActivity) == activityExecutionMapping
                .get(compensationThrowingActivity.getFlowScope());
      } else {
        // for transaction sub processes with cancel end events, the compensation throwing execution
        // waits in the boundary event, not in the end
        // event; cancel boundary events are currently not scope
        return compensationThrowingActivity
            .getActivityBehavior() instanceof CancelBoundaryEventActivityBehavior;
      }
    } else {
      return false;
    }
