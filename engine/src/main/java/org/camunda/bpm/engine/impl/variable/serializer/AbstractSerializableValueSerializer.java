...
public abstract class AbstractSerializableValueSerializer<T extends SerializableValue>
    ...
    if (value.isDeserialized()) {
      ...
      if (objectToSerialize != null) {
        ...
        try {
          ...
          throw new ProcessEngineException("Cannot serialize object in variable '"
              + valueFields.getName() + "': " + e.getMessage(), e);
        }
      }
    } else {
    ...
    if (deserializeObjectValue) {
      ...
      if (serializedByteValue != null) {
        try {
          ...
          throw new ProcessEngineException("Cannot deserialize object in variable '"
              + valueFields.getName() + "': " + e.getMessage(), e);
        }
      }
  ...
  protected abstract void writeToValueFields(T value, ValueFields valueFields,
  ...
  protected boolean canWriteValue(TypedValue typedValue) {
    ...
    if (typedValue instanceof SerializableValue) {
      ...
      if (!serializableValue.isDeserialized()) {
        ...
        final boolean canSerialize = typedValue.getValue() == null
            || canSerializeValue(typedValue.getValue());
        return canSerialize
            && (requestedDataFormat == null || serializationDataFormat.equals(requestedDataFormat));
      }
    } else {
   ...
   *
   * @param value
   *          the object to test (guaranteed to be a non-null value)
