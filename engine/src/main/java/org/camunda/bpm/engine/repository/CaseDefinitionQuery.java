...
public interface CaseDefinitionQuery extends Query<CaseDefinitionQuery, CaseDefinition> {
  ...
  /**
   * Only select case definitions where the key matches the given parameter. The syntax that should
   * be used is the same as in SQL, eg. %activiti%
   *
   * @param caseDefinitionKeyLike
   *          the pattern to match the case definition key
   */
  CaseDefinitionQuery caseDefinitionKeyLike(String caseDefinitionKeyLike);

  /**
   * Only select case definitions where the name matches the given parameter. The syntax that should
   * be used is the same as in SQL, eg. %activiti%
   *
   * @param caseDefinitionNameLike
  ...
  /**
   * Only select case definition with a certain version. Particularly useful when used in
   * combination with {@link #caseDefinitionKey(String)}
   *
  ...
  /**
   * Only select the case definitions which are the latest deployed (ie. which have the highest
   * version number for the given key).
   *
   * Can only be used in combination with {@link #caseDefinitionKey(String)} or
   * {@link #caseDefinitionKeyLike(String)}. Can also be used without any other criteria (ie.
   * query.latest().list()), which will then give all the latest versions of all the deployed case
   * definitions.
   *
   */
  ...
  /**
   * Only select case definition with a resource name like the given. The syntax that should be used
  ...
  /**
   * Select case definitions which have no tenant id. Can be used in combination with
  ...
  /**
   * Order by the category of the case definitions (needs to be followed by {@link #asc()} or
   * {@link #desc()}).
   */
  ...
  /**
   * Order by the id of the case definitions (needs to be followed by {@link #asc()} or
   * {@link #desc()}).
   */
  CaseDefinitionQuery orderByCaseDefinitionId();

  /**
   * Order by the version of the case definitions (needs to be followed by {@link #asc()} or
   * {@link #desc()}).
   */
  CaseDefinitionQuery orderByCaseDefinitionVersion();

  /**
   * Order by the name of the case definitions (needs to be followed by {@link #asc()} or
   * {@link #desc()}).
   */
   ...
   * ordering of case instances without tenant id is database-specific.
   */
