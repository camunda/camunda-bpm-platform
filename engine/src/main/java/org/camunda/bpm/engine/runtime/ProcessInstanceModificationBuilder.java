...
/**
 * <p>
 * A fluent builder to specify a modification of process instance state in terms of cancellation of
 * activity instances and instantiations of activities and sequence flows. Allows to specify an
 * ordered set of instructions that are all executed within one transaction. Individual instructions
 * are executed in the order of their specification.
 * </p>
 *
...
public interface ProcessInstanceModificationBuilder
   ...
   *
   * <p>
   * Start before the specified activity. Instantiate the given activity as a descendant of the
   * given ancestor activity instance.
   * </p>
   *
   * <p>
   * In particular:
   ...
   * <li>Instantiate and execute the given activity (respects the asyncBefore attribute of the
   * activity)</li>
   * </ul>
   * </p>
   *
   ...
   * @param ancestorActivityInstanceId
   *          the ID of an existing activity instance under which the new activity instance should
   *          be created
   */
  ...
  /**
   * Submits an instruction that behaves like {@link #startTransition(String,String)} and always
   * instantiates the single outgoing sequence flow of the given activity. Does not consider
   * asyncAfter.
   *
   * @param activityId
   ...
   *
   * <p>
   * Start the specified sequence flow. Instantiate the given sequence flow as a descendant of the
   * given ancestor activity instance.
   * </p>
   *
   * <p>
   ...
   * @param ancestorActivityInstanceId
   *          the ID of an existing activity instance under which the new transition should be
   *          executed
   */
   ...
   *
   * <p>
   * Cancel an activity instance in a process. If this instance has child activity instances (e.g.
   * in a subprocess instance), these children, their grandchildren, etc. are cancelled as well.
   * </p>
   *
   * <p>
   * Process instance cancellation will propagate upward, removing any parent process instances that
   * are only waiting on the cancelled process to complete.
   * </p>
   *
   ...
   *
   * <p>
   * Therefore behaves like {@link #cancelActivityInstance(String)} for each individual activity
   * instance and like {@link #cancelTransitionInstance(String)} for each individual transition
   * instance.
   * </p>
   *
   * <p>
  ...
  /**
   * Execute all instructions. Custom execution and task listeners, as well as task input output
   ...
   *           permission on {@link Resources#PROCESS_DEFINITION}. if the process instance will be
   *           delete and the user has no {@link Permissions#DELETE} permission on
   ...
   *          transaction when executing the instructions
   *
   * @throws AuthorizationException
   *           if the user has no {@link Permissions#UPDATE} permission on
   *           {@link Resources#PROCESS_INSTANCE} or no {@link Permissions#UPDATE_INSTANCE}
  ...
  /**
   * Execute all instructions asynchronously. Custom execution and task listeners, as well as task
   * input output mappings are executed.
   *
   * @throws AuthorizationException
