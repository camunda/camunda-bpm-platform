...
public interface ProcessInstanceQuery extends Query<ProcessInstanceQuery, ProcessInstance> {
  ...
  /**
   * Select historic process instances that don't have a process-definition of which the key is
  ...
  /**
   * Select the process instance that has as sub case instance the given case instance. Note that
   * there will always be at most <b>one</b> such process instance that can be the result of this
   * query.
   *
   * @since 7.3
   */
  ...
  /**
   * Only selects process instances which are suspended, either because the process instance itself
   * is suspended or because the corresponding process definition is suspended
   */
  ProcessInstanceQuery suspended();

  /**
   * Only selects process instances which are active, which means that neither the process instance
  ...
  /**
   * <p>
   * Only selects process instances with leaf activity instances or transition instances (async
   * before, async after) in at least one of the given activity ids.
   *
   * <p>
   * <i>Leaf instance</i> means this filter works for instances of a user task is matched, but not
   * the embedded sub process it is contained in.
   */
  ...
  /**
   * Only selects process instances which don't have subprocesses and thus are leaves of the
   * execution tree.
   */
   ...
   * </p>
   *
   * @return an object of the type {@link ProcessInstanceQuery} on which an arbitrary amount of
