...
public class AuthorizationScenarioInstance {
  ...
  public AuthorizationScenarioInstance(AuthorizationScenario scenario,
      AuthorizationService authorizationService, Map<String, String> resourceBindings) {
  ...
  public void init(AuthorizationService authorizationService,
    ...
    for (AuthorizationSpec authorizationSpec : scenario.getGivenAuthorizations()) {
      Authorization authorization = authorizationSpec.instantiate(authorizationService,
          resourceBindings);
  ...
  public void tearDown(AuthorizationService authorizationService) {
    ...
    for (Authorization activeAuthorization : authorizationService.createAuthorizationQuery()
        .list()) {
  ...
  public void assertAuthorizationException(AuthorizationException e) {
    if (!missingAuthorizations.isEmpty() && e != null) {
      ...
      String assertionFailureMessage = describeScenarioFailure(
          "Expected an authorization exception but the message was wrong: " + e.getMessage());

      List<MissingAuthorization> actualMissingAuthorizations = new ArrayList<MissingAuthorization>(
          e.getMissingAuthorizations());
      List<MissingAuthorization> expectedMissingAuthorizations = MissingAuthorizationMatcher
          .asMissingAuthorizations(missingAuthorizations);

      Assert.assertThat(actualMissingAuthorizations, containsInAnyOrder(
          MissingAuthorizationMatcher.asMatchers(expectedMissingAuthorizations)));

      for (Authorization missingAuthorization : missingAuthorizations) {
        Assert.assertTrue(assertionFailureMessage,
            message.contains(missingAuthorization.getUserId()));
        ...
        if (!Authorization.ANY.equals(missingAuthorization.getResourceId())) {
          ...
          Assert.assertTrue(assertionFailureMessage,
              message.contains(missingAuthorization.getResourceId()));
        }

        Resource resource = AuthorizationTestUtil
            .getResourceByType(missingAuthorization.getResourceType());
      ...
      if (e != null) {
        Assert.fail(describeScenarioFailure(
            "Expected no authorization exception but got one: " + e.getMessage()));
      } else {
        Assert.fail(describeScenarioFailure(
