...
public class ExternalTaskServiceTest extends PluggableProcessEngineTestCase {
    ...
    public void testFailOnMalformedpriorityInput() {
        try {
            repositoryService.createDeployment().addClasspathResource(
                    "org/camunda/bpm/engine/test/api/externaltask/externalTaskInvalidPriority.bpmn20.xml").deploy();
            fail("deploying a process with malformed priority should not succeed");
        } catch (ProcessEngineException e) {
            assertTextPresentIgnoreCase("value 'NOTaNumber' for attribute 'taskPriority' " + "is not a valid number",
                    e.getMessage());
        }
    }
    ...
    public void testFetch() {
        ...
        // when
        List<LockedExternalTask> externalTasks = externalTaskService.fetchAndLock(1, WORKER_ID).topic(TOPIC_NAME,
                LOCK_TIME).execute();
        ...
        ActivityInstance activityInstance = runtimeService.getActivityInstance(
                processInstance.getId()).getActivityInstances("externalTask")[0];
    ...
    @Deployment(
            resources = "org/camunda/bpm/engine/test/api/externaltask/twoExternalTaskWithPriorityProcess.bpmn20.xml")
    public void testFetchWithPriority() {
        // given
        ProcessInstance processInstance = runtimeService.startProcessInstanceByKey(
                "twoExternalTaskWithPriorityProcess");

        // when
        List<LockedExternalTask> externalTasks = externalTaskService.fetchAndLock(1, WORKER_ID, true).topic(TOPIC_NAME,
                LOCK_TIME).execute();
        ...
        ActivityInstance activityInstance = runtimeService.getActivityInstance(
                processInstance.getId()).getActivityInstances("externalTaskWithPrio")[0];
    ...
    public void testFetchProcessWithPriority() {
        ...
        // when
        List<LockedExternalTask> externalTasks = externalTaskService.fetchAndLock(2, WORKER_ID, true).topic(TOPIC_NAME,
                LOCK_TIME).execute();
    ...
    @Deployment(
            resources = "org/camunda/bpm/engine/test/api/externaltask/externalTaskPriorityExpressionProcess.bpmn20.xml")
    ...
    public void testFetchNextWithPriority() {
        ...
        externalTasks = externalTaskService.fetchAndLock(1, "anotherWorkerId", true).topic(TOPIC_NAME,
                LOCK_TIME).execute();
        assertEquals(1, externalTasks.size());
    ...
    public void testFetchTopicSelection() {
        ...
        // when
        List<LockedExternalTask> topic1Tasks = externalTaskService.fetchAndLock(5, WORKER_ID).topic("topic1",
                LOCK_TIME).execute();

        List<LockedExternalTask> topic2Tasks = externalTaskService.fetchAndLock(5, WORKER_ID).topic("topic2",
                LOCK_TIME).execute();
    ...
    public void testFetchLessTasksThanExist() {
        ...
        List<LockedExternalTask> externalTasks = externalTaskService.fetchAndLock(5, WORKER_ID).topic(TOPIC_NAME,
                LOCK_TIME).execute();
    ...
    public void testFetchVariables() {
        ...
        customValue.setTestValue("value3");
        runtimeService.startProcessInstanceByKey("subProcessExternalTask",
                Variables.createVariables().putValue("processVar1", 42).putValue("processVar2", 43).putValue(
                        "processVar3", customValue));

        // when
        List<LockedExternalTask> externalTasks = externalTaskService.fetchAndLock(1, WORKER_ID).topic(TOPIC_NAME,
                LOCK_TIME).variables("processVar1", "subProcessVar", "taskVar", "processVar3").execute();
    ...
    public void testFetchSerializedVariables() {
        ...
        // when
        List<LockedExternalTask> externalTasks = externalTaskService.fetchAndLock(1, WORKER_ID).topic(TOPIC_NAME,
                LOCK_TIME).variables("processVar1", "subProcessVar", "taskVar", "processVar3")
                .deserializeVariables(true).execute();
        ...
        assertEquals(44L, variables.get("subProcessVar"));
        assertEquals(45L, variables.get("taskVar"));
        final ExternalTaskCustomValue receivedCustomValue = (ExternalTaskCustomValue) variables.get("processVar3");
        assertNotNull(receivedCustomValue);
        assertNotNull(receivedCustomValue.getTestValue());
        assertEquals("value3", receivedCustomValue.getTestValue());
    ...
    public void testFetchNonExistingVariable() {
        ...
        // when
        List<LockedExternalTask> tasks = externalTaskService.fetchAndLock(1, WORKER_ID).topic(TOPIC_NAME,
                LOCK_TIME).variables("nonExistingVariable").execute();
    ...
    public void testFetchMultipleTopics() {
        // given a process instance with external tasks for topics "topic1",
        // "topic2", and "topic3"
        runtimeService.startProcessInstanceByKey("parallelExternalTaskProcess");

        // when fetching tasks for two topics
        List<LockedExternalTask> tasks = externalTaskService.fetchAndLock(5, WORKER_ID).topic("topic1",
                LOCK_TIME).topic("topic2", LOCK_TIME * 2).execute();
        ...
        tasks = externalTaskService.fetchAndLock(5, WORKER_ID).topic("topic1", LOCK_TIME).topic("topic2",
                LOCK_TIME * 2).topic("topic3", LOCK_TIME * 3).execute();
    ...
    public void testFetchMultipleTopicsWithVariables() {
        // given a process instance with external tasks for topics "topic1" and
        // "topic2"
        ...
        // when
        List<LockedExternalTask> tasks = externalTaskService.fetchAndLock(5, WORKER_ID).topic("topic1",
                LOCK_TIME).variables("var1", "var2").topic("topic2", LOCK_TIME).variables("var1").execute();
    ...
    @Deployment(
            resources = "org/camunda/bpm/engine/test/api/externaltask/ExternalTaskServiceTest.testFetchMultipleTopics.bpmn20.xml")
    public void testFetchMultipleTopicsMaxTasks() {
        ...
        // when
        List<LockedExternalTask> tasks = externalTaskService.fetchAndLock(5, WORKER_ID).topic("topic1",
                LOCK_TIME).topic("topic2", LOCK_TIME).topic("topic3", LOCK_TIME).execute();
    ...
    /**
     * Note: this does not test a hard API guarantee, i.e. the test is stricter
     * than the API (Javadoc). Its purpose is to ensure that the API
     * implementation is less error-prone to use.
     *
     * Bottom line: if there is good reason to change behavior such that this
     * test breaks, it may be ok to change the test.
     */
    @Deployment(resources = "org/camunda/bpm/engine/test/api/externaltask/oneExternalTaskProcess.bpmn20.xml")
    ...
    public void testComplete() {
        ...
        assertThat(activityInstance).hasStructure(
                describeActivityInstanceTree(processInstance.getProcessDefinitionId()).activity(
                        "afterExternalTask").done());
    }
    ...
    public void testCompleteWithVariables() {
        ...
        externalTaskService.complete(externalTasks.get(0).getId(),
                WORKER_ID,
    ...
    public void testCompleteWithWrongWorkerId() {
        ...
        // then it is not possible to complete the task with a different worker
        // id
        try {
    ...
    public void testCompleteNonExistingTask() {
        try {
            ...
            // not found exception lets client distinguish this from other
            // failures
    ...
    public void testCompleteReclaimedLockExpiredTask() {
        ...
        List<LockedExternalTask> reclaimedTasks = externalTaskService.fetchAndLock(1, "anotherWorkerId").topic(
                TOPIC_NAME, LOCK_TIME).execute();
    ...
    public void testExternalTaskExecutionTreeCompaction() {
        ...
        // when the user task completes meanwhile, thereby trigger execution
        // tree compaction
    ...
    public void testHandleFailure() {
        ...
        List<LockedExternalTask> tasks = externalTaskService.fetchAndLock(5, WORKER_ID).topic(TOPIC_NAME,
                LOCK_TIME).execute();

        LockedExternalTask task = tasks.get(0);

        // when submitting a failure (after a simulated processing time of three
        // seconds)
    ...
    public void testHandleFailureNullErrorMessage() {
        ...
        // then the failure was reported successfully and the error message is
        // null
    ...
    public void testHandleBpmnError() {
        ...
        runtimeService.startProcessInstanceByKey("twoExternalTaskProcess");
        // when
        List<LockedExternalTask> externalTasks = helperHandleBpmnError(1,
                WORKER_ID,
                TOPIC_NAME,
                LOCK_TIME,
                "ERROR-OCCURED");
    ...
    /**
     * Helper method to handle a bmpn error on an external task, which is
     * fetched with the given parameters.
     *
     * @param taskCount
     ...
     * @return returns the locked external tasks after the bpmn error was
     *         handled
     */
    public List<LockedExternalTask> helperHandleBpmnError(int taskCount, String workerID, String topicName,
        ...
        // when
        List<LockedExternalTask> externalTasks = externalTaskService.fetchAndLock(taskCount, workerID).topic(topicName,
                lockTime).execute();
    ...
    public void handleBpmnErrorReclaimedLockExpiredTask() {
        ...
        try {
            ...
            assertTextPresent("Bpmn error of External Task " + externalTasks.get(0).getId()
                    + " cannot be reported by worker '" + WORKER_ID + "'. It is locked by worker 'anotherWorkerId'.",
                    e.getMessage());
        }
