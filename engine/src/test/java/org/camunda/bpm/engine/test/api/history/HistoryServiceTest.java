...
public class HistoryServiceTest extends PluggableProcessEngineTestCase {
  ...
  public void testHistoricProcessInstanceQueryOrderBy() {
    ...
    historyService.createHistoricTaskInstanceQuery().orderByHistoricActivityInstanceId().asc()
        .list();
    historyService.createHistoricTaskInstanceQuery().orderByHistoricActivityInstanceStartTime()
        .asc().list();
  ...
  public void testOrderProcessWithCallActivity() {
    ...
    // verify
    HistoricProcessInstance historicProcessInstance = historyService
        .createHistoricProcessInstanceQuery().superProcessInstanceId(pi.getId()).singleResult();
  ...
  public void testHistoricProcessInstanceQueryByProcessDefinitionKey() {
    ...
    HistoricProcessInstance historicProcessInstanceSub = historyService
        .createHistoricProcessInstanceQuery().processDefinitionKey("checkCreditProcess")
        .singleResult();
    HistoricProcessInstance historicProcessInstanceSuper = historyService
        .createHistoricProcessInstanceQuery().processDefinitionKey("orderProcess").singleResult();
    assertEquals(historicProcessInstanceSuper.getId(),
        historicProcessInstanceSub.getSuperProcessInstanceId());
  }
  ...
  public void testHistoricProcessInstanceQueryByProcessInstanceIds() {
    ...
    for (int i = 0; i < 4; i++) {
      processInstanceIds
          .add(runtimeService.startProcessInstanceByKey(ONE_TASK_PROCESS, i + "").getId());
    }
    processInstanceIds
        .add(runtimeService.startProcessInstanceByKey("otherOneTaskProcess", "1").getId());
    ...
    HistoricProcessInstanceQuery processInstanceQuery = historyService
        .createHistoricProcessInstanceQuery().processInstanceIds(processInstanceIds);
  ...
  public void testQueryByRootProcessInstances() {
    ...
    String processInstanceId1 = runtimeService.startProcessInstanceByKey(superProcess)
  ...
  public void testQueryByRootProcessInstancesAndSuperProcess() {
    ...
    try {
      historyService.createHistoricProcessInstanceQuery()
          .superProcessInstanceId("processInstanceId").rootProcessInstances();
  ...
  public void testHistoricVariableInstancesOnParallelExecution() {
    ...
    for (Task task : tasks) {
      ...
      LOG.debug(
          "setting variables on task " + task.getId() + ", execution " + task.getExecutionId());
    ...
    assertEquals(1, historyService.createHistoricProcessInstanceQuery()
        .variableValueEquals("rootValue", "test").count());

    assertEquals(1, historyService.createHistoricProcessInstanceQuery()
        .variableValueEquals("parallelValue1", "Receive Payment").count());
    assertEquals(1, historyService.createHistoricProcessInstanceQuery()
        .variableValueEquals("parallelValue1", "Ship Order").count());
    assertEquals(1, historyService.createHistoricProcessInstanceQuery()
        .variableValueEquals("parallelValue2", "test").count());
  }
  ...
  public void testQueryStringVariable() {
    ...
    ProcessInstance processInstance1 = runtimeService.startProcessInstanceByKey(ONE_TASK_PROCESS,
        vars);
    taskService.complete(taskService.createTaskQuery().processInstanceId(processInstance1.getId())
        .singleResult().getId());
    ...
    ProcessInstance processInstance2 = runtimeService.startProcessInstanceByKey(ONE_TASK_PROCESS,
        vars);
    taskService.complete(taskService.createTaskQuery().processInstanceId(processInstance2.getId())
        .singleResult().getId());

    vars = new HashMap<String, Object>();
    vars.put("stringVar", "azerty");
    ProcessInstance processInstance3 = runtimeService.startProcessInstanceByKey(ONE_TASK_PROCESS,
        vars);
    taskService.complete(taskService.createTaskQuery().processInstanceId(processInstance3.getId())
        .singleResult().getId());
    ...
    assertEquals(3, historyService.createHistoricProcessInstanceQuery()
        .variableValueLike("stringVar", "a%").count());
    assertEquals(0, historyService.createHistoricProcessInstanceQuery()
        .variableValueLike("stringVar", "%x%").count());
   ...
   * not want to duplicate all test case logic here. Basically copied from
  ...
  public void testQueryDateVariable() throws Exception {
    ...
    Assert.assertEquals(0, historyService.createHistoricProcessInstanceQuery()
        .variableValueGreaterThan("dateVar", nextYear.getTime()).count());
    Assert.assertEquals(3, historyService.createHistoricProcessInstanceQuery()
  ...
  public void testNativeHistoricVariableInstanceTest() {
    ...
    assertEquals(2, historyService.createNativeHistoricVariableInstanceQuery().sql(
        "SELECT count(*) FROM " + managementService.getTableName(HistoricVariableInstance.class))
        .count());
    assertEquals(1,
  ...
  public void testDeleteProcessInstance() {
    ProcessInstance processInstance = runtimeService.startProcessInstanceByKey(ONE_TASK_PROCESS);
    assertEquals(1,
        runtimeService.createProcessInstanceQuery().processDefinitionKey(ONE_TASK_PROCESS).count());
  ...
  public void testDeleteProcessInstancesWithFake() {
    ...
    try {
      ...
      Assert.assertThat(e.getMessage(),
          CoreMatchers.containsString("No historic process instance found with id: [aFake]"));
    }
  ...
  public void testDeleteHistoricVariableAndDetails() {
    ...
    HistoricVariableInstanceQuery histVariableQuery = historyService
        .createHistoricVariableInstanceQuery().processInstanceId(executionId)
        .variableName("myVariable");
    HistoricVariableInstanceQuery secondHistVariableQuery = historyService
        .createHistoricVariableInstanceQuery().processInstanceId(executionId)
    ...
    HistoricDetailQuery detailsQuery = historyService.createHistoricDetailQuery()
        .processInstanceId(executionId).variableInstanceId(variableInstanceId);
    HistoricDetailQuery secondDetailsQuery = historyService.createHistoricDetailQuery()
        .processInstanceId(executionId).variableInstanceId(secondVariableInstanceId);
  ...
  public void testDeleteHistoricVariableAndDetailsOnRunningInstance() {
    ...
    VariableInstanceQuery variableQuery = runtimeService.createVariableInstanceQuery()
        .processInstanceIdIn(executionId).variableName("myVariable");
  ...
  public void testDeleteHistoricVariableAndDetailsFromCase() {
    ...
    HistoricVariableInstance variableInstance = historyService.createHistoricVariableInstanceQuery()
        .singleResult();
    HistoricDetailQuery detailsQuery = historyService.createHistoricDetailQuery()
        .caseInstanceId(caseInstanceId).variableInstanceId(variableInstance.getId());
  ...
  public void testDeleteHistoricVariableAndDetailsFromStandaloneTask() {
    ...
    HistoricVariableInstance variableInstance = historyService.createHistoricVariableInstanceQuery()
        .singleResult();
    HistoricDetailQuery detailsQuery = historyService.createHistoricDetailQuery()
        .taskId(task.getId()).variableInstanceId(variableInstance.getId());
    assertEquals(1, historyService.createHistoricVariableInstanceQuery().count());
  ...
  public void testDeleteAllHistoricVariablesAndDetailsOnRunningInstance() {
    ...
    VariableInstanceQuery variableQuery = runtimeService.createVariableInstanceQuery()
        .processInstanceIdIn(executionId).variableName("myVariable");
    VariableInstanceQuery secondVariableQuery = runtimeService.createVariableInstanceQuery()
        .processInstanceIdIn(executionId).variableName("mySecondVariable");
    ...
    // then
    HistoricVariableInstanceQuery allHistVariableQuery = historyService
        .createHistoricVariableInstanceQuery().processInstanceId(executionId);
    HistoricDetailQuery allDetailsQuery = historyService.createHistoricDetailQuery()
  ...
  public void testDeleteAllHistoricVariablesAndDetailsOnRunningInstanceAndSetAgain() {
    ...
    HistoricDetailQuery detailsQuery = historyService.createHistoricDetailQuery()
        .processInstanceId(executionId)
        .variableInstanceId(histVariableQuery.singleResult().getId());
    HistoricDetailQuery secondDetailsQuery = historyService.createHistoricDetailQuery()
        .processInstanceId(executionId)
        .variableInstanceId(secondHistVariableQuery.singleResult().getId());
    assertEquals(1L, histVariableQuery.count());
    assertEquals(1L, secondHistVariableQuery.count());
  ...
  protected List<String> prepareHistoricProcesses() {
    ...
    List<String> processInstanceIds = new ArrayList<String>(
        Arrays.asList(new String[] { processInstance.getId(), processInstance2.getId() }));
