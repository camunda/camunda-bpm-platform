...
/**
 * To create a testcase, that tests the write a Member/Group/Membership in on step, an entry Point
 * into the write option within the same Command Context is needed. This is done by extending the
 * to-test class and overriding a not in scope Method. This Method will trigger the write of
 * Member/Group/Membership in one step. <br>
 * <br>
 * The Group will be the userId extended by _group <br>
 * <br>
 * The checkPassword method must return true, because exactly the requested user with the requested
 * Password will be created within this Method
 *
