...
public class MetricsIntervalTest extends AbstractMetricsIntervalTest {
  ...
  public void testMeterQueryLimit() {
    // since generating test data of 200 metrics will take a long time we check if the default
    // values are set of the query
    // given metric query
  ...
  public void testMeterQueryOffset() {
    ...
    List<MetricIntervalValue> metrics = managementService.createMetricsQuery().offset(metricsCount)
        .interval();

    // then 2 * metricsCount values are returned and highest interval is second last interval, since
    // first 9 was skipped
    assertEquals(2 * metricsCount, metrics.size());
    assertEquals(firstInterval.plusMinutes(DEFAULT_INTERVAL).getMillis(),
        metrics.get(0).getTimestamp().getTime());
  }
  ...
  public void testMeterQueryMaxOffset() {
    ...
    List<MetricIntervalValue> metrics = managementService.createMetricsQuery()
        .offset(Integer.MAX_VALUE).interval();
  ...
  public void testMeterQueryDefaultIntervalWhereReporter() {
    ...
    List<MetricIntervalValue> metrics = managementService.createMetricsQuery().reporter(REPORTER_ID)
        .interval();

    // then result contains only metrics from given reporter, since it is the default it contains
    // all
  ...
  public void testMeterQueryDefaultIntervalWhereReporterNotExist() {
    ...
    List<MetricIntervalValue> metrics = managementService.createMetricsQuery().reporter("notExist")
        .interval();
  ...
  public void testMeterQueryCustomIntervalWhereReporterNotExist() {
    ...
    // when query metric interval data with custom interval and non existing reporter in where
    // clause
    List<MetricIntervalValue> metrics = managementService.createMetricsQuery().reporter("notExist")
        .interval(300);
  ...
  public void testMeterQueryDefaultIntervalWhereName() {
    ...
    List<MetricIntervalValue> metrics = managementService.createMetricsQuery()
        .name(ACTIVTY_INSTANCE_START).interval();
  ...
  public void testMeterQueryDefaultIntervalWhereNameNotExist() {
    ...
    List<MetricIntervalValue> metrics = managementService.createMetricsQuery().name("notExist")
        .interval();
  ...
  public void testMeterQueryCustomIntervalWhereName() {
    ...
    List<MetricIntervalValue> metrics = managementService.createMetricsQuery()
        .name(ACTIVTY_INSTANCE_START).interval(interval);
  ...
  public void testMeterQueryDefaultIntervalWhereStartDate() {
    ...
    List<MetricIntervalValue> metrics = managementService.createMetricsQuery().startDate(startDate)
        .interval();
  ...
  public void testMeterQueryCustomIntervalWhereStartDate() {
    ...
    // when query metric interval data with custom interval and second last interval as start date
    // in where clause
    Date startDate = firstInterval.plusMinutes(DEFAULT_INTERVAL).toDate();
    List<MetricIntervalValue> metrics = managementService.createMetricsQuery().startDate(startDate)
        .interval(300);
  ...
  public void testMeterQueryDefaultIntervalWhereEndDate() {
    ...
    List<MetricIntervalValue> metrics = managementService.createMetricsQuery()
        .endDate(endDate.toDate()).interval();
  ...
  public void testMeterQueryCustomIntervalWhereEndDate() {
    ...
    // when query metric interval data with custom interval and second interval as end date in where
    // clause
    Date endDate = firstInterval.plusMinutes(DEFAULT_INTERVAL).toDate();
    List<MetricIntervalValue> metrics = managementService.createMetricsQuery().endDate(endDate)
        .interval(300);
  ...
  public void testMeterQueryDefaultIntervalWhereStartAndEndDate() {
    ...
    List<MetricIntervalValue> metrics = managementService.createMetricsQuery()
        .startDate(startDate.toDate()).endDate(endDate.toDate()).interval();
  ...
  public void testMeterQueryCustomIntervalWhereStartAndEndDate() {
    ...
    DateTime startDate = firstInterval;
    List<MetricIntervalValue> metrics = managementService.createMetricsQuery()
        .startDate(startDate.toDate()).endDate(endDate.toDate()).interval(300);
  ...
  public void testMeterQueryDefaultIntervalCalculatedValue() {
    ...
    DateTime endDate = firstInterval.plusMinutes(DEFAULT_INTERVAL);
    DateTime startDate = firstInterval;
    MetricsQuery metricQuery = managementService.createMetricsQuery().startDate(startDate.toDate())
        .endDate(endDate.toDate()).name(ACTIVTY_INSTANCE_START);
  ...
  public void testMeterQueryNotLoggedInterval() {
    // given metric data
    List<MetricIntervalValue> metrics = managementService.createMetricsQuery()
        .name(ACTIVTY_INSTANCE_START).limit(1).interval();
    ...
    // on query with name
    metrics = managementService.createMetricsQuery().name(ACTIVTY_INSTANCE_START).limit(1)
        .interval();
  ...
  public void testIntervallQueryWithGeneratedDataAfterSomeTime() {
    ...
    assertEquals(newMetrics.get(0).getTimestamp().getTime(),
        metrics.get(0).getTimestamp().getTime() + DEFAULT_INTERVAL_MILLIS);
  }
  ...
  public void testIntervallQueryWithGeneratedDataAfterSomeTimeForSpecificMetric() {
    // given metric data and result of interval query
    List<MetricIntervalValue> metrics = managementService.createMetricsQuery()
        .name(ACTIVTY_INSTANCE_START).startDate(new Date(0))
  ...
  public void testMetricQueryAggregatedByReporterSingleReporter() {
    ...
    // when
    List<MetricIntervalValue> aggregatedMetrics = managementService.createMetricsQuery()
        .aggregateByReporter().interval();
  ...
  public void testMetricQueryAggregatedByReporterThreeReporters() {
    ...
    // then
    // multiply by 3 because there are three reporters: 'REPORTER_ID' (check the #initMetrics()),
    // reporter1 and reporter2
  ...
  public void testMetricQueryAggregatedByReporterLimitAndTwoReporters() {
    ...
    // when
    List<MetricIntervalValue> metrics = managementService.createMetricsQuery()
        .name(Metrics.ACTIVTY_INSTANCE_START).limit(limit).interval();
    List<MetricIntervalValue> aggregatedMetrics = managementService.createMetricsQuery()
        .name(Metrics.ACTIVTY_INSTANCE_START).limit(limit).aggregateByReporter().interval();

    // then aggregatedMetrics contains wider time interval
    assertTrue(metrics.get(limit - 1).getTimestamp().getTime() > aggregatedMetrics.get(limit - 1)
