...
public class TelemetryReporterTest {
  ...
  public void shouldSendCompleteTelemetryData() {
    ...
    // setup
    TelemetryDataImpl initialTelemetryData = (TelemetryDataImpl) configuration.getManagementService().getTelemetryData(null, null);
  ...
  public void shouldResetDataCollectionTimeFrameAfterSending() {
    ...
    verify(postRequestedFor(urlEqualTo(TELEMETRY_ENDPOINT_PATH))
        .withHeader("Content-Type",  equalTo("application/json")));
    assertThat(managementService.getTelemetryData(null, null).getProduct().getInternals().getDataCollectionStartDate())
