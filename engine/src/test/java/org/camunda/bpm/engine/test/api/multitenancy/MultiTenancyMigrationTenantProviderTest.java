...
public class MultiTenancyMigrationTenantProviderTest {
  ...
  public void cannotMigrateInstanceBetweenDifferentTenants() {
    ...
    ProcessDefinition tenantDefinition = testHelper.deployForTenantAndGetDefinition(TENANT_TWO,
  ...
  public void canMigrateInstanceBetweenSameTenantCase2() {
    ...
    ProcessInstance processInstance = startInstanceForTenant(sharedDefinition, TENANT_ONE);
    MigrationPlan migrationPlan = engineRule.getRuntimeService()
        .createMigrationPlan(sharedDefinition.getId(), targetDefinition.getId())
        .mapEqualActivities().build();
  ...
  public void canMigrateWithProcessInstanceQueryAllInstancesOfAuthenticatedTenants() {
    ...
    // when
    engineRule.getIdentityService().setAuthentication("user", null,
        Arrays.asList(TENANT_ONE, TENANT_TWO));
    engineRule.getRuntimeService().newMigration(migrationPlan)
        .processInstanceQuery(engineRule.getRuntimeService().createProcessInstanceQuery())
        .execute();
    engineRule.getIdentityService().clearAuthentication();
  ...
  protected void assertInstanceOfDefinition(ProcessInstance processInstance,
  ...
  protected ProcessInstance startInstanceForTenant(ProcessDefinition processDefinition,
      String tenantId) {
    return engineRule.getRuntimeService().startProcessInstanceById(processDefinition.getId(),
        Variables.createVariables().putValue(VariableBasedTenantIdProvider.TENANT_VARIABLE,
            tenantId));
  }
