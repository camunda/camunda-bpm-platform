...
public class MultiTenancySharedDeploymentStatisticsQueryTest {
  ...
  protected static final BpmnModelInstance oneTaskProcess = Bpmn
      .createExecutableProcess(ONE_TASK_PROCESS_DEFINITION_KEY).startEvent().userTask().done();

  protected static final BpmnModelInstance failingProcess = Bpmn
      .createExecutableProcess(FAILED_JOBS_PROCESS_DEFINITION_KEY).startEvent().serviceTask()
      .camundaClass("org.camunda.bpm.engine.test.api.multitenancy.FailingDelegate")
      .camundaAsyncBefore().done();

  protected static final BpmnModelInstance anotherFailingProcess = Bpmn
      .createExecutableProcess(ANOTHER_FAILED_JOBS_PROCESS_DEFINITION_KEY).startEvent()
      .serviceTask().camundaClass("org.camunda.bpm.engine.test.api.multitenancy.FailingDelegate")
      .camundaAsyncBefore().done();
  ...
  public void activeProcessInstancesCountWithNoAuthenticatedTenant() {
    ...
    List<DeploymentStatistics> deploymentStatistics = managementService
        .createDeploymentStatisticsQuery().list();
  ...
  public void failedJobsCountWithWithNoAuthenticatedTenant() {
    ...
    List<DeploymentStatistics> deploymentStatistics = managementService
        .createDeploymentStatisticsQuery().includeFailedJobs().list();
  ...
  public void incidentsCountWithNoAuthenticatedTenant() {
    ...
    List<DeploymentStatistics> deploymentStatistics = managementService
        .createDeploymentStatisticsQuery().includeIncidents().list();

    // then
    assertEquals(1, deploymentStatistics.size());

    List<IncidentStatistics> incidentStatistics = deploymentStatistics.get(0)
  ...
  public void incidentsCountWithIncidentTypeAndAuthenticatedTenant() {
    ...
    List<DeploymentStatistics> deploymentStatistics = managementService
        .createDeploymentStatisticsQuery().includeIncidentsForType("failedJob").list();
  ...
  public void instancesFailedJobsAndIncidentsCountWithAuthenticatedTenant() {
    ...
    List<DeploymentStatistics> deploymentStatistics = managementService
        .createDeploymentStatisticsQuery().includeFailedJobs().includeIncidents().list();
    ...
    List<IncidentStatistics> incidentStatistics = singleDeploymentStatistics
        .getIncidentStatistics();
    assertEquals(1, incidentStatistics.size());
