...
public class MultiTenancyExternalTaskCmdsTenantCheckTest {
  ...
  public void testFetchAndLockWithTenantId() {
    ...
    // when
    List<LockedExternalTask> externalTasks = externalTaskService.fetchAndLock(1, WORKER_ID)
        .topic(TOPIC_NAME, LOCK_TIME).tenantIdIn(TENANT_ONE).execute();
  ...
  public void testFetchAndLockWithTenantIdIn() {
    ...
    // when
    List<LockedExternalTask> externalTasks = externalTaskService.fetchAndLock(1, WORKER_ID)
        .topic(TOPIC_NAME, LOCK_TIME).tenantIdIn(TENANT_ONE, TENANT_TWO).execute();
  ...
  public void testFetchAndLockWithTenantIdInTwoTenants() {
    ...
    engineRule.getRuntimeService().startProcessInstanceByKey("twoExternalTaskWithPriorityProcess")
        .getId();
    testRule.deployForTenant(TENANT_TWO,
        "org/camunda/bpm/engine/test/api/externaltask/oneExternalTaskProcess.bpmn20.xml");
    String instanceId = engineRule.getRuntimeService()
        .startProcessInstanceByKey(PROCESS_DEFINITION_KEY_ONE).getId();
  ...
  public void testCompleteWithAuthenticatedTenant() {
    ...
    String externalTaskId = externalTaskService.fetchAndLock(1, WORKER_ID)
        .topic(TOPIC_NAME, LOCK_TIME).execute().get(0).getId();
  ...
  public void testCompleteWithNoAuthenticatedTenant() {
    ...
    thrown.expectMessage("Cannot update the process instance '" + processInstanceId
  ...
  public void testHandleFailureWithAuthenticatedTenant() {
    ...
    LockedExternalTask task = externalTaskService.fetchAndLock(1, WORKER_ID)
        .topic(TOPIC_NAME, LOCK_TIME).execute().get(0);
    ...
    assertEquals(ERROR_MESSAGE, externalTaskService.fetchAndLock(1, WORKER_ID)
        .topic(TOPIC_NAME, LOCK_TIME).execute().get(0).getErrorMessage());
  ...
  public void testHandleFailureWithDisabledTenantCheck() {
    ...
    String taskId = externalTaskService.fetchAndLock(1, WORKER_ID).topic(TOPIC_NAME, LOCK_TIME)
        .execute().get(0).getId();
  ...
  public void testHandleBPMNErrorWithAuthenticatedTenant() {
    ...
    // then
    assertEquals(taskService.createTaskQuery().singleResult().getTaskDefinitionKey(),
        "afterBpmnError");
  }
  ...
  public void testSetRetriesWithAuthenticatedTenant() {
    ...
    // then
    assertEquals(5,
        (int) externalTaskService.createExternalTaskQuery().singleResult().getRetries());
  }
  ...
  public void testSetPriorityWithAuthenticatedTenant() {
    ...
    // then
    assertEquals(1,
        (int) externalTaskService.createExternalTaskQuery().singleResult().getPriority());
  }
  ...
  public void testGetErrorDetailsWithAuthenticatedTenant() {
    ...
    externalTaskService.handleFailure(externalTaskId, WORKER_ID, ERROR_MESSAGE, ERROR_DETAILS, 1,
        1000L);
  ...
  public void testGetErrorDetailsWithNoAuthenticatedTenant() {
    ...
    thrown.expect(ProcessEngineException.class);
    thrown.expectMessage("Cannot read the process instance '" + processInstanceId
        + "' because it belongs to no authenticated tenant.");
    // when
