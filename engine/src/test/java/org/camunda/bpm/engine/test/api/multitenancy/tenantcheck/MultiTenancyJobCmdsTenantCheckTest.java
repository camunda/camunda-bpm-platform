...
public class MultiTenancyJobCmdsTenantCheckTest {
  ...
  protected static final BpmnModelInstance BPMN_PROCESS = Bpmn
      .createExecutableProcess("exceptionInJobExecution").startEvent().userTask("aUserTask")
      .boundaryEvent("timerEvent").timerWithDuration("PT4H").serviceTask()
      .camundaExpression("${failing}").endEvent().done();

  BpmnModelInstance BPMN_NO_FAIL_PROCESS = Bpmn.createExecutableProcess("noFail").startEvent()
      .userTask("aUserTask").boundaryEvent("timerEvent").timerWithDuration("PT4H").endEvent()
      .done();
  ...
  public void testSetJobRetriesWithAuthenticatedTenant() {
    ...
    assertEquals(5, managementService.createJobQuery().processInstanceId(processInstance.getId())
        .singleResult().getRetries());
  }
  ...
  public void testSetJobRetriesDefinitionWithAuthenticatedTenant() {
    ...
    identityService.setAuthentication("aUserId", null, Arrays.asList(TENANT_ONE));
    // sets the retries for failed jobs - That's the reason why job retries are made 0 in the above
    // step
  ...
  public void testSetJobRetriesDefinitionWithNoAuthenticatedTenant() {
    ...
    thrown.expectMessage(
        "Cannot update the process definition '" + jobDefinition.getProcessDefinitionId()
            + "' because it belongs to no authenticated tenant.");
    // when
    managementService.setJobRetriesByJobDefinitionId(jobDefinition.getId(), 1);
  }
  ...
  public void testSetJobDueDateWithAuthenticatedTenant() {
    ...
    // then
    assertEquals(1, managementService.createJobQuery().duedateLowerThan(new Date()).count());
  }
  ...
  public void testSetOverridingJobPriorityWithAuthenticatedTenant() {
    ...
    // then
    assertThat(managementService.createJobDefinitionQuery().jobDefinitionId(jobDefinition.getId())
        .singleResult().getOverridingJobPriority(), is(1701L));
  }
  ...
  public void testClearOverridingJobPriorityWithAuthenticatedTenant() {
    ...
    // then
    assertThat(managementService.createJobDefinitionQuery().jobDefinitionId(jobDefinition.getId())
        .singleResult().getOverridingJobPriority(), nullValue());
  ...
  public void testGetJobExceptionStackTraceWithAuthenticatedTenant() {
    ...
    String processInstanceId = engineRule.getRuntimeService()
        .startProcessInstanceByKey(PROCESS_DEFINITION_KEY).getId();

    testRule.executeAvailableJobs();

    String timerJobId = managementService.createJobQuery().processInstanceId(processInstanceId)
        .singleResult().getId();
  ...
  public void testGetJobExceptionStackTraceWithNoAuthenticatedTenant() {
    ...
    thrown.expect(ProcessEngineException.class);
    thrown.expectMessage(
        "Cannot read the job '" + timerJobId + "' because it belongs to no authenticated tenant.");
  ...
  public void testExecuteJobWithAuthenticatedTenant() {
    ...
    String noFailProcessInstanceId = engineRule.getRuntimeService()
        .startProcessInstanceByKey("noFail").getId();
    ...
    String timerJobId = managementService.createJobQuery()
        .processInstanceId(noFailProcessInstanceId).singleResult().getId();
