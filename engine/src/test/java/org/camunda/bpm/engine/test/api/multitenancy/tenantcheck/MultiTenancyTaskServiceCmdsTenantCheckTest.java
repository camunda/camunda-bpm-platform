...
public class MultiTenancyTaskServiceCmdsTenantCheckTest {
  ...
  public void saveTaskWithNoAuthenticatedTenant() {
    ...
    thrown.expectMessage("Cannot create the task '" + task.getId()
  ...
  public void updateTaskWithNoAuthenticatedTenant() {
    ...
    // then
    thrown.expect(ProcessEngineException.class);
    thrown.expectMessage("Cannot assign the task '" + task.getId()
        + "' because it belongs to no authenticated tenant.");
    taskService.saveTask(task);
  ...
  public void claimTaskWithNoAuthenticatedTenant() {
    ...
    // then
    thrown.expect(ProcessEngineException.class);
    thrown.expectMessage("Cannot work on task '" + task.getId()
        + "' because it belongs to no authenticated tenant.");
    taskService.claim(task.getId(), "bUser");
  ...
  public void resolveTaskWithAuthenticatedTenant() {
    ...
    assertThat(taskService.createTaskQuery().taskDelegationState(DelegationState.RESOLVED)
        .taskId(task.getId()).count(), is(1L));
  }
  ...
  public void deleteTaskWithNoAuthenticatedTenant() {
    ...
    try {
      ...
      // then
      thrown.expect(ProcessEngineException.class);
      thrown.expectMessage("Cannot delete the task '" + task.getId()
          + "' because it belongs to no authenticated tenant.");
      taskService.deleteTask(task.getId(), true);
    } finally {
