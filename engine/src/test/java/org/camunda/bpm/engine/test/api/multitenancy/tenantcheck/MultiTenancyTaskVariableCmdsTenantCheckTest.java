...
public class MultiTenancyTaskVariableCmdsTenantCheckTest {
  ...
  public void init() {
    ...
    engineRule.getRuntimeService()
        .startProcessInstanceByKey(PROCESS_DEFINITION_KEY, Variables.createVariables()
            .putValue(VARIABLE_1, VARIABLE_VALUE_1).putValue(VARIABLE_2, VARIABLE_VALUE_2))
        .getId();
  ...
  public void setTaskVariableWithNoAuthenticatedTenant() {
    ...
    thrown.expect(ProcessEngineException.class);
    thrown.expectMessage(
        "Cannot update the task '" + taskId + "' because it belongs to no authenticated tenant.");
    engineRule.getTaskService().setVariable(taskId, "newVariable", "newValue");
