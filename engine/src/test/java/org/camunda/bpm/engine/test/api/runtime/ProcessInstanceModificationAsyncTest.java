...
public class ProcessInstanceModificationAsyncTest extends PluggableProcessEngineTestCase {
  ...
  public void testStartBeforeAsync() {
    ...
    assertThat(updatedTree)
        .hasStructure(describeActivityInstanceTree(processInstance.getProcessDefinitionId())
            .activity("task1").transition("task2").done());
  ...
  public void testStartAfterAsync() {
    ...
    runtimeService.createProcessInstanceModification(processInstance.getId())
        .startAfterActivity("task2").execute();

    // there is now a job for the end event after task2
    Job job = managementService.createJobQuery().singleResult();
    assertNotNull(job);

    Execution jobExecution = runtimeService.createExecutionQuery().activityId("end2")
  ...
  public void testCancelParentScopeOfParallelAsyncBeforeScopeActivity() {
    ...
    ProcessInstance processInstance = runtimeService
        .startProcessInstanceByKey("nestedConcurrentTasksProcess");
  ...
  public void testCancelAsyncBeforeTransitionInstanceEndsProcessInstance() {
    ...
    runtimeService.createProcessInstanceModification(processInstanceId).cancelTransitionInstance(
        getChildTransitionInstanceForTargetActivity(tree, "theTask").getId()).execute();
  ...
  public void testCancelNestedConcurrentTransitionInstance() {
    ...
    assertThat(updatedTree)
        .hasStructure(describeActivityInstanceTree(processInstance.getProcessDefinitionId())
            .activity("outerTask").beginScope("subProcess").transition("innerTask2").done());
    ...
    Execution innerTask2Execution = runtimeService.createExecutionQuery().activityId("innerTask2")
        .singleResult();
  ...
  public void testCancelAsyncAfterEndEventTransitionInstance() {
    ...
    ProcessInstance processInstance = runtimeService
        .startProcessInstanceByKey("nestedAsyncEndEventProcess");
  ...
  public void testCancelAsyncAfterTransitionInstanceInvokesParentListeners() {
    ...
    runtimeService.createProcessInstanceModification(processInstanceId).cancelTransitionInstance(
        getChildTransitionInstanceForTargetActivity(tree, "subProcessEnd").getId()).execute();
  ...
  public void testCancelAllCancelsTransitionInstances() {
    ...
    runtimeService.createProcessInstanceModification(processInstance.getId())
        .cancelAllForActivity("innerTask").execute();
  ...
  public void testStartBeforeAsyncAfterTask() {
    // given a process instance with an async task in a subprocess
    ProcessInstance processInstance = runtimeService
        .startProcessInstanceByKey("failingAfterAsyncTask");
    ...
    assertThat(tree)
        .hasStructure(describeActivityInstanceTree(processInstance.getProcessDefinitionId())
            .transition("task1").transition("task1").done());
  ...
  public void testStartBeforeAsyncAfterTaskActivityStatistics() {
    ...
    List<ActivityStatistics> statistics = managementService
        .createActivityStatisticsQuery(processInstance.getProcessDefinitionId()).includeFailedJobs()
  ...
  public void testCancelAllTransitionInstanceInScope() {
    ...
    ProcessInstance instance = runtimeService.createProcessInstanceByKey("nestedOneTaskProcess")
        .startBeforeActivity("innerTask").startBeforeActivity("innerTask")
        .startBeforeActivity("outerTask").execute();
    ...
    runtimeService.createProcessInstanceModification(instance.getId())
        ...
        .cancelTransitionInstance(transitionInstances[1].getId()).execute();
  ...
  public void testCancelTransitionInstanceTwiceFails() {
    ...
    // and an active activity instance in an outer scope
    ProcessInstance instance = runtimeService.createProcessInstanceByKey("nestedOneTaskProcess")
        .startBeforeActivity("innerTask").startBeforeActivity("innerTask").execute();
    ...
    try {
      runtimeService.createProcessInstanceModification(instance.getId())
          ...
          .cancelTransitionInstance(transitionInstanceId).execute();
      fail("should not be possible to cancel the first instance twice");
    } catch (NotValidException e) {
      assertTextPresentIgnoreCase("Cannot perform instruction: Cancel transition instance '"
          + transitionInstanceId + "'; Transition instance '" + transitionInstanceId
          + "' does not exist: transitionInstance is null", e.getMessage());
    }
  }
    ...
    try {
      runtimeService.createProcessInstanceModification(instance.getId())
          .cancelTransitionInstance(transitionInstances[0].getId()) // compacts the tree;
                                                                    // => execution for
                                                                    // transitionInstances[1] is
                                                                    // replaced by scope execution
          .startBeforeActivity("innerTask") // expand tree again
                                            // => scope execution is replaced by a new concurrent
                                            // execution
          .startBeforeActivity("innerTask").cancelTransitionInstance(transitionInstances[1].getId()) // does
                                                                                                     // not
                                                                                                     // trigger
                                                                                                     // compaction
          .cancelTransitionInstance(transitionInstances[1].getId()) // should fail
                                                                    // => execution for
                                                                    // transitionInstances[1] should
                                                                    // no longer have a replacedBy
                                                                    // link
          .execute();
      fail("should not be possible to cancel the first instance twice");
    } catch (NotValidException e) {
  ...
  public void testCancelStartCancelInScope() {
    ...
    ProcessInstance instance = runtimeService
        .createProcessInstanceByKey("nestedConcurrentTasksProcess")
        .startBeforeActivity("innerTask1").startBeforeActivity("innerTask1")
        .startBeforeActivity("outerTask").execute();
  ...
  public void testStartAndCancelAllForTransitionInstance() {
    ...
    runtimeService.createProcessInstanceModification(instance.getId())
        .startBeforeActivity("innerTask2").cancelAllForActivity("innerTask1").execute();

    // then the activity was successfully instantiated
    ActivityInstance tree = runtimeService.getActivityInstance(instance.getId());

    assertThat(tree).hasStructure(describeActivityInstanceTree(instance.getProcessDefinitionId())
        .beginScope("subProcess").transition("innerTask2").done());

    // assert executions
    ExecutionTree executionTree = ExecutionTree.forExecution(instance.getId(), processEngine);

    assertThat(executionTree)
        .matches(describeExecutionTree(null).scope().child("innerTask2").scope().done());
  }
  ...
  public void testRepeatedCancellationAndStartForTransitionInstance() {
    ...
    ProcessInstance instance = runtimeService
        .createProcessInstanceByKey("nestedConcurrentTasksProcess")
        .startBeforeActivity("innerTask1").startBeforeActivity("innerTask1").execute();
  ...
  public void testStartBeforeAndCancelSingleTransitionInstance() {
    ...
    // and cancel the first transition instance
    runtimeService.createProcessInstanceModification(instance.getId())
        .startBeforeActivity("innerTask2").cancelTransitionInstance(transitionInstance.getId())
        .execute();
  ...
  public void testStartBeforeSyncEndAndCancelSingleTransitionInstance() {
    ...
    ProcessInstance instance = runtimeService
        .createProcessInstanceByKey("nestedConcurrentTasksProcess").startBeforeActivity("outerTask")
        .startBeforeActivity("innerTask1").execute();
    ...
    // and cancel the first transition instance
    runtimeService.createProcessInstanceModification(instance.getId())
        .startBeforeActivity("subProcessEnd2").cancelTransitionInstance(transitionInstance.getId())
        .execute();
  ...
  public void testRestartAFailedServiceTask() {
    ...
    ActivityInstance tree = runtimeService.getActivityInstance(instance.getId());
    runtimeService.createProcessInstanceModification(instance.getId()).startBeforeActivity("task2")
        .cancelTransitionInstance(tree.getTransitionInstances("task2")[0].getId()).execute();
  ...
  protected TransitionInstance getChildTransitionInstanceForTargetActivity(
      ActivityInstance activityInstance, String targetActivityId) {
    for (TransitionInstance childTransitionInstance : activityInstance
    ...
    for (ActivityInstance childInstance : activityInstance.getChildActivityInstances()) {
      TransitionInstance instance = getChildTransitionInstanceForTargetActivity(childInstance,
          targetActivityId);
      if (instance != null) {
        return instance;
      }
