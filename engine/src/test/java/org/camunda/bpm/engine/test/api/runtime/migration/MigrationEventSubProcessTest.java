...
public class MigrationEventSubProcessTest {
  ...
  public void testMigrateActiveEventSubProcess() {
    ...
    ProcessInstance processInstance = rule.getRuntimeService()
        ...
        .startBeforeActivity(EVENT_SUB_PROCESS_TASK_ID).execute();

    MigrationPlan migrationPlan = rule.getRuntimeService()
        .createMigrationPlan(sourceProcessDefinition.getId(), targetProcessDefinition.getId())
        .mapActivities("eventSubProcess", "eventSubProcess")
        .mapActivities(EVENT_SUB_PROCESS_TASK_ID, EVENT_SUB_PROCESS_TASK_ID).build();
    ...
    testHelper.assertExecutionTreeAfterMigration()
        ...
        .matches(describeExecutionTree(null).scope().id(processInstance.getId())
    ...
    testHelper.assertActivityTreeAfterMigration()
        .hasStructure(describeActivityInstanceTree(targetProcessDefinition.getId())
            ...
            .activity(EVENT_SUB_PROCESS_TASK_ID, testHelper
                .getSingleActivityInstanceBeforeMigration(EVENT_SUB_PROCESS_TASK_ID).getId())
            .done());

    testHelper.assertEventSubscriptionRemoved(EVENT_SUB_PROCESS_START_ID,
        EventSubProcessModels.MESSAGE_NAME);
    testHelper.assertEventSubscriptionCreated(EVENT_SUB_PROCESS_START_ID,
        EventSubProcessModels.MESSAGE_NAME);
  ...
  public void testMigrateActiveEventSubProcessToEmbeddedSubProcess() {
    ...
    MigrationPlan migrationPlan = rule.getRuntimeService()
        ...
        .mapActivities(EVENT_SUB_PROCESS_TASK_ID, USER_TASK_ID).build();
    ...
    testHelper.assertExecutionTreeAfterMigration()
        .hasProcessDefinitionId(targetProcessDefinition.getId())
        .matches(describeExecutionTree(null).scope().id(processInstance.getId()).child(USER_TASK_ID)
            .scope()
            .id(testHelper.getSingleExecutionIdForActivityBeforeMigration("eventSubProcess"))
            .done());
    ...
    testHelper.assertActivityTreeAfterMigration()
        .hasStructure(describeActivityInstanceTree(targetProcessDefinition.getId())
            .beginScope("subProcess",
                testHelper.getSingleActivityInstanceBeforeMigration("eventSubProcess").getId())
            .activity(USER_TASK_ID, testHelper
                .getSingleActivityInstanceBeforeMigration(EVENT_SUB_PROCESS_TASK_ID).getId())
            .done());
  ...
  public void testMigrateActiveEmbeddedSubProcessToEventSubProcess() {
    ...
    MigrationPlan migrationPlan = rule.getRuntimeService()
        ...
        .mapActivities(USER_TASK_ID, EVENT_SUB_PROCESS_TASK_ID).build();
  ...
  public void testMigrateActiveErrorEventSubProcess() {
    // given
    ProcessDefinition sourceProcessDefinition = testHelper
        .deployAndGetDefinition(EventSubProcessModels.ERROR_EVENT_SUBPROCESS_PROCESS);
    ProcessDefinition targetProcessDefinition = testHelper
        .deployAndGetDefinition(EventSubProcessModels.ERROR_EVENT_SUBPROCESS_PROCESS);
  ...
  public void testMigrateActiveCompensationEventSubProcess() {
    // given
    ProcessDefinition sourceProcessDefinition = testHelper
        .deployAndGetDefinition(EventSubProcessModels.COMPENSATE_EVENT_SUBPROCESS_PROCESS);
    ProcessDefinition targetProcessDefinition = testHelper
        .deployAndGetDefinition(EventSubProcessModels.COMPENSATE_EVENT_SUBPROCESS_PROCESS);
  ...
  public void testMigrateActiveEscalationEventSubProcess() {
    // given
    ProcessDefinition sourceProcessDefinition = testHelper
        .deployAndGetDefinition(EventSubProcessModels.ESCALATION_EVENT_SUBPROCESS_PROCESS);
    ProcessDefinition targetProcessDefinition = testHelper
        .deployAndGetDefinition(EventSubProcessModels.ESCALATION_EVENT_SUBPROCESS_PROCESS);
  ...
  public void testMigrateTaskAddEventSubProcess() {
    ...
    testHelper.assertExecutionTreeAfterMigration()
        ...
        .matches(describeExecutionTree(null).scope()
            .id(testHelper.snapshotBeforeMigration.getProcessInstanceId())
            .child(EVENT_SUB_PROCESS_TASK_ID).scope().done());
  ...
  public void testMigrateEventSubprocessMessageKeepTrigger() {
    ...
    MigrationPlan migrationPlan = rule.getRuntimeService()
        ...
        .mapActivities(EVENT_SUB_PROCESS_START_ID, EVENT_SUB_PROCESS_START_ID).build();
  ...
  public void testMigrateEventSubprocessSignalKeepTrigger() {
    ProcessDefinition sourceProcessDefinition = testHelper
        .deployAndGetDefinition(EventSubProcessModels.SIGNAL_EVENT_SUBPROCESS_PROCESS);
    ProcessDefinition targetProcessDefinition = testHelper
        .deployAndGetDefinition(EventSubProcessModels.SIGNAL_EVENT_SUBPROCESS_PROCESS);
  ...
  public void testMigrateConditionalBoundaryEventKeepTrigger() {
    // given
    ProcessDefinition sourceProcessDefinition = testHelper
        .deployAndGetDefinition(CONDITIONAL_EVENT_SUBPROCESS_PROCESS);
    ProcessDefinition targetProcessDefinition = testHelper
        .deployAndGetDefinition(CONDITIONAL_EVENT_SUBPROCESS_PROCESS);
  ...
  public void testMigrateEventSubprocessChangeStartEventType() {
    ...
    try {
      ...
      // then
      assertThat(e.getValidationReport()).hasInstructionFailures(EVENT_SUB_PROCESS_START_ID,
  ...
  public void testMigrateNonInterruptingEventSubprocessMessageTrigger() {
    BpmnModelInstance nonInterruptingModel = modify(
        EventSubProcessModels.MESSAGE_EVENT_SUBPROCESS_PROCESS)
            .startEventBuilder(EVENT_SUB_PROCESS_START_ID).interrupting(false).done();

    ProcessDefinition sourceProcessDefinition = testHelper
        .deployAndGetDefinition(nonInterruptingModel);
    ProcessDefinition targetProcessDefinition = testHelper
        .deployAndGetDefinition(nonInterruptingModel);
  ...
  public void testUpdateEventMessage() {
    ...
    BpmnModelInstance targetProcess = modify(EventSubProcessModels.MESSAGE_EVENT_SUBPROCESS_PROCESS)
        .renameMessage(EventSubProcessModels.MESSAGE_NAME,
            "new" + EventSubProcessModels.MESSAGE_NAME);
    ...
    // then
    testHelper.assertEventSubscriptionMigrated(EVENT_SUB_PROCESS_START_ID,
        EventSubProcessModels.MESSAGE_NAME, EVENT_SUB_PROCESS_START_ID,
        "new" + EventSubProcessModels.MESSAGE_NAME);
  ...
  public void testUpdateEventSignal() {
    ...
    // then
    testHelper.assertEventSubscriptionMigrated(EVENT_SUB_PROCESS_START_ID,
        EventSubProcessModels.SIGNAL_NAME, EVENT_SUB_PROCESS_START_ID,
        "new" + EventSubProcessModels.SIGNAL_NAME);
  ...
  public void testUpdateEventTimer() {
    ...
    BpmnModelInstance targetProcess = modify(EventSubProcessModels.TIMER_EVENT_SUBPROCESS_PROCESS)
        .removeChildren(EVENT_SUB_PROCESS_START_ID).startEventBuilder(EVENT_SUB_PROCESS_START_ID)
        .timerWithDuration("PT50M").done();
    ...
    testHelper.assertJobMigrated(testHelper.snapshotBeforeMigration.getJobs().get(0),
        EVENT_SUB_PROCESS_START_ID, newDueDate);
  ...
  public void testUpdateEventMessageWithExpression() {
    ...
    BpmnModelInstance targetProcess = modify(
    ...
    MigrationPlan migrationPlan = rule.getRuntimeService()
        ...
        .mapActivities(USER_TASK_ID, USER_TASK_ID)
        .mapActivities("eventSubProcess", "eventSubProcess")
        .mapActivities("catchMessage", "catchMessage").updateEventTrigger().build();
  ...
  public void testUpdateEventSignalWithExpression() {
    ...
    testHelper.assertEventSubscriptionMigrated(EVENT_SUB_PROCESS_START_ID,
        EventSubProcessModels.SIGNAL_NAME, EVENT_SUB_PROCESS_START_ID, resolvedsignalName);
