...
public class MigrationUserTaskTest {
  ...
  public void testCannotMigrateWhenNotAllActivityInstancesAreMapped() {
    ...
    try {
      ...
      assertThat(e.getValidationReport()).hasActivityInstanceFailures("userTask2",
  ...
  public void testCannotMigrateWhenNotAllTransitionInstancesAreMapped() {
    // given
    BpmnModelInstance model = ModifiableBpmnModelInstance
        .modify(ProcessModels.PARALLEL_GATEWAY_PROCESS).activityBuilder("userTask1")
        .camundaAsyncBefore().moveToActivity("userTask2").camundaAsyncBefore().done();
    ...
    try {
      ...
      assertThat(e.getValidationReport()).hasTransitionInstanceFailures("userTask2",
          "There is no migration instruction for this instance's activity");
    }
  }
  ...
  public void testMigrateWithSubTask() {
    ...
    // then the sub task properties have not been updated (i.e. subtask should not reference the
    // process instance/definition now)
    Task subTaskAfterMigration = rule.getTaskService().createTaskQuery().taskId(subTask.getId())
        .singleResult();
    ...
    if (!rule.getProcessEngineConfiguration().getHistoryLevel()
        .equals(HistoryLevel.HISTORY_LEVEL_NONE)) {
  ...
  public void testAccessModelInTaskListenerAfterMigration() {
    ...
    addTaskListener(targetModel, "newUserTask", TaskListener.EVENTNAME_ASSIGNMENT,
        AccessModelInstanceTaskListener.class.getName());
    ...
    String variableValue = (String) rule.getRuntimeService().getVariable(processInstance.getId(),
        AccessModelInstanceTaskListener.VARIABLE_NAME);
    Assert.assertEquals("newUserTask", variableValue);

  }

  protected static void addTaskListener(BpmnModelInstance targetModel, String activityId,
      String event, String className) {
