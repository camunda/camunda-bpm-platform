...
public class CallActivityTest extends PluggableProcessEngineTestCase {
  ...
  public void testSubProcessEndsSuperProcess() {
    ProcessInstance processInstance = runtimeService
        .startProcessInstanceByKey("subProcessEndsSuperProcess");
  ...
  @Deployment(resources = {
      "org/camunda/bpm/engine/test/bpmn/callactivity/CallActivity.testCallSequentialSubProcess.bpmn20.xml",
      "org/camunda/bpm/engine/test/bpmn/callactivity/CallActivity.testCallSimpleSubProcessWithExpressions.bpmn20.xml",
      "org/camunda/bpm/engine/test/bpmn/callactivity/simpleSubProcess.bpmn20.xml",
  ...
  public void testCallSequentialSubProcessWithExpressions() {
    ...
    ProcessInstance processInstance = runtimeService
        .startProcessInstanceByKey("callSequentialSubProcess");
  ...
  public void testSubProcessWithDataInputOutput() {
    ...
    ProcessInstance processInstance = runtimeService
        .startProcessInstanceByKey("subProcessDataInputOutput", vars);
    ...
    runtimeService.setVariable(taskBeforeSubProcess.getProcessInstanceId(), "subVariable",
    ...
    assertEquals("Hello from sub process.",
        runtimeService.getVariable(processInstance.getId(), "superVariable"));
    assertEquals("Hello from sub process.",
        taskService.getVariable(taskAfterSubProcess.getId(), "superVariable"));
    ...
    assertEquals(10l,
        runtimeService.getVariable(taskInSecondSubProcess.getProcessInstanceId(), "y"));
    ...
    assertEquals(15l,
        runtimeService.getVariable(taskAfterSecondSubProcess.getProcessInstanceId(), "z"));
  ...
  /**
   * Test case for handing over process variables to a sub process via the typed api and passing
  ...
  public void testSubProcessWithLimitedDataInputOutputTypedApi() {
    ...
    assertThat(
        runtimeService.getVariableTyped(taskInSubProcess.getProcessInstanceId(), "subVariable"),
        is(superVariable));
    assertThat(taskService.getVariableTyped(taskInSubProcess.getId(), "subVariable"),
        is(superVariable));
    ...
    // super variable is unchanged
    assertThat(runtimeService.getVariableTyped(processInstance.getId(), "superVariable"),
        is(superVariable));
    ...
    assertThat(runtimeService.getVariableTyped(processInstance.getId(), "superVariable"),
        is(subVariable));
    assertThat(taskService.getVariableTyped(taskAfterSubProcess.getId(), "superVariable"),
        is(subVariable));
  ...
  /**
   * Test case for handing over process variables to a sub process via the typed api and passing all
   * variables
   */
  @Deployment(resources = {
  ...
  public void testSubProcessWithAllDataInputOutputTypedApi() {
    ...
    assertThat(taskInSubProcess.getName(), is("Task in subprocess"));
    assertThat(
        runtimeService.getVariableTyped(taskInSubProcess.getProcessInstanceId(), "superVariable"),
        is(superVariable));
    assertThat(taskService.getVariableTyped(taskInSubProcess.getId(), "superVariable"),
        is(superVariable));
    ...
    Task taskAfterSubProcess = taskQuery.singleResult();
    assertThat(taskAfterSubProcess.getName(), is("Task in super process"));
    assertThat(runtimeService.getVariableTyped(processInstance.getId(), "subVariable"),
        is(subVariable));
    assertThat(taskService.getVariableTyped(taskAfterSubProcess.getId(), "superVariable"),
        is(superVariable));
  ...
  public void testSubProcessWithDataInputOutputWithoutTarget() {
    ...
    BpmnModelInstance modelInstance = Bpmn.createExecutableProcess(processId).startEvent()
        .callActivity("callActivity").calledElement("simpleSubProcess").userTask().endEvent()
        .done();

    CallActivityBuilder callActivityBuilder = ((CallActivity) modelInstance
        .getModelElementById("callActivity")).builder();
  ...
  public void testSubProcessWithNullDataInput() {
    String processInstanceId = runtimeService.startProcessInstanceByKey("subProcessDataInputOutput")
        .getId();
    ...
    VariableInstance variable = runtimeService.createVariableInstanceQuery()
        .processInstanceIdIn(processInstanceId).variableName("subVariable").singleResult();
    assertNull(variable);

    variable = runtimeService.createVariableInstanceQuery().processInstanceIdIn(processInstanceId)
        .variableName("superVariable").singleResult();
    assertNull(variable);
    ...
    variable = runtimeService.createVariableInstanceQuery()
        .processInstanceIdIn(subProcessInstanceId).variableName("subVariable").singleResult();
  ...
  public void testSubProcessWithNullDataInputAsExpression() {
    ...
    String processInstanceId = runtimeService
        .startProcessInstanceByKey("subProcessDataInputOutput", params).getId();
  ...
  public void testSubProcessWithNullDataOutput() {
    ...
    variable = runtimeService.createVariableInstanceQuery().processInstanceIdIn(processInstanceId)
        .variableName("subVariable").singleResult();
    assertNull(variable);
    ...
    variable = runtimeService.createVariableInstanceQuery().processInstanceIdIn(processInstanceId)
        .variableName("hisLocalVariable").singleResult();
    assertNotNull(variable);
    assertNull(variable.getValue());
  ...
  public void testTwoSubProcesses() {
    ProcessInstance processInstance = runtimeService
        .startProcessInstanceByKey("callTwoSubProcesses");
  ...
  public void testSubProcessAllDataInputOutput() {
    ...
    ProcessInstance processInstance = runtimeService
        .startProcessInstanceByKey("subProcessAllDataInputOutput", vars);
    ...
    assertEquals("Task before subprocess", taskBeforeSubProcess.getName());
    assertEquals("Hello from the super process.",
        runtimeService.getVariable(taskBeforeSubProcess.getProcessInstanceId(), "superVariable"));
    assertEquals("Hello from the super process.",
        taskService.getVariable(taskBeforeSubProcess.getId(), "superVariable"));
    assertEquals("Only a test.",
        runtimeService.getVariable(taskBeforeSubProcess.getProcessInstanceId(), "testVariable"));
    assertEquals("Only a test.",
        taskService.getVariable(taskBeforeSubProcess.getId(), "testVariable"));
    ...
    assertEquals("Task in subprocess", taskInSubProcess.getName());
    assertEquals("Hello from the super process.",
        runtimeService.getVariable(taskInSubProcess.getProcessInstanceId(), "superVariable"));
    assertEquals("Hello from the super process.",
        taskService.getVariable(taskInSubProcess.getId(), "superVariable"));
    assertEquals("Only a test.",
        runtimeService.getVariable(taskInSubProcess.getProcessInstanceId(), "testVariable"));
    assertEquals("Only a test.", taskService.getVariable(taskInSubProcess.getId(), "testVariable"));

    // changed variables in sub process
    runtimeService.setVariable(taskInSubProcess.getProcessInstanceId(), "superVariable",
        "Hello from sub process.");
    runtimeService.setVariable(taskInSubProcess.getProcessInstanceId(), "testVariable",
        "Variable changed in sub process.");
    ...
    assertEquals("Hello from sub process.",
        runtimeService.getVariable(processInstance.getId(), "superVariable"));
    assertEquals("Variable changed in sub process.",
        runtimeService.getVariable(processInstance.getId(), "testVariable"));
  ...
  public void testSubProcessAllDataInputOutputWithAdditionalInputMapping() {
    ...
    assertEquals("Hello from the super process.",
        runtimeService.getVariable(taskInSubProcess.getProcessInstanceId(), "superVariable"));
    assertEquals("Hello from the super process.",
        runtimeService.getVariable(taskInSubProcess.getProcessInstanceId(), "subVariable"));
    assertEquals("Hello from the super process.",
        taskService.getVariable(taskInSubProcess.getId(), "superVariable"));
    assertEquals("Only a test.",
        runtimeService.getVariable(taskInSubProcess.getProcessInstanceId(), "testVariable"));
  ...
  /**
   * This testcase verifies that <camunda:out variables="all" /> works also in case super process
   * has no variables
   *
  ...
  public void testSubProcessAllDataOutput() {
    ...
    ProcessInstance processInstance = runtimeService
        .startProcessInstanceByKey("subProcessAllDataInputOutput");
  ...
  public void testSubProcessLocalInputAllVariables() {
    ProcessInstance processInstance = runtimeService
        .startProcessInstanceByKey("subProcessLocalInputAllVariables");
    ...
    Map<String, Object> calledInstanceVariables = runtimeService
        .getVariables(calledInstance.getId());
    ...
    Map<String, Object> callingInstanceVariables = runtimeService
  ...
  public void testSubProcessLocalInputSingleVariable() {
    ProcessInstance processInstance = runtimeService
        .startProcessInstanceByKey("subProcessLocalInputSingleVariable");
    Task beforeCallActivityTask = taskService.createTaskQuery().singleResult();
  ...
  public void testSubProcessLocalInputSingleVariableExpression() {
    ProcessInstance processInstance = runtimeService
        .startProcessInstanceByKey("subProcessLocalInputSingleVariableExpression");
    Task beforeCallActivityTask = taskService.createTaskQuery().singleResult();
    ...
    // and executing a call activity in parameter where the source variable is not mapped by an
    // activity
  ...
  public void testSubProcessLocalOutputAllVariables() {
    ProcessInstance processInstance = runtimeService
        .startProcessInstanceByKey("subProcessLocalOutputAllVariables");
    Task beforeCallActivityTask = taskService.createTaskQuery().singleResult();
  ...
  public void testSubProcessLocalOutputSingleVariable() {
    ProcessInstance processInstance = runtimeService
        .startProcessInstanceByKey("subProcessLocalOutputSingleVariable");
    Task beforeCallActivityTask = taskService.createTaskQuery().singleResult();
  ...
  public void testSubProcessBusinessKeyInput() {
    ...
    ProcessInstance processInstance = runtimeService
        .startProcessInstanceByKey("subProcessBusinessKeyInput", businessKey);
    ...
    if (processEngineConfiguration.getHistoryLevel()
      ...
      assertEquals(2, historyService.createHistoricProcessInstanceQuery()
          .processInstanceBusinessKey(businessKey).list().size());
    }
    ...
    if (processEngineConfiguration.getHistoryLevel()
        .getId() > ProcessEngineConfigurationImpl.HISTORYLEVEL_NONE) {
      HistoricProcessInstance hpi = historyService.createHistoricProcessInstanceQuery()
          .superProcessInstanceId(processInstance.getId()).finished().singleResult();
      assertEquals(businessKey, hpi.getBusinessKey());

      assertEquals(2, historyService.createHistoricProcessInstanceQuery()
          .processInstanceBusinessKey(businessKey).finished().list().size());
    }
  }

  @Deployment(resources = {
      "org/camunda/bpm/engine/test/bpmn/callactivity/CallActivity.testCallSimpleSubProcessWithHashExpressions.bpmn20.xml",
      "org/camunda/bpm/engine/test/bpmn/callactivity/simpleSubProcess.bpmn20.xml" })
  ...
  @Deployment(resources = {
      "org/camunda/bpm/engine/test/bpmn/callactivity/CallActivity.testInterruptingEventSubProcessEventSubscriptions.bpmn20.xml",
  ...
  public void testInterruptingMessageEventSubProcessEventSubscriptionsInsideCallActivity() {
    ProcessInstance processInstance = runtimeService
        .startProcessInstanceByKey("callInterruptingEventSubProcess");
    ...
    assertEquals("taskBeforeInterruptingEventSubprocess",
        taskInsideCallActivity.getTaskDefinitionKey());

    // we should have no event subscriptions for the parent process
    assertEquals(0, runtimeService.createEventSubscriptionQuery()
        .processInstanceId(processInstance.getId()).count());
    // we should have two event subscriptions for the called process instance, one for message and
    // one for signal
    ...
    Task taskAfterMessageStartEvent = taskQuery.processInstanceId(calledProcessInstanceId)
        .singleResult();
  ...
  public void testInterruptingSignalEventSubProcessEventSubscriptionsInsideCallActivity() {
    ...
    Task taskAfterSignalStartEvent = taskQuery.processInstanceId(calledProcessInstanceId)
        .singleResult();
  ...
  public void testInputParameterLiteralSourceExpression() {
    ...
    String subInstanceId = runtimeService.createProcessInstanceQuery()
        .processDefinitionKey("simpleSubProcess").singleResult().getId();
  ...
  public void testTwoSubProcessesDataOutputOnError() {
    ...
    Object variable = runtimeService.getVariable(task.getProcessInstanceId(), variableName);
    // both processes have and out mapping for all, so we want the variable to be propagated to the
    // process with the event handler
    assertThat(variable, is(notNullValue()));
    assertThat(variable, is(variableValue));
  }
  ...
  public void testCallCaseByVersionAsExpression() {
    ...
    String secondDeploymentId = repositoryService.createDeployment()
        .addClasspathResource(bpmnResourceName).deploy().getId();

    String thirdDeploymentId = repositoryService.createDeployment()
        .addClasspathResource(bpmnResourceName).deploy().getId();

    String processDefinitionIdInSecondDeployment = repositoryService.createProcessDefinitionQuery()
        .processDefinitionKey("oneTaskProcess").deploymentId(secondDeploymentId).singleResult()
        .getId();
  ...
  public void testCallProcessByVersionTag() {
    ...
    // then
    ProcessInstance subInstance = runtimeService.createProcessInstanceQuery()
        .processDefinitionKey("subProcess").superProcessInstanceId(processInstance.getId())
        .singleResult();
    assertNotNull(subInstance);
  ...
  public void testCallProcessByVersionTagAsExpression() {
    // given
    BpmnModelInstance modelInstance = getModelWithCallActivityVersionTagBinding(
        "${versionTagExpr}");
  ...
  public void testCallProcessWithoutVersionTag() {
    ...
    BpmnModelInstance modelInstance = Bpmn.createExecutableProcess("process").startEvent()
        ...
        .camundaCalledElementBinding("versionTag").endEvent().done();
    ...
    try {
      ...
      assertTrue(e.getMessage().contains(
          "Missing attribute 'calledElementVersionTag' when 'calledElementBinding' has value 'versionTag'"));
    }
  }
  ...
  public void testCallProcessByVersionTagNoneSubprocess() {
    ...
    try {
      ...
      // then
      assertTrue(e.getMessage().contains(
          "no processes deployed with key = 'subProcess', versionTag = 'ver_tag_1' and tenant-id = 'null': processDefinition is null"));
    }
  }
  ...
  public void testCallProcessByVersionTagTwoSubprocesses() {
    ...
    try {
      ...
      // then
      assertTrue(e.getMessage().contains(
          "There are '2' results for a process definition with key 'subProcess', versionTag 'ver_tag_1' and tenant-id '{}'."));
    }
  ...
  @Deployment(resources = { "org/camunda/bpm/engine/test/bpmn/callactivity/orderProcess.bpmn20.xml",
  ...
  public void testDeleteProcessInstanceInCallActivity() {
    ...
    runtimeService.deleteProcessInstance(taskInSubProcess.getProcessInstanceId(),
   ...
   *
   * Checks that deletion of process Instance will resepct other process instances in the scope and
   * stop its upward deletion propagation will stop at this point
   *
   */
  @Deployment(resources = {
      "org/camunda/bpm/engine/test/bpmn/callactivity/CallActivity.testTwoSubProcesses.bpmn20.xml",
  ...
  public void testSingleDeletionWithTwoSubProcesses() {
    ...
    List<String> activeActivityIds = runtimeService
        .getActiveActivityIds(processInstance.getProcessInstanceId());
    assertNotNull(activeActivityIds);
    assertEquals(2, activeActivityIds.size());

    // when
    runtimeService.deleteProcessInstance(taskList.get(0).getProcessInstanceId(),
        "Test upstream deletion");
  ...
  public void testDeleteMultilevelProcessInstanceInCallActivity() {
    ...
    // when
    // Delete the ProcessInstance in the sub process
    runtimeService.deleteProcessInstance(taskInNestedSubProcess.getProcessInstanceId(),
        "Test cascading upstream deletion");
   ...
   *
   * At each nested level there is only one process instance, which is waiting on the next level to
   * complete
   *
   * When we delete the process instance of the most inner call activity sub process the expected
   * behaviour is that the delete will propagate upward and delete all process instances.
   *
   */
  @Deployment(resources = {
      "org/camunda/bpm/engine/test/bpmn/callactivity/CallActivity.testCallSimpleSubProcess.bpmn20.xml",
  ...
  public void testDeleteDoubleNestedProcessInstanceInCallActivity() {
    ...
    // when
    // Delete the ProcessInstance in the sub process
    runtimeService.deleteProcessInstance(taskInDoubleNestedSubProcess.getProcessInstanceId(),
        "Test cascading upstream deletion");
  ...
  protected BpmnModelInstance getModelWithCallActivityVersionTagBinding(String versionTag) {
    return Bpmn.createExecutableProcess("process").startEvent().callActivity("callActivity")
        .calledElement("subProcess").camundaCalledElementBinding("versionTag")
        .camundaCalledElementVersionTag(versionTag).endEvent().done();
  }
