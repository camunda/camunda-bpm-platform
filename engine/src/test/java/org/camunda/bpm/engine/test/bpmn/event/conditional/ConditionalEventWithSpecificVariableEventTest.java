...
public class ConditionalEventWithSpecificVariableEventTest
    ...
    return Arrays.asList(new Object[][] { {
        ...
        new ConditionalProcessVarSpecification() {
          ...
          public BpmnModelInstance getProcessWithVarName(boolean interrupting, String condition) {
            return modify(TASK_MODEL).userTaskBuilder(TASK_BEFORE_CONDITION_ID).boundaryEvent()
                .cancelActivity(interrupting).conditionalEventDefinition(CONDITIONAL_EVENT)
                .condition(condition).camundaVariableName(VARIABLE_NAME)
                .conditionalEventDefinitionDone().userTask().name(TASK_AFTER_CONDITION).endEvent()
                .done();
          }

          @Override
          public BpmnModelInstance getProcessWithVarNameAndEvents(boolean interrupting,
              String varEvent) {
            return modify(TASK_MODEL).userTaskBuilder(TASK_BEFORE_CONDITION_ID).boundaryEvent()
                .cancelActivity(interrupting).conditionalEventDefinition(CONDITIONAL_EVENT)
                .condition(CONDITION_EXPR).camundaVariableName(VARIABLE_NAME)
                .camundaVariableEvents(varEvent).conditionalEventDefinitionDone().userTask()
          ...
          public BpmnModelInstance getProcessWithVarEvents(boolean interrupting, String varEvent) {
            return modify(TASK_MODEL).userTaskBuilder(TASK_BEFORE_CONDITION_ID).boundaryEvent()
                .cancelActivity(interrupting).conditionalEventDefinition(CONDITIONAL_EVENT)
                .condition(CONDITION_EXPR).camundaVariableEvents(varEvent)
                .conditionalEventDefinitionDone().userTask().name(TASK_AFTER_CONDITION).endEvent()
                .done();
          }
        ...
        { new ConditionalProcessVarSpecification() {
          @Override
          public BpmnModelInstance getProcessWithVarName(boolean interrupting, String condition) {
            return modify(TASK_MODEL).addSubProcessTo(CONDITIONAL_EVENT_PROCESS_KEY)
                .triggerByEvent().embeddedSubProcess().startEvent().interrupting(interrupting)
                .conditionalEventDefinition(CONDITIONAL_EVENT).condition(condition)
                .camundaVariableName(VARIABLE_NAME).conditionalEventDefinitionDone().userTask()
                .name(TASK_AFTER_CONDITION).endEvent().done();
          }
          ...
          public BpmnModelInstance getProcessWithVarNameAndEvents(boolean interrupting,
            ...
            return modify(TASK_MODEL).addSubProcessTo(CONDITIONAL_EVENT_PROCESS_KEY)
                ...
                .camundaVariableName(VARIABLE_NAME).camundaVariableEvents(varEvent)
                .conditionalEventDefinitionDone().userTask().name(TASK_AFTER_CONDITION).endEvent()
                .done();
          }
  ...
  public void testVariableConditionWithVariableNameAndEvent() {
    ...
    final BpmnModelInstance modelInstance = specifier.getProcessWithVarNameAndEvents(true,
        CONDITIONAL_VAR_EVENT_UPDATE);
    engine.manageDeployment(repositoryService.createDeployment()
        .addModelInstance(CONDITIONAL_MODEL, modelInstance).deploy());
  ...
  public void testNonInterruptingVariableConditionWithVariableName() {
    ...
    // given process with non interrupting boundary conditional event and defined variable name and
    // true condition
  ...
  public void testNonInterruptingVariableConditionWithVariableNameAndEvents() {
    ...
    // given process with non interrupting boundary conditional event and defined variable name and
    // events
    final BpmnModelInstance modelInstance = specifier.getProcessWithVarNameAndEvents(false,
        CONDITIONAL_VAR_EVENTS);
    engine.manageDeployment(repositoryService.createDeployment()
        .addModelInstance(CONDITIONAL_MODEL, modelInstance).deploy());
  ...
  public void testVariableConditionWithVariableEvent() {
    ...
    final BpmnModelInstance modelInstance = specifier.getProcessWithVarEvents(true,
        CONDITIONAL_VAR_EVENT_UPDATE);
    engine.manageDeployment(repositoryService.createDeployment()
        .addModelInstance(CONDITIONAL_MODEL, modelInstance).deploy());
  ...
  public void testNonInterruptingVariableConditionWithVariableEvent() {
    ...
    final BpmnModelInstance modelInstance = specifier.getProcessWithVarEvents(false,
        CONDITIONAL_VAR_EVENT_UPDATE);
    engine.manageDeployment(repositoryService.createDeployment()
        .addModelInstance(CONDITIONAL_MODEL, modelInstance).deploy());
