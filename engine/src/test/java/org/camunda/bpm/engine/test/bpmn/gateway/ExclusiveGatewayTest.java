...
public class ExclusiveGatewayTest extends PluggableProcessEngineTestCase {
  ...
  public void testNoSequenceFlowSelected() {
    try {
      ...
      assertTextPresent(
          "ENGINE-02004 No outgoing sequence flow for the element with id 'exclusiveGw' could be selected for continuing the process.",
          e.getMessage());
    }
  }
  ...
  public void testDecideBasedOnListOrArrayOfBeans() {
    ...
    ExclusiveGatewayTestOrder[] orderArray = orders
        .toArray(new ExclusiveGatewayTestOrder[orders.size()]);
  ...
  public void testNoIdOnSequenceFlow() {
    runtimeService.startProcessInstanceByKey("noIdOnSequenceFlow",
  ...
  public void testInvalidProcessDefinition() {
    String flowWithoutConditionNoDefaultFlow = "<?xml version='1.0' encoding='UTF-8'?>"
        ...
        "    <exclusiveGateway id='exclusiveGw' name='Exclusive Gateway' /> " + // no default =
                                                                                // "flow3" !!
        "    <sequenceFlow id='flow2' sourceRef='exclusiveGw' targetRef='theTask1'> "
        + "      <conditionExpression xsi:type='tFormalExpression'>${input == 1}</conditionExpression> "
        + "    </sequenceFlow> "
        + "    <sequenceFlow id='flow3' sourceRef='exclusiveGw' targetRef='theTask2'/> " + // one
                                                                                           // would
                                                                                           // be OK
        "    <sequenceFlow id='flow4' sourceRef='exclusiveGw' targetRef='theTask2'/> " + // but two
                                                                                         // unconditional
                                                                                         // not!
    ...
    try {
      repositoryService.createDeployment()
          .addString("myprocess.bpmn20.xml", flowWithoutConditionNoDefaultFlow).deploy();
      fail(
          "Could deploy a process definition with a sequence flow out of a XOR Gateway without condition with is not the default flow.");
    ...
    try {
      ...
      assertTrue(ex.getMessage()
          .contains("Exclusive Gateway 'exclusiveGw' has no outgoing sequence flows."));
    }
  ...
  public void testLoopWithManyIterations() {
    ...
    runtimeService.startProcessInstanceByKey("testProcess",
        Variables.createVariables().putValue("numOfIterations", numOfIterations));
  }
