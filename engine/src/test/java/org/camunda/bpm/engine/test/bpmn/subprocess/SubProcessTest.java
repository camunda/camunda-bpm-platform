...
public class SubProcessTest extends PluggableProcessEngineTestCase {
  ...
  public void testSimpleSubProcess() {
    ...
    ActivityInstance rootActivityInstance = runtimeService
        .getActivityInstance(pi.getProcessInstanceId());
  ...
  public void IGNORE_testSimpleSubProcessWithConcurrentTimer() {
    ...
    ProcessInstance pi = runtimeService
        .startProcessInstanceByKey("simpleSubProcessWithConcurrentTimer");
  ...
  public void testNestedSimpleSubprocessWithTimerOnInnerSubProcess() {
    ...
    // Setting the clock forward 1 hour 1 second (timer fires in 1 hour) and fire up the job
    // executor
    ...
    // Completing the escalated task, destroys the outer scope and activates the task after the
    // subprocess
  ...
  public void testSimpleParallelSubProcess() {
    ...
    // Completing both tasks, should destroy the subprocess and activate the task after the
    // subprocess
  ...
  public void testSimpleParallelSubProcessWithTimer() {
    ...
    ProcessInstance processInstance = runtimeService
        .startProcessInstanceByKey("simpleParallelSubProcessWithTimer");
    ...
    Task taskAfterTimer = taskService.createTaskQuery().processInstanceId(processInstance.getId())
        .singleResult();
  ...
  public void testTwoSubProcessInParallel() {
    ...
    for (ActivityInstance activityInstance : childActivityInstances) {
      assertTrue(Arrays.asList(new String[] { "subProcessA", "subProcessB" })
          .contains(activityInstance.getActivityId()));
      ActivityInstance[] subProcessChildren = activityInstance.getChildActivityInstances();
      assertEquals(1, subProcessChildren.length);
      assertTrue(Arrays.asList(new String[] { "subProcessATask", "subProcessBTask" })
          .contains(subProcessChildren[0].getActivityId()));
    }
  ...
  public void testTwoSubProcessInParallelWithinSubProcess() {
    ...
    ActivityInstance rootActivityInstance = runtimeService
        .getActivityInstance(pi.getProcessInstanceId());
    ActivityInstanceAssert.assertThat(rootActivityInstance).hasStructure(
        ActivityInstanceAssert.describeActivityInstanceTree(pi.getProcessDefinitionId())
            .beginScope("outerSubProcess").beginScope("subProcessA").activity("subProcessATask")
            .endScope().beginScope("subProcessB").activity("subProcessBTask").done());
  ...
  public void testTwoNestedSubProcessesInParallelWithTimer() {
    ...
    ProcessInstance pi = runtimeService
        .startProcessInstanceByKey("nestedParallelSubProcessesWithTimer");
    TaskQuery taskQuery = taskService.createTaskQuery().processInstanceId(pi.getId())
        .orderByTaskName().asc();
    List<Task> tasks = taskQuery.list();
  ...
  public void testSimpleSubProcessWithoutEndEvent() {
    ProcessInstance pi = runtimeService
        .startProcessInstanceByKey("testSimpleSubProcessWithoutEndEvent");
    assertProcessEnded(pi.getId());
  }
  ...
  public void testNestedSubProcessesWithoutEndEvents() {
    ProcessInstance pi = runtimeService
        .startProcessInstanceByKey("testNestedSubProcessesWithoutEndEvents");
    assertProcessEnded(pi.getId());
  }
  ...
  public void testConcurrencyInSubProcess() {
    ...
    org.camunda.bpm.engine.repository.Deployment deployment = repositoryService.createDeployment()
        .addClasspathResource(
            "org/camunda/bpm/engine/test/bpmn/subprocess/SubProcessTest.fixSystemFailureProcess.bpmn20.xml")
        .deploy();
