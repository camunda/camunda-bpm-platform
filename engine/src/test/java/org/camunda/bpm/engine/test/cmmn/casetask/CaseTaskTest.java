...
public class CaseTaskTest extends CmmnProcessEngineTestCase {
  ...
  public void testInputDifferentBusinessKey() {
    ...
    // when
    caseService.withCaseExecution(caseTaskId).setVariable("myOwnBusinessKey", "myOwnBusinessKey")
        .manualStart();
  ...
  public void testInputSource() {
    ...
    // when
    caseService.withCaseExecution(caseTaskId).setVariable("aVariable", "abc")
        .setVariable("anotherVariable", 999).setVariable("aThirdVariable", "def").manualStart();
  ...
  /**
   * Default manual activation changed - add variables to case instantiation, remove manual
   * activation
   */
  @Deployment(resources = {
  ...
  public void testInputAllLocal() {
    ...
    // when
    caseService.withCaseExecution(caseTaskId).setVariable("aVariable", "abc")
        .setVariableLocal("aLocalVariable", "def").manualStart();
  ...
  public void testOutputSource() {
    ...
    // then
    List<VariableInstance> variables = runtimeService.createVariableInstanceQuery()
        .caseInstanceIdIn(superCaseInstanceId).list();
  ...
  /**
   * assertion on variables - change process definition manual start on case not needed enaymore and
   * therefore removed
   */
  @Deployment(resources = {
  ...
  /**
   * since assertion happens on variables, changing oneTaskCase definition to have manual
   * activation, case task behaviour changed, so manual activation is taken out
   */
  @Deployment(resources = {
  ...
  public void testOutputVariablesShouldNotExistAnymore() {
    ...
    caseService.withCaseExecution(caseTaskId)
        // set variables local
        .setVariableLocal("aVariable", "xyz").setVariableLocal("anotherVariable", 123)
        .manualStart();
  ...
  /**
   * default behaviour of manual activation changed - remove manual activation change definition of
   * oneTaskCase in order to allow suspension state
   */
  @Deployment(resources = { "org/camunda/bpm/engine/test/api/cmmn/oneCaseTaskCase.cmmn",
      "org/camunda/bpm/engine/test/api/cmmn/oneTaskCaseWithManualActivation.cmmn" })
  ...
  public void testNotBlockingCaseTask() {
    ...
    CaseInstance superCaseInstance = caseService.createCaseInstanceQuery()
        .caseDefinitionKey(ONE_CASE_TASK_CASE).singleResult();
