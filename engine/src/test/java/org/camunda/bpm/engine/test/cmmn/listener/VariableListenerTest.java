...
public class VariableListenerTest extends PluggableProcessEngineTestCase {
  ...
  public void testAnyEventListenerByClass() {
    ...
    caseService.withCaseExecution(caseInstance.getId())
        .setVariable("anInstanceVariable", "anInstanceValue").execute();

    // then the listener is not invoked
    assertTrue(LogVariableListener.getInvocations().isEmpty());

    // when i set a variable on the human task (ie the source execution matters although the
    // variable ends up in the same place)
    caseService.withCaseExecution(taskExecution.getId())
        .setVariableLocal("aTaskVariable", "aTaskValue").execute();

    // then the listener is invoked
    assertEquals(1, LogVariableListener.getInvocations().size());

    DelegateVariableInstanceSpec.fromCaseExecution(taskExecution).event(VariableListener.CREATE)
        .name("aTaskVariable").value("aTaskValue")
    ...
    caseService.withCaseExecution(taskExecution.getId())
        .setVariable("aTaskVariable", "aNewTaskValue").execute();

    // then the listener is invoked
    assertEquals(1, LogVariableListener.getInvocations().size());
    DelegateVariableInstanceSpec.fromCaseExecution(taskExecution).event(VariableListener.UPDATE)
        .name("aTaskVariable").value("aNewTaskValue").activityInstanceId(taskExecution.getId())
        .matches(LogVariableListener.getInvocations().get(0));
    LogVariableListener.reset();
    ...
    // then the listener is invoked
    assertEquals(1, LogVariableListener.getInvocations().size());
    DelegateVariableInstanceSpec.fromCaseExecution(taskExecution).event(VariableListener.DELETE)
        .name("aTaskVariable").value(null).activityInstanceId(taskExecution.getId())
        .matches(LogVariableListener.getInvocations().get(0));
  ...
  public void testVariableListenerInvokedFromSourceScope() {
    ...
    caseService.withCaseExecution(caseInstance.getId()).setVariable("aTaskVariable", "aTaskValue")
        .execute();
    ...
    caseService.withCaseExecution(taskExecution.getId()).setVariable("aTaskVariable", "aTaskValue")
        .execute();

    // then the listener is invoked
    assertEquals(1, LogVariableListener.getInvocations().size());

    DelegateVariableInstanceSpec.fromCaseExecution(caseInstance).sourceExecution(taskExecution)
        .event(VariableListener.UPDATE).name("aTaskVariable").value("aTaskValue")
  ...
  public void testListenerSourceExecution() {
    ...
    DelegateVariableInstanceSpec.fromCaseExecution(caseInstance).sourceExecution(taskExecution)
        .event(VariableListener.CREATE).name("aTaskVariable").value("aTaskValue")
        .matches(LogVariableListener.getInvocations().get(0));
  ...
  public void FAILING_testListenerDoesNotInterfereWithHistory() {
    ...
    caseService.withCaseExecution(caseInstance.getId()).setVariableLocal("variable", "value1")
        .execute();
  ...
  public void testListenerInvocationFinishesBeforeSubsequentInvocations() {
    ...
    caseService.withCaseExecution(taskExecution.getId()).setVariableLocal("variable", "value1")
        .execute();
    ...
    // there are four invocations since both listeners have set "value2" and both were again
    // executed, i.e. 2*2 = 4

    for (int i = 2; i < 6; i++) {
      DelegateVariableInstanceSpec.fromCaseExecution(taskExecution).event(VariableListener.UPDATE)
          .name("variable").value("value2")
  ...
  public void testTwoListenersOnSameScope() {
    ...
    caseService.withCaseExecution(taskExecution.getId()).setVariableLocal("testVariable", "value1")
        .execute();
    ...
    DelegateVariableInstanceSpec.fromCaseExecution(taskExecution).event(VariableListener.CREATE)
        .name("testVariable").value("value1").matches(LogVariableListener.getInvocations().get(0));

    assertEquals(1, LogAndUpdateVariableListener.getInvocations().size());

    DelegateVariableInstanceSpec.fromCaseExecution(taskExecution).event(VariableListener.CREATE)
        .name("testVariable").value("value1")
        .matches(LogAndUpdateVariableListener.getInvocations().get(0));
  ...
  public void testVariableListenerByClassWithFieldExpressions() {
    ...
    assertEquals("injectedValue",
        LogInjectedValuesListener.getResolvedStringValueExpressions().get(0));
  ...
  public void testVariableListenerExecutionContext() {
    ...
    CaseExecutionContext executionContext = LogExecutionContextListener.getCaseExecutionContexts()
        .get(0);
  ...
  public void testInvokeBuiltinListenersOnly() {
    ...
    processEngineConfiguration.getDeploymentCache().getCaseDefinitionById(caseDefinition.getId())
  ...
  public void testVariableListenerWithProcessTask() {
    ...
    CaseExecution processTask = caseService.createCaseExecutionQuery()
    ...
    DelegateVariableInstanceSpec.fromCaseExecution(caseInstance).sourceExecution(processTask)
        .event(VariableListener.CREATE).name("aVariable").value("aValue")
        .matches(LogVariableListener.getInvocations().get(0));
