...
//  };
  ...
  public static DescriptionProvider JOB_EXECUTOR_DEC = new DescriptionProvider() {

    public ModelNode getModelDescription(Locale locale) {
      final ModelNode node = new ModelNode();
      node.get(DESCRIPTION).set("The configuration of a job executor");
      
      node.get(ATTRIBUTES, Attribute.NAME.getLocalName(), DESCRIPTION).set("The name of the job executor thread pool.");
      node.get(ATTRIBUTES, Attribute.NAME.getLocalName(), TYPE).set(ModelType.STRING);
      node.get(ATTRIBUTES, Attribute.NAME.getLocalName(), REQUIRED).set(true);
      node.get(ATTRIBUTES, Attribute.NAME.getLocalName(), DEFAULT).set("default");
  ...
  public static DescriptionProvider JOB_EXECUTOR_ADD = new DescriptionProvider() {
    public ModelNode getModelDescription(Locale locale) {      
      final ModelNode node = new ModelNode();
      node.get(DESCRIPTION).set("Adds a new job executor");
      node.get(OPERATION_NAME).set(ADD);
      return node;
    }
  };
