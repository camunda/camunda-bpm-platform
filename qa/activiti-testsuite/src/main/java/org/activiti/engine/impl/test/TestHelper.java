...
public abstract class TestHelper {
  ...
  private static ProcessArchive processArchive; 
  ...
  public static String annotationDeploymentSetUp(ProcessEngine processEngine, Class<?> testClass, String methodName) {
    ...
    if (deploymentAnnotation != null) {
      ...
      String deploymentName = ClassNameUtil.getClassNameWithoutPackage(testClass)+"."+methodName;  
      
      deploymentId = performDeployment(deploymentName, resources);
    }
    
    return deploymentId;
  }
  
  protected static String performDeployment(final String deploymentName, final String[] resources) {
    
    // perform a process archive deployment to the container managed process engine
    
    final ProcessArchiveService processArchiveService = PluggableActivitiTestCase.getProcessArchiveService();
    final ProcessArchiveContextExecutor processArchiveContextExecutor = PluggableActivitiTestCase.getProcessArchiveContextExecutor();
    ...
    for (String string : resources) {   
      byte[] readInputStream = IoUtil.readInputStream(classloader.getResourceAsStream(string), string);
      resourceMap.put(string, readInputStream);
    }
    
    processArchive = new ProcessArchive() {
  ...
  public static void annotationDeploymentTearDown(ProcessEngine processEngine, String deploymentId, Class<?> testClass, String methodName) {
    ...
    ProcessArchiveService processArchiveService = PluggableActivitiTestCase.getProcessArchiveService();
  ...
  public static String getBpmnProcessDefinitionResource(Class< ? > type, String name) {
    return type.getName().replace('.', '/') + "." + name + ".bpmn20.xml";
  }
