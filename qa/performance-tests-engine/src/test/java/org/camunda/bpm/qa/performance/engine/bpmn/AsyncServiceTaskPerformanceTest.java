...
public class AsyncServiceTaskPerformanceTest extends ProcessEnginePerformanceTestCase {
	  ...
	  @Test
	  @Deployment(resources = {"org/camunda/bpm/qa/performance/engine/bpmn/NFT_TEST_FLOW_Async_Service_20.bpmn"})
	  public void sequence20AsyncServiceTasks() {

	    performanceTest()
	        .step(new StartProcessInstanceStep(engine, "NFT_TEST_FLOW_Async_Service_20"))
	        .step(new ExecuteJobStep(engine))
	        .step(new ExecuteJobStep(engine))
	        .step(new ExecuteJobStep(engine))
	        .step(new ExecuteJobStep(engine))
	        .step(new ExecuteJobStep(engine))
