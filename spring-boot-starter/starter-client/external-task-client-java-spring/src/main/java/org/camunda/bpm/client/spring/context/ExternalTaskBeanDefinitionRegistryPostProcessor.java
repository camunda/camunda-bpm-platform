...
public class ExternalTaskBeanDefinitionRegistryPostProcessor implements BeanDefinitionRegistryPostProcessor {
    ...
    public ExternalTaskBeanDefinitionRegistryPostProcessor(
            Class<? extends SubscribedExternalTaskBean> subscribedExternalTaskBeanClass) {
            ...
            if (beanNamesForType != null) {
                for (String beanName : beanNamesForType) {
                    getSchedulingDefinition(beanName, registry.getBeanDefinition(beanName)).ifPresent(definition -> {
                        ...
                        BeanDefinitionBuilder builder = getBeanDefinitionBuilderForExternalTaskBean(listableBeanFactory,
                                definition);
    ...
    protected BeanDefinitionBuilder getBeanDefinitionBuilderForExternalTaskBean(ListableBeanFactory listableBeanFactory,
        ...
        BeanDefinitionBuilder builder = BeanDefinitionBuilder.genericBeanDefinition(subscribedExternalTaskBeanClass)
                ...
                .addPropertyValue("subscriptionInformation", definition.getSubscriptionInformation())
                .setDestroyMethodName("close");
        return builder;
    }

    protected Optional<ExternalTaskSubscriptionDefinition> getSchedulingDefinition(String beanName,
                                                                                   BeanDefinition beanDefinition) {
        return findSubscriptionAttributes(beanDefinition)
                .map(attributes -> new ExternalTaskSubscriptionDefinition(beanName, attributes));
    }
