...
/**
 * Slightly modified for the git-connector.
 * 
...
public abstract class AbstractConnectorTestBase {
  ...
  public static final ConnectorNode TMP_FOLDER = new ConnectorNode("//" + TMP_DIR_NAME, TMP_DIR_NAME, ConnectorNodeType.FOLDER);
  ...
  public void shouldUpdateSingleFileContentFromConnector() throws Exception {
    ...
    try {
      ...
      // when
      ContentInformation updatedContentInfo = connector.updateContent(destFileNode, new ByteArrayInputStream(inputBytes), null);
  ...
  public void shouldUpdateSingleFileContents() throws Exception {
    ...
    try {
      ...
      // when
      ContentInformation updatedContentInfo = connector.updateContent(fileNode, new ByteArrayInputStream(inputBytes), null);
  ...
  private InputStream getDiagramResourceAsStream(final String file) {
    return getClass().getResourceAsStream("/org/camunda/bpm/cycle/roundtrip/repository/" + file);
  }

  private void importFile(final Connector connector, final String file, final String connectorNodeName) throws Exception {
  ...
  private void assertCorrectLastModified(final Date beforeUpdate, final Date lastModified) {
  ...
  private void safeDeleteNode(final ConnectorNode destFileNode, final Connector connector) {
