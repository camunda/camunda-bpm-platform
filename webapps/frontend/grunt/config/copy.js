...
module.exports = {
  webapp_libs : {
    files: [
      ...
      { expand: true, cwd: 'camunda-commons-ui/cache/', src: ['deps.js'], dest: '<%= pkg.gruntConfig.libTargetDir %>/' },
      // { expand: true, cwd: 'node_modules/angular/', src: ['angular.js'], dest: '<%= pkg.gruntConfig.libTargetDir %>/' },
      { expand: true, cwd: 'libs/', src: ['globalize.js'], dest: '<%= pkg.gruntConfig.libTargetDir %>/' }
    ]
  }
};
