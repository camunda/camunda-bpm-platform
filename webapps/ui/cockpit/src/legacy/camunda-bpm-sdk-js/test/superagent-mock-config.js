...
function log() {
  ...
  var n = typeof process !== 'undefined' && process.env.CAMSDKLOG;
...
for (i = 0; i < 40; i++) {
  ...
  _store.user[id] = {
    ...
    email:
      fixturer.randomString(fixturer.random(4, 8)) +
      '@' +
      fixturer.randomString(fixturer.random(5, 7)) +
      '.com'
  };
}
_store.user.jonny1 = {
...
for (i = 0; i < 600; i++) {
  ...
  _store.variable[id] = {
    ...
    type: fixturer.randomItem(['integer', 'double', 'string']),
    value: 5,
...
function filter(src, data) {
  ...
  var notFilters = ['sortBy', 'sortOrder', 'firstResult', 'maxResults'];
  ...
  _.each(data, function(val, key) {
    ...
    if (beforeExp.test(key)) {
      ...
      /* jshint evil: false */
      where.like[key.split(likeExp).shift()] = new RegExp(
        ('' + val).slice(1).slice(0, -1),
        'g'
      );
    } else {
      where.exact[key] = val;
  ...
  found = _.size(where.like)
    ? _.filter(found, function(item) {
        var keep = true;
        // var realKey;
...
module.exports = [
  {
    ...
    get: function(match, fixture) {
      ...
      pathParts[pathParts.length - 1] = pathParts[pathParts.length - 1].split(
        '?'
      )[0];
      ...
      switch (resourceName) {
        case 'process-definition':
          ...
          if (action === 'form-variables') {
            var definition;
            if (pathParts[0] === 'key') {
              definition = _.findWhere(_store.processDefinition, {
                key: pathParts[1]
              });
            } else {
                  ...
                  links: [
                    {
                      method: 'GET',
                      href:
                        'http://localhost:8080/rest-test/process-instance/' +
                        instanceId,
                      rel: 'self'
                    }
                  ]
